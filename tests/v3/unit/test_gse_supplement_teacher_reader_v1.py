...
from mtare_topo.data import gse_supplement_teacher_reader_v1 as module
...
def test_real_codec_full_task_binding(tmp_path, monkeypatch, damage):
    ...
    sources = [dict(task=task, frame_rows=list(range(5*i,5*i+5)),
                    source_frame_count=15,source_sequence_id=i) for i in range(3)]
    arrays = dict(sensor_xyz_m=np.zeros((15,3),dtype=np.float64), yaw_deg=np.zeros(15,dtype=np.float64),
                  primitive_membership_code=np.ones((15,16,720),dtype=np.uint16))
    if damage == 'pose_drift': arrays['sensor_xyz_m'][14,0]=1.
    files = {}; plans = {}
    def pin(path):
    ...
    for field, values in arrays.items():
        ...
        plan = combine_plans(header,field,sources)
    ...
    np.savez(path,ranges_m=np.ones((3,5,16,720),dtype=np.float32),
        valid_mask=np.ones((3,5,16,720),dtype=np.uint8),
        relative_translation_current_sensor_m=np.zeros((3,5,3),dtype=np.float32),
        relative_yaw_current_sensor_deg=np.zeros((3,5),dtype=np.float32),
        frame_rows=np.arange(15,dtype=np.int32).reshape(3,5),source_sequence_ids=sequence)
    ...
    monkeypatch.setattr(module,'compile_supplement_teacher_scope',lambda root:scope)
    reader=module.SupplementTeacherReader(tmp_path,scope)
    ...
    if damage:
        ...
        assert len(result)==3 and result[-1]['source']['source_sequence_id']==2
