"""Taskwise supplemental evidence reader; one variable-size task resident.
...
from .gse_supplement_teacher_scope_v1 import compile_supplement_teacher_scope, combine_plans
...
class SupplementTeacherReader:
    def __init__(self, root, scope):
        self.root = Path(root).resolve(strict=True)
        if digest(compile_supplement_teacher_scope(self.root)) != digest(scope):
    ...
    def read_task(self, task):
        ...
        for field in FIELDS:
            ...
            if combine_plans(header, field, sources) != plan:
        ...
        n = len(sources)
        shapes = {'ranges_m': (n,5,16,720), 'valid_mask': (n,5,16,720),
                  'relative_translation_current_sensor_m': (n,5,3),
                  'relative_yaw_current_sensor_deg': (n,5), 'frame_rows': (n,5),
                  'source_sequence_ids': (n,)}
        if any(student_all[k].shape != shape for k, shape in shapes.items()):
            raise ValueError('all complete supplemental observations required')
        documents = {}
