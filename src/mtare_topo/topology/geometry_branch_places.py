...
class GeometryBranchPlaces:
    def __init__(self, *, direction_tolerance_deg, target_match_m, place_radius_m, support_observations,
                 retain_matching_partial_observations=False):
        ...
        self.retain_partial=retain_matching_partial_observations
    ...
    def observe(self,record,proposals,*,navigation_node_id):
        ...
        if self.retain_partial and self.active is not None:
            old=self.places[self.active]
            # Partial observations preserve identity only, never add support.
            # No cross-visit merge, missing-direction invention or larger radius.
            hits=[]
            for group in groups:
                target,direction,_=group[0]
                hits.append([f['id'] for f in old['frontiers']
                    if direction@np.asarray(f['direction_world'])>=self.cosine
                    and np.linalg.norm(target-np.asarray(f['target_xyz_m']))<=self.target_distance])
            compatible=(len(groups)>=2 and np.linalg.norm(xyz-np.asarray(old['observer_xyz_m']))<=self.radius
                and all(len(h)==1 for h in hits) and len({h[0] for h in hits if len(h)==1})==len(groups))
            if not compatible:self.active=None
        if len(groups)<3:
            if not self.retain_partial:self.active=None
            return dict(status='NO_MULTI_CONTINUATION_EVENT',place_id=None,
                retained_place_id=self.active,added_support=False)
