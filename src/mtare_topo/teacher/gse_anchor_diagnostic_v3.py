...
def diagnose_observation(bundle, *, range_error_bound_m):
    ...
    for group in groups:
        ...
        axis_starts=tuple(p["axis_start_index"] for p in group["paths"])
        row["axis_start_indices"]=list(axis_starts)
        if any(p is None for p in paths):
            row.update(status="UNKNOWN",reason="ROI_CLIP_GEOMETRY_AMBIGUITY",semantic_label=None)
        elif any(i+1>=len(p) for p,i in zip(paths,axis_starts)):
            row.update(status="UNKNOWN",reason="SOURCE_AXIS_OUTSIDE_CLIPPED_REFERENCE",semantic_label=None)
        else:
            support=junction_support(grid,anchor_m=anchor,incident_paths_m=paths,
                endpoint_keys=tuple(p["endpoint_key"] for p in group["paths"]),axis_start_indices=axis_starts)
            row.update(asdict(support));row["reference_paths_current_sensor_m"]=[p.tolist() for p in paths]
            row["branch_cell_evidence"]=[axis_evidence_detail(grid,s) for s in support.branch_support]
        rows.append(row)
