...
## 10. 源码补充与已实现的最小边界（同日后续）

本节收窄第6.4节中尚未实现的完整门面适配要求：**不新增凸多面体 region 标签作为训练前置，不把原数据没有的分区字段制造成新依赖。** 原180数据可以继续用于已经成立的几何／空间关系监督；完整区域归属不是所有学习任务的必要前置。

源码确认，`EndpointComposition.as_dict()` 只有 `operation=endpoint_union`、`node_id`、`degree`、`anchor_xyz_m` 和 `member_endpoints`；`SweptSuperellipseProvenanceField` 有各 operand 的采样轴线、截面及 union 距离；`CSGMeshProvenanceRaycaster` 用所有闭合基元的有序交点更新占据，输出 union 的最终退出。这些是可复用的真实构造几何，**没有已经分割好的每-node region 外壳、junction throat 多边形或其观测完整性字段**。没有字段不等于不能从现有几何发展算法，更不等于原始数据不能训练。

本轮只新增 [gse_local_opening.py](../src/mtare_topo/teacher/gse_local_opening.py) 中的 `section_positive_evidence`。名称虽位于局部开口路径，函数严格只输出 **section-positive evidence**：

- 用真实多边形世界坐标及截面参数进行精确重复去重；相反法向的同形截面不重复计数。没有容差半径、没有移动门面，近似重合仍是两个候选。
- 对全部唯一候选复用原 finite-ray 竞争。串联截面仍共同竞争；没有正见证保留 UNKNOWN，不判封闭。
- 同时返回去重前的全部计数／歧义与去重后的证据，保留每个原 cap 对应哪个几何截面。重复来源没有丢失。
- 明确 `region_membership_available=false`、`complete_event_labels_available=false`、`independent_opening_claim=false`；即使 T／Y 三个截面均有穿越见证，也不自动输出 junction 标签。

15项纯合成测试通过（0.15秒）：T／Y、分层、同观察两个区域的截面、相反法向重合cap、近似cap不去重、串联UNKNOWN、遮挡／无回波／回波恰落cap、缺真实多边形拒绝、旧target_node根本无输入接口。另有两个可运行反例：

1. 两束有限射线均不穿过未观测的中间隔板；有隔板和无隔板的场景可提供同一组 cap 与 first-return 证据。该输入不包含足以证明整个局部自由空间结构的信息。
2. 同一直廊中两道截面的射线证据固定，但 query 指向前一段或两截面之间的段时，边界成员不同。缺少 region reference 的函数不能唯一输出 membership。这是缺少查询语义的接口反例，不是主张必须生成任意新 region 网格。

### 当前字段真正支持的最小监督任务

现成教师可直接提供“观测到的基元形状及相对布局”目标：共同坐标轴线、截面、相对位移、夹角、高差；全部可见片段参与，不用GT预筛成员。可训练因果输入到这些局部几何／关系量的恢复，也可作为多实例 query 头的几何容量对照。应与同输入解析几何计算／非学习估计对照，不把确定性关系公式本身包装为创新。

新增 section-positive 核可为**给定截面查询**提供正射线证据测试，不自动变成 event／region 教师；以 GT cap 作为查询时必须标记 oracle-query 条件，不声称部署预测中心已成功。当前真实射线未被读取，所以正证据人口仍未知。

如果希望立刻用完整构造 incidence／degree／anchor 训练多实例结构头，而不等待可观测开口标签，需要明确采用“部分观测预测潜在构造结构”的监督任务，并报告不可辨样本、构造先验和无法由输入证实的目标；**这与‘只监督已证实局部开口’不同，不能静默把旧关系标签当强可观测真值。** 是否采用这一弱／潜在结构监督、怎样设置 UNKNOWN 与评分人口，是需要主线规格明确的方法决策，不由本轮算子自行决定。持续授权不等于可以偷偷变更标签语义。

最小下一动作因而不是读取全图或补任意region：由主线将预测中心query和上述几何目标先接成纯合成接口；真实180中的完整事件／成员若仍缺证据，保留为待明确的监督范围，不阻止已有几何受控训练。所有新增实际读取／teacher物化／训练仍单列准确卡和spec。
