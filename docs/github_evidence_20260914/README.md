...
- [逐目录实验导航](RUN_INDEX.md)：761 项原路径入口，附图片示例；不只列成功实验。
...
`included.jsonl` 的 Git 内容哈希对应首次归档提交 `cbdca53e06741049442b24fe7745e39c8b6cdec0`；后续状态与上传回执单独提交，不能要求后续更新的 project_status.json 仍与旧快照哈希相同。原实验文件不变。只有回执包含 `published: true` 且其提交已推送，才表示附件正式发布；准备阶段的状态不代表最终结果。
