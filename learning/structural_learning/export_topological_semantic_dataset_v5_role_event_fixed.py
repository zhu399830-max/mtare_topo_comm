...
def make_pairs(records_by_split: dict[str, list[dict[str, Any]]], output: Path, config: dict[str, Any], seed: int) -> dict[str, Any]:
    ...
    for split, records in records_by_split.items():
        ...
        for first, second, sd, td, count_delta in candidates:
            if sd >= float(config["positive_min_surface_distance"]) and td <= float(config["positive_max_topology_distance"]) and count_delta <= int(config["positive_max_exit_count_delta"]):
                kind = "positive_invariance"
            elif sd <= float(config["negative_max_surface_distance"]) and td >= float(config["negative_min_topology_distance"]):
                kind = "hard_negative"
            else:
                continue
    ...
    return {"count": len(rows), "by_split": {split: dict(Counter(row["pair_type"] for row in rows if row["split"] == split)) for split in records_by_split}, "selection": config}
