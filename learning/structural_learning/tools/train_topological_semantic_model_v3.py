...
class Net(nn.Module):
    ...
    def forward(self, cur, hist, valid, dt, poses):
        ...
        return {"z": F.normalize(structural,dim=1), "role": F.normalize(self.role(structural),dim=1),
                ...
                "coverage": torch.sigmoid(self.coverage_head(reverse_gradient(structural, 0.25))),
                "dynamic": torch.sigmoid(self.dynamic(temporal)) if self.dynamic is not None else None,
...
def loss_fn(p,b,weights, ordered):
    ...
    if ordered:
        dynamic = masked_l1(p["dynamic"], b["dynamic"], b["dynamic_mask"])
        ...
        dynamic = p["z"].sum()*0
        state_aux = p["z"].sum()*0
    parts={"direction":direction,"distance":distance,"area":area,"exit":exit_loss,"count":count,"center":center,"width":width,"length":length,"static":static,"role":role,"coverage":coverage,"dynamic":dynamic,"state_aux":state_aux}
...
def main():
    ap=argparse.ArgumentParser(); ap.add_argument("--dataset-root",type=Path,default=Path("results/topological_semantic_dataset_v4_supervision_fixed")); ap.add_argument("--out",type=Path,required=True); ap.add_argument("--seed",type=int,default=20260806); args=ap.parse_args(); cfg={"model":{"base":16,"dim":96},"train":{"batch_size":64,"epochs":20,"lr":8e-4},"loss":{"direction":1,"distance":1,"area":.5,"exit":1,"count":.5,"center":.25,"width":.25,"length":.25,"static":1,"role":1,"coverage":0.8,"dynamic":1.2,"state_aux":0.35}}
