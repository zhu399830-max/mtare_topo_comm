    ...
    def __getitem__(self, i):
        with np.load(self.files[i], allow_pickle=False) as z:
            return {
                ...
                "coverage": np.asarray([z["student_input_current"].astype(np.float32)[0].mean(), z["history_valid_mask"].astype(np.float32).mean()], dtype=np.float32),
            }
...
class Net(nn.Module):
    def __init__(self, ordered: bool, cfg, use_history: bool = True):
        ...
        self.structure_head = nn.Sequential(nn.Linear(d, d), nn.SiLU(), nn.Linear(d, d), nn.SiLU())
        self.coverage_head = nn.Sequential(nn.Linear(d, 32), nn.SiLU(), nn.Linear(32, 2))
        ...
        self.dynamic = nn.Sequential(nn.Linear(d, d), nn.SiLU(), nn.Linear(d,4)) if ordered else None
        self.temporal_state_aux = nn.Sequential(nn.Linear(d, 48), nn.SiLU(), nn.Linear(48, 6)) if ordered else None
        self.dynamic_motion = nn.Sequential(nn.Linear(d + 24, d), nn.SiLU(), nn.Linear(d, 4)) if ordered else None
    ...
    def forward(self, cur, hist, valid, dt, poses):
        ...
        temporal = zi[:, :zc.shape[1]]
        z = self.fuse(torch.cat([zc, temporal], 1)); s = self.static_head(z)
        structural = self.structure_head(s)
        return {"z": F.normalize(structural,dim=1), "role": F.normalize(self.role(structural),dim=1),
                ...
                "coverage": torch.sigmoid(self.coverage_head(reverse_gradient(structural, 1.0))),
                "dynamic": torch.sigmoid(self.dynamic_motion(torch.cat([temporal, self.pose(aux[:, -1])], 1))) if self.dynamic is not None else None,
                "temporal_state_aux": self.temporal_state_aux(temporal) if self.temporal_state_aux is not None else None}
...
def loss_fn(p,b,weights, ordered):
    ...
    coverage = F.mse_loss(p["coverage"], b["coverage"])
    role_center = p["role"] - p["role"].mean(0, keepdim=True)
    cov_center = b["coverage"] - b["coverage"].mean(0, keepdim=True)
    decorrelation = ((role_center.T @ cov_center) / max(len(role_center), 1)).pow(2).mean()
    if ordered:
        dynamic_weight = 1.0 + 5.0 * b["dynamic"]
        dynamic = (torch.abs(p["dynamic"] - b["dynamic"]) * b["dynamic_mask"] * dynamic_weight).sum() / (b["dynamic_mask"] * dynamic_weight).sum().clamp_min(1)
        last = b["history_valid_mask"].sum(1).long().clamp_min(1)-1
        pose_target = b["relative_poses"][torch.arange(len(last),device=last.device), last]
        dt_target = b["history_time_deltas"][torch.arange(len(last),device=last.device), last].unsqueeze(1) / 4.0
        state_aux = F.smooth_l1_loss(p["temporal_state_aux"], torch.cat([pose_target, dt_target],1))
    else:
        dynamic = p["z"].sum()*0
        state_aux = p["z"].sum()*0
    parts={"direction":direction,"distance":distance,"area":area,"exit":exit_loss,"count":count,"center":center,"width":width,"length":length,"static":static,"role":role,"coverage":coverage,"decorrelation":decorrelation,"dynamic":dynamic,"state_aux":state_aux}
...
def run_epoch(model, loader, opt, dev, cfg, ordered, pair_loader=None):
    ...
    for raw in loader:
        ...
        if pit:
            try: r1,r2,typ=next(pit); fw=lambda r: model(r["cur"].to(dev),r["hist"].to(dev),r["valid"].to(dev),r["dt"].to(dev),r["pose"].to(dev)); pl=pair_loss(fw(r1),fw(r2),typ); total=total+.8*pl; parts["pair"]=float(pl.detach())
...
def main():
    ap=argparse.ArgumentParser(); ap.add_argument("--dataset-root",type=Path,default=Path("results/topological_semantic_dataset_v4_supervision_fixed")); ap.add_argument("--out",type=Path,required=True); ap.add_argument("--seed",type=int,default=20260806); args=ap.parse_args(); cfg={"model":{"base":16,"dim":96},"train":{"batch_size":64,"epochs":20,"lr":8e-4},"loss":{"direction":1,"distance":1,"area":.5,"exit":1,"count":.5,"center":.25,"width":.25,"length":.25,"static":1,"role":0.35,"coverage":2.0,"decorrelation":4.0,"dynamic":1.5,"state_aux":0.5}}
