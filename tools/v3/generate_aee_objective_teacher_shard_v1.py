#!/usr/bin/env python3
"""Generate objective structural targets for one frozen AEE sensor shard."""
...
def main() -> int:
    ...
    sensor_path = args.sensor_shard.resolve()
    map_path = args.complete_map.resolve()
    if sha256(sensor_path) != args.sensor_shard_sha256:
        raise RuntimeError("sensor shard identity drift")
    if sha256(map_path) != args.complete_map_sha256:
        raise RuntimeError("complete-map identity drift")
    ...
    expected_prefix = f"{args.trajectory_id}:"
    if not np.all(np.char.startswith(frame_ids, expected_prefix)):
        raise RuntimeError("trajectory identity does not match sensor frame IDs")
    oracle = LayeredGTMapOracle.from_ply(map_path, LayeredGTMapConfig())
    ...
    for index, (xyz, yaw) in enumerate(zip(sensor["sensor_xyz_m"], sensor["yaw_deg"])):
        ...
        support_z.append(float(prediction.evidence.selected_center_support_z_m))
    payload = {
    ...
    summary = {
        "schema_version": "aee_objective_teacher_shard_summary_v1",
