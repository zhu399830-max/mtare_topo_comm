#!/usr/bin/env python3
"""Run topology replays with the qualified risk-calibrated GSE observation."""
...
PERCEPTION_COMPONENT_STATUS = "FAIL_GSE_PERCEPTION_VALIDATION_V1"
CORRECTED_PERCEPTION_STATUS = "PASS_GSE_RISK_CALIBRATED_PERCEPTION_VALIDATION_V2"
...
def _overlay_risk_calibrated_slope(
    base: Mapping[str, np.ndarray],
    corrected: Mapping[str, np.ndarray],
    *,
    expected_count: int = 24462,
) -> dict[str, np.ndarray]:
    base_indices = np.asarray(base["global_sequence_index"], dtype=np.int64)
    corrected_indices = np.asarray(corrected["global_sequence_index"], dtype=np.int64)
    slope = np.asarray(corrected["corrected_slope_deg"], dtype=np.float32)
    if (
        base_indices.shape != (expected_count,)
        or corrected_indices.shape != base_indices.shape
        or slope.shape != base_indices.shape
        or len(np.unique(base_indices)) != expected_count
        or len(np.unique(corrected_indices)) != expected_count
        or set(base_indices.tolist()) != set(corrected_indices.tolist())
        or not np.all(np.isfinite(slope))
    ):
        raise RuntimeError("risk-calibrated slope identity drift")
    corrected_row = {int(value): row for row, value in enumerate(corrected_indices)}
    reordered_slope = slope[
        np.asarray([corrected_row[int(value)] for value in base_indices], dtype=np.int64)
    ]
    output = dict(base)
    output["slope_deg"] = reordered_slope
    return output
...
def _gse_observations(
    ...
    for seed in (0, 1, 2):
        ...
        corrected = _load_archive(
            corrected_perception_run
            / f"artifacts/risk_calibrated_slope_c09/seed{seed}_outputs.npz"
        )
        arrays = _overlay_risk_calibrated_slope(arrays, corrected)
...
def _run_sweep(
    *,
    ...
    association_backends_by_world: Mapping[str, Any] | None = None,
    node_generation_backends_by_world: Mapping[str, Any] | None = None,
) -> dict[str, Any]:
    ...
    if association_mode in frozen_modes:
        if association_backends_by_world is None or set(association_backends_by_world) != set(worlds):
            raise RuntimeError("frozen ensemble backend/world contract mismatch")
    elif association_backends_by_world is not None:
        raise RuntimeError("only the frozen ensemble method may receive learned backends")
    if association_mode in {
        "frozen_ensemble_with_open_set_node_gate",
        "frozen_event_node_ensemble",
    }:
        if node_generation_backends_by_world is None or set(node_generation_backends_by_world) != set(worlds):
            raise RuntimeError("frozen node-generation backend/world contract mismatch")
    elif node_generation_backends_by_world is not None:
        raise RuntimeError("only the open-set node-gated method may receive node backends")
    rows = []
                ...
                for parent, world in worlds.items():
                    replay_results.append(
                        replay_typed_observation_world(
                            ...
                            association_backend=(
                                association_backends_by_world[parent]
                                if association_backends_by_world is not None else None
                            ),
                            node_generation_backend=(
                                node_generation_backends_by_world[parent]
                                if node_generation_backends_by_world is not None else None
                            ),
                        )
                    )
            row = {
    ...
    summary = {
        ...
        "association_mode": association_mode,
        "association_backend": (
            "frozen_v2_exit_token_three_seed_equal_weight_ensemble"
            if association_backends_by_world is not None else None
        ),
        "node_generation_backend": (
            "frozen_v2_matchability_three_seed_equal_weight_ensemble"
            if node_generation_backends_by_world is not None else None
        ),
...
def evaluate(
    ...
    paths = [path.resolve() for path in (training_run, perception_component_run, corrected_perception_run, dataset_run, teacher_run, output_dir.parent)]
    for path in paths:
        path.relative_to(PROJECT_ROOT.resolve())
    training_run, perception_component_run, corrected_perception_run, dataset_run, teacher_run, _ = paths
    output_dir = output_dir.resolve()
    source_status = {
        training_run: TRAINING_STATUS,
        corrected_perception_run: CORRECTED_PERCEPTION_STATUS,
    ...
    source_verification[perception_component_run.name] = verify_failed_component_run_seal(
        PROJECT_ROOT, perception_component_run, PERCEPTION_COMPONENT_STATUS
    )
    training_state = load_json(training_run / "RUN_STATE.json")
    training_summary = load_json(training_run / "metrics/summary.json")
    perception_state = load_json(corrected_perception_run / "RUN_STATE.json")
    perception_summary = load_json(corrected_perception_run / "metrics/summary.json")
    ...
    if (
        ...
        or perception_state.get("overall_status") != CORRECTED_PERCEPTION_STATUS
        or perception_summary.get("overall_status") != CORRECTED_PERCEPTION_STATUS
        or complete_gate.get("passed") is not True
        ...
        or any(
            ...
            for source in (training_summary, perception_summary, calibration_summary, complete_gate)
        )
    ):
    ...
    learned_observations = _gse_observations(training_run, corrected_perception_run, contracts)
    ...
    exit_observations = _exit_only_observations(perception_component_run)
    ...
    summary = {
        "schema_version": "gse_offline_topology_validation_v2",
