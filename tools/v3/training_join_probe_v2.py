#!/usr/bin/env python3
"""Single frozen query-coverage diagnostic, no training or label promotion."""
import argparse
import hashlib
import json
...
from mtare_topo.governance_training_join_probe_v2 import SCHEMA,SLUG,POLICY,validate_card
...
def freeze():
    ...
    approval=dict(status="APPROVED",approved_by="user-standing-scope-authorization",approved_at="2026-09-08",
        ...
        scope="One existing C01 observation225,frames289-293; reader decodes7 cached observations35frames of S01_flat_tree_small_C01__c1_mixed only. Reuse sealed V6 partial targets; one pinned GPU feature cache and raw source input joined with sealed V6 target; no model predictions, full labels or training",
        confirmation_reference="Standing user authorization for in-scope local supervision development and versioned paired validation. Sealed V4 remains unchanged; no population retry, protected data, training or complete-label promotion.")
    ...
    command=["env","OMP_NUM_THREADS=1","OPENBLAS_NUM_THREADS=1","MKL_NUM_THREADS=1","PYTHONHASHSEED=20260906",PYTHON,
        "tools/v3/training_join_probe_v2.py","--spec",str(root/SPEC),"--run-dir",str(root/run)]
    ...
    spec=dict(schema_version="v3_run_spec_v1",gate=3,date="20260908",slug=SLUG,seed=20260906,operation="data_export",
        ...
        question="Does the sealed GPU feature cache join exactly to original scans and V6 targets through the real training input interface?",
        method="Read unchanged pinned cache/input/target bytes; use exact original CUDA projection for features and independently canonical CPU projection for teacher grid; verify original point coordinates and identity, construct deterministic patches and grid, and bind the loss-only context. No inference or training; explicit1m opening parameter is diagnostic only.",
        ...
        wall_time_cap_s=1800,estimated_cost=dict(compute="CPU teacher grid plus original CUDA projection only;1 analyzed observation5frames; collateral cached7 observations35frames from1task",host_ram_gb=4,gpu_vram_gb=1,disk_gb=.5,wall_time_hours=.5),
        acceptance_criteria=["Exact observation225; fixed sources/poses and geometry; no targets may change; grid derived from original scans; diagnostic counts only; no C08-C10, new scans, models or training.",
        ...
        expected_evidence=["Source snapshot,command/environment,per-query source-bound coverage, unknown and conditional counts,source frame indices,raw log,summary,RUN_STATE,SHA256 seal;failure preserves active task."],
...
def execute(spec,run):
    ...
    from mtare_topo.teacher.gse_training_join_probe_v2 import inspect_join
    ...
    try:
        ...
        with (run/"logs/observations.jsonl").open("x") as log:
            for row in card["scope"]["entries"]:
                if row["task"]!=POLICY["task"]:continue
                active=row["task"]
                write(run/"artifacts/active_observation.json",row)
                bundles=reader.read_task(active)
                rows=[]
                for bundle in bundles:
                    if bundle["source"]["source_sequence_id"]!=POLICY["sequence"]:continue
                    rows.append(inspect_join(bundle, root, reader.opened))
                storage=write_evidence(run/"artifacts"/(active+".json.gz"),dict(task=active,observations=rows))
                entry=dict(task=active,observations=len(rows),joined_observations=sum(r["joined_observations"] for r in rows))
        ...
        summary=dict(status="FAILED" if error else "TRAINING_JOIN_PROBE_COMPLETE",error=error,active_task=active,
