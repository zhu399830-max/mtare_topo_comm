...
from mtare_topo.governance_population_terminal_v1 import SCHEMA,SLUG,POLICY,validate_card
from mtare_topo.governance_surface_selection import digest
from mtare_topo.governance_surface_material import read_pinned
from mtare_topo.data.gse_surface_population_teacher_scope_v1 import compile_population_teacher_scope
...
def freeze():
    ...
    scope=compile_population_teacher_scope(root)
    approval=dict(status="APPROVED",approved_by="user-standing-scope-authorization",approved_at="2026-09-07",
        ...
        scope="Fixed3360C01-C07observations;original fiveframe input caches,poses,source codes and constructions;terminal reference evidence only;no labels or training",
        confirmation_reference="User explicitly authorized automatic local teacher development: 别阻断了直接开始吧. User approved fixed3360 C01-C07 development population and autonomous in-scope diagnostics; precise original fiveframe source expansion for observation evidence, no sample reselection or labels.")
    ...
    command=["env","CUDA_VISIBLE_DEVICES=","OMP_NUM_THREADS=1","OPENBLAS_NUM_THREADS=1","MKL_NUM_THREADS=1","PYTHONHASHSEED=20260906",PYTHON,
        "tools/v3/population_terminal_v1.py","--spec",str(root/SPEC),"--run-dir",str(root/run)]
    ...
    spec=dict(schema_version="v3_run_spec_v1",gate=3,date="20260907",slug=SLUG,seed=20260906,operation="data_export",
        ...
        question="Which fixed3360 observations support a source terminal cap with a finite uniquely owned first return?",
        method="Original degree-one cap triangles .05m/64; original fiveframe ray first-return and unique source membership; open10m original-anchor ROI; partial reference evidence, not semantic labels.",
        ...
        wall_time_cap_s=1800,estimated_cost=dict(compute="CPU only;3360 fiveframe observations,16800 selected frames,210 source tasks; cached scans only",host_ram_gb=4,gpu_vram_gb=0,disk_gb=.5,wall_time_hours=.5),
        acceptance_criteria=["Exact fixed3360 selection,source hashes,poses and membership; no C08-C10, new scan rendering, models, labels or training.",
...
def execute(spec,run):
    ...
    from mtare_topo.teacher.gse_terminal_diagnostic_v1 import diagnose_observation
    ...
    try:
        ...
        reader=PopulationTeacherReader(root,card["scope"])
        with (run/"logs/observations.jsonl").open("x") as log:
            for row in card["scope"]["entries"]:
                ...
                write(run/"artifacts"/(active+".json"),dict(task=active,observations=rows))
        ...
        summary=dict(status="FAILED" if error else "POPULATION_TERMINAL_DIAGNOSTIC_COMPLETE",error=error,active_task=active,
