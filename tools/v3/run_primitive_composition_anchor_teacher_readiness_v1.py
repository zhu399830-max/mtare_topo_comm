#!/usr/bin/env python3
"""Run and seal one immutable fit/C07 composition-anchor Teacher readiness."""
...
PASS = "PASS_PRIMITIVE_COMPOSITION_ANCHOR_TEACHER_READINESS_V1"
SCIENCE_FAIL = "FORMAL_MACHINE_PASS_RESEARCH_CONTRACT_FAIL_PRIMITIVE_COMPOSITION_ANCHOR_TEACHER_READINESS_V1"
SYSTEM_FAIL = "FAIL_SYSTEM_PRIMITIVE_COMPOSITION_ANCHOR_TEACHER_READINESS_V1"
CARD_STATUS = "APPROVED_FOR_ONE_IMMUTABLE_PRIMITIVE_COMPOSITION_ANCHOR_TEACHER_READINESS_V1"
...
def _tree_hash(path: Path) -> str:
    ...
    for value in files:
        ...
        digest.update(relative); digest.update(bytes.fromhex(_sha(value)))
    return digest.hexdigest()


def _dataset_contract() -> dict[str, str]:
    result = {}
    ...
    for split in ("fit", "c07"):
        ...
        for task in tasks:
            construction = construction_root / split / f"{task}.json"
            result[f"construction/{split}/{task}.json"] = _sha(construction)
            sensor = sensor_root / split / f"{task}.zarr"
            for array in ("sensor_xyz_m", "yaw_deg"):
                result[f"sensor_pose/{split}/{task}/{array}"] = _tree_hash(sensor / array)
            result[f"p1b/{split}/{task}"] = _tree_hash(teacher_root / split / f"{task}.zarr")
            result[f"sidecar/{split}/{task}"] = _tree_hash(sidecar_root / split / f"{task}.zarr")
    return result
...
def main() -> int:
    ...
    started = time.monotonic(); overall = SYSTEM_FAIL; error = None
    audit = {}; before_files = {}; after_files = {}; before_data = {}; after_data = {}
    subprocesses = []; peak_rss_kib = None
    try:
        if run.name != RUN_ID or load_json(run / "RUN_STATE.json").get("state") != "CREATED_NOT_EXECUTED":
            raise RuntimeError("composition-anchor readiness executes exactly once")
        card = load_json(PROJECT_ROOT / spec["data_card"])
        validation = validate_data_card(card)
        if not validation.passed or card.get("status") != CARD_STATUS:
            raise RuntimeError(f"composition-anchor Data Card invalid: {validation.errors}")
        for relative, expected in spec["frozen_inputs"].items():
            path = PROJECT_ROOT / relative
            if not path.is_file():
                raise RuntimeError(f"composition-anchor frozen input missing: {relative}")
        ...
        for source in (P1A, P1B, SIDECAR):
            if load_json(source / "RUN_STATE.json").get("state") != "COMPLETED":
                raise RuntimeError(f"composition-anchor source incomplete: {source.name}")
        ...
        before_data = _dataset_contract()
        if before_data != spec["frozen_dataset_contract"]:
            raise RuntimeError("composition-anchor fit/C07 data contract drift")
        write_json(run / "config/dataset_integrity_before.json", before_data)
        ...
        if result.returncode:
            raise RuntimeError("composition-anchor readiness unit tests failed")

        output = run / "metrics/teacher_readiness"
        command = [
            PYTHON,
            str(PROJECT_ROOT / "tools/v3/execute_primitive_composition_anchor_teacher_readiness_v1.py"),
        ...
        (run / "config/readiness_command.txt").write_text(" ".join(command) + "\n", encoding="utf-8")
        with (run / "logs/01_teacher_readiness.log").open("w", encoding="utf-8") as stream:
            result = subprocess.run(
                ["/usr/bin/time", "-v", *command], cwd=PROJECT_ROOT, env=env,
                stdout=stream, stderr=subprocess.STDOUT, text=True,
                timeout=3600, check=False,
            )
        subprocesses.append({"stage": "teacher_readiness", "returncode": result.returncode})
        log = (run / "logs/01_teacher_readiness.log").read_text(encoding="utf-8")
        match = re.search(r"Maximum resident set size \(kbytes\):\s*(\d+)", log)
        peak_rss_kib = int(match.group(1)) if match else None
        if result.returncode:
            raise RuntimeError("composition-anchor readiness process failed")
        audit = load_json(output / "summary.json")
        if (
            audit.get("fit_rows_read") != 426_552
            or audit.get("c07_rows_read") != 64_644
            or audit.get("sensor_range_rows_read") != 0
            or audit.get("c08_rows_read") != 0
            or audit.get("model_forward_rows") != 0
            or audit.get("optimizer_steps") != 0
            or audit.get("decision") not in {
                "PROCEED_TO_COMPOSITION_ANCHOR_MODEL_READINESS",
                "STOP_LEARNED_COMPOSITION_ANCHOR_CANDIDATE",
            }
        ):
            raise RuntimeError("composition-anchor readiness evidence contract drift")
        if peak_rss_kib is None or peak_rss_kib > 4 * 1024 * 1024:
            raise RuntimeError("composition-anchor readiness host RAM evidence missing or exceeded")
        required = [
            output / "summary.json", output / "per_task.json", output / "per_task.csv",
            output / "figure_source.json",
            *[
                output / f"primitive_composition_anchor_teacher_readiness.{suffix}"
                for suffix in ("png", "pdf", "svg")
            ],
        ]
        if any(not path.is_file() or path.stat().st_size == 0 for path in required):
            raise RuntimeError("composition-anchor readiness evidence incomplete")
        for suffix in ("png", "pdf", "svg"):
            shutil.copy2(
                output / f"primitive_composition_anchor_teacher_readiness.{suffix}",
                run / "previews",
        ...
        after_data = _dataset_contract()
        if after_data != before_data:
            raise RuntimeError("composition-anchor data changed during readiness")
        write_json(run / "config/dataset_integrity_after.json", after_data)
        after_files = {relative: _sha(PROJECT_ROOT / relative) for relative in before_files}
        if after_files != before_files:
            raise RuntimeError("composition-anchor frozen inputs changed")
        write_json(run / "config/source_integrity_after.json", after_files)
        if time.monotonic() - started > 3600:
            raise RuntimeError("composition-anchor readiness wall-time exceeded")
        if sum(path.stat().st_size for path in run.rglob("*") if path.is_file()) > 100 * 1024**2:
            raise RuntimeError("composition-anchor readiness output exceeded 100 MiB")
        overall = PASS if audit.get("scientific_pass") is True else SCIENCE_FAIL
    except Exception as exc:
        error = f"{type(exc).__name__}: {exc}"
        (run / "logs/failure_traceback.log").write_text(traceback.format_exc(), encoding="utf-8")

    duration = time.monotonic() - started
    system_pass = error is None and overall in {PASS, SCIENCE_FAIL}
    write_json(run / "metrics/summary.json", {
        "schema_version": "primitive_composition_anchor_teacher_readiness_outer_v1",
        "overall_status": overall, "system_evidence_pass": system_pass,
        "primitive_composition_anchor_teacher_ready": (
            system_pass and audit.get("scientific_pass") is True
        ),
        "error": error, "subprocesses": subprocesses, "audit": audit,
        "decision": audit.get("decision"), "peak_host_rss_kib": peak_rss_kib,
        "duration_seconds": duration,
        "fit_rows_read": int(audit.get("fit_rows_read", 0)),
        "c07_rows_read": int(audit.get("c07_rows_read", 0)),
        "sensor_range_rows_read": 0, "c08_rows_read": 0,
        "c09_c10_worlds_read": 0, "model_forward_rows": 0,
        "optimizer_steps": 0, "graph_replays": 0, "mtare_worlds_read": 0,
        "source_unchanged": bool(
            before_files and before_files == after_files
            and before_data and before_data == after_data
        ),
    })
    write_json(run / "RUN_STATE.json", {
        "schema_version": "v3_run_state_v1", "run_id": RUN_ID,
        "state": "COMPLETED" if system_pass else "FAILED",
    ...
    write_json(run / "artifacts/seal_summary.json", {
        "schema_version": "primitive_composition_anchor_teacher_readiness_seal_v1",
    ...
    print(json.dumps({
        ...
        "scientific_pass": audit.get("scientific_pass"),
        "evidence_files": entries, "expected_evidence_files": expected_entries,
    }, indent=2))
    return 0 if system_pass and entries == expected_entries else 2
