"""One immutable real307 R0/R1/R2 partial-reference paired training."""
...
from mtare_topo.governance_development_corrective_train_v1 import SCHEMA,SLUG,POLICY,validate_card
...
from mtare_topo.representation.development_paired_training import build_model,state_sha256,batch_schedule,forward_observation
...
def freeze():
    ...
    initial=build_model(seed=0,device='cpu')
    ...
    approval=dict(status='APPROVED',approved_by='user-standing-scope-authorization',approved_at='2026-09-09',
        ...
        scope='Exactly307 existing observations250fit27calibration30development; R0/R1/R2 each2000updates micro1accum4; no encoder training, no new labels or test data.',
        confirmation_reference='User continuing autonomous authorization; PLAN and GSE_BRANCH_SELECTION_CORRECTIVE_V1 authorize one registered branch matching/class-balance correction on the same307 assets after19 software tests. Original node supervision and independent scoring unchanged; no repeated corrective search.')
    ...
    files=sorted({str(p.relative_to(ROOT)) for folder in ('src/mtare_topo','tools/v3') for p in (ROOT/folder).rglob('*.py')}|{CARD,MATCHING,'docs/GSE_BRANCH_SELECTION_CORRECTIVE_V1.md'})
    command=['env','OMP_NUM_THREADS=1','OPENBLAS_NUM_THREADS=1','MKL_NUM_THREADS=1','PYTHONHASHSEED=0','CUBLAS_WORKSPACE_CONFIG=:4096:8',PYTHON,
             'tools/v3/development_corrective_train_v1.py','--spec',str(ROOT/SPEC),'--run-dir',str(ROOT/RUN)]
    spec=dict(schema_version='v3_run_spec_v1',gate=3,date='20260909',slug=SLUG,seed=0,operation='training',
        ...
        question='Does the single registered branch matching and class-balance correction improve witnessed branch recovery under the unchanged paired inputs and independent scoring?',
        method='Existing point encoder and block query/branch head488584parameters; frozen context; shared seed0 schedule; AdamW1e-3wd1e-4 micro1accum4; each2000updates; no best checkpoint selection; only branch matching and balanced group classification changed per GSE_BRANCH_SELECTION_CORRECTIVE_V1.',
        baseline='R0 includes coordinates and inherited geometry pretraining; R1 voxel; R2 official SPG adapter. Grouping comparison alone does not prove relation-learning or graph contribution.',
        ...
        wall_time_cap_s=43200,estimated_cost=dict(compute='5090D6000totalupdates plus initial/final307observations per method; zero encoder inference',host_ram_gb=32,gpu_vram_gb=28,disk_gb=30,wall_time_hours=12),
        acceptance_criteria=['Exact307 and951 observed branches; reference completeness291,partial16; fit-only250updates population; same initial weights and schedule.',
            ...
            'All three final checkpoints,optimizer states,source/command/environment,logs,metrics,seals; no additional budget or protected worlds.'],
        expected_evidence=['Population and source manifests,initial state,shared schedule,initial/final raw predictions and parent-scored diagnostics,6000update logs,three final states,previews,summary,RUN_STATE,seal.'],
...
def evaluate(model,examples,method,run,stage,guard):
    ...
    with (run/'metrics'/(label+'.jsonl')).open('x') as log:
        for index,o in enumerate(examples):
            p=forward_observation(model,o,method)
...
def execute(spec,run):
    ...
    try:
        ...
        initial=build_model(device='cpu');initial_sha=state_sha256(initial)
        ...
        for method in POLICY['methods']:
            ...
            with (run/'logs'/(method+'_updates.jsonl')).open('x') as log:
                for index,batch in enumerate(schedule):
                    result=train_update(model,optimizer,[fit[i] for i in batch],method)
