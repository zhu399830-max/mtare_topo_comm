"""Teacher-only source metadata for141 existing fit observations."""
import collections
import hashlib
import json
...
def compile_scope(root):
    ...
    joined=json.loads(read('docs/figures/gse_graph/fit_opposite_join_check_20260909.json',
                          '670ea0c18099147eda0e5b5128bce6ed7ee2f4af9236c4a76153f0736fc84a8d'))
    base=json.loads(read('configs/v3/gate3/fit_opposite_input_scope_v1.json',
                        'a30b281a3f0be068071f37ca717303ad870a9c000b4a583ca7c21adee1f742b9'))
    ...
    for task,rows in sorted(grouped.items()):
        ...
        if meta['partition']!='fit' or meta['parent_id'].rsplit('_',1)[-1] not in {'C01','C02','C03','C04','C05','C06'}:raise ValueError('C01-C06 fit only')
        for row in rows:
            ...
            if s['split']!='fit':raise ValueError('not fit')
        ...
        for folder,key in (('constructions','construction_path'),('codebooks','codebook_path')):
            p=P1A+'/artifacts/'+folder+'/'+meta['partition']+'/'+task+'.json';files[p]=pins[p];docs[key]=p
        entries.append(dict(task=task,split='fit',observations=obs,input_references=refs,sensor_prefix=meta['sensor'],**docs))
    ...
    if [counts[k] for k in ('observations','tasks','parents','unique_variant_frames')]!=[141,13,5,193]:raise ValueError('population drift')
    return dict(schema='fit_opposite141_teacher_scope_v1',status='METADATA_BOUND_NOT_EXECUTION',entries=entries,
        ...
        restrictions=['fit_only','teacher_never_student_input','no_test','unknown_not_negative','no_training',
