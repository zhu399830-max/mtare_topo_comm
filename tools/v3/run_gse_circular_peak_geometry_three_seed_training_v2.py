#!/usr/bin/env python3
"""Execute and seal the single circular peak geometry V2 training run."""
...
PASS = "PASS_GSE_CIRCULAR_PEAK_GEOMETRY_THREE_SEED_TRAINING_V2"
FAIL = "FAIL_GSE_CIRCULAR_PEAK_GEOMETRY_THREE_SEED_TRAINING_V2"
CARD_STATUS = "APPROVED_FOR_ONE_IMMUTABLE_GSE_CIRCULAR_PEAK_GEOMETRY_THREE_SEED_TRAINING_V2"
...
def main() -> int:
    ...
    if run_dir.name != run_id or load_json(run_dir / "RUN_STATE.json").get("state") != "CREATED_NOT_EXECUTED":
        raise RuntimeError("circular peak V2 training executes exactly once")
    started = time.monotonic()
    overall, error, selection = FAIL, None, {}
    before: dict[str, str] = {}
    after: dict[str, str] = {}
    ...
    try:
        ...
        if not validation.passed or card.get("status") != CARD_STATUS:
            raise RuntimeError(f"circular peak V2 Data Card invalid: {validation.errors}")
        if card.get("approval", {}).get("authorized_operations") != ["training"] or card.get("approval", {}).get("authorized_gates") != [3]:
            raise RuntimeError("circular peak V2 approval scope drift")
        ...
        if environment != expected_environment:
            raise RuntimeError(f"circular peak V2 environment drift: {environment}")
        ...
        for seed in range(3):
            command = [str(PYTHON), str(PROJECT_ROOT / "tools/v3/train_gse_circular_peak_geometry_v2.py"), "--teacher-root", str(TEACHER / "artifacts/export/teacher"), "--source-root", str(DATASET / "artifacts/dataset/train"), "--output-dir", str(models / f"seed{seed}"), "--seed", str(seed), "--epochs", "10", "--batch-size", "128", "--evaluation-batch-size", "256", "--learning-rate", "0.0003", "--weight-decay", "0.0001"]
            code, rss = execute(command, run_dir / f"logs/{seed:02d}_seed{seed}_training.log", process_environment, 28_800)
            subprocesses.append({"stage": "training", "seed": seed, "returncode": code, "peak_host_rss_kib": rss})
            if code != 0:
                raise RuntimeError(f"circular peak V2 seed{seed} training failed")
            summary = load_json(models / f"seed{seed}/summary.json")
            if summary.get("schema_version") != "gse_circular_peak_geometry_training_seed_v2" or summary.get("optimizer_steps") != 11370 or summary.get("parameters") != 769268 or summary.get("c08_checkpoint_observations") != 0 or summary.get("development_output_observations") != 45942:
                raise RuntimeError(f"circular peak V2 seed{seed} optimization/split drift")
        evaluation = run_dir / "metrics/selection"
        command = [str(PYTHON), str(PROJECT_ROOT / "tools/v3/evaluate_gse_circular_peak_geometry_selection_v1.py"), "--teacher-root", str(TEACHER / "artifacts/export/teacher"), "--source-root", str(DATASET / "artifacts/dataset/train"), "--output-dir", str(evaluation)]
        for seed in range(3):
            command.extend(("--prediction-root", str(models / f"seed{seed}/development_predictions")))
        code, rss = execute(command, run_dir / "logs/03_selection.log", process_environment, 3_600)
        subprocesses.append({"stage": "selection", "returncode": code, "peak_host_rss_kib": rss})
        if code not in (0, 2):
            raise RuntimeError("circular peak V2 evaluator program failure")
        selection = load_json(evaluation / "summary.json")
        scientific_pass = selection.get("status") == "PASS_GSE_CIRCULAR_PEAK_GEOMETRY_SELECTION_V1"
        if (code == 0) != scientific_pass:
            raise RuntimeError("circular peak V2 selection status/return mismatch")
        required = [
            *[models / f"seed{seed}/{name}" for seed in range(3) for name in ("best.pt", "history.json", "summary.json")],
            *[models / f"seed{seed}/development_predictions/{parent}.npz" for seed in range(3) for parent in [path.stem for path in sorted((TEACHER / "artifacts/export/teacher/selection").glob("*.zarr"))]],
            evaluation / "summary.json", evaluation / "per_world_metrics.csv", evaluation / "figure_source.json",
            *[evaluation / f"gse_circular_peak_geometry_selection_v1.{suffix}" for suffix in ("png", "pdf", "svg")],
        ]
        if any(not path.is_file() or path.stat().st_size == 0 for path in required):
            raise RuntimeError("circular peak V2 evidence incomplete")
        after = {relative: sha256(PROJECT_ROOT / relative) for relative in before}
        if before != after:
            raise RuntimeError("circular peak V2 changed a frozen source")
    ...
    total_steps = sum(11370 for item in subprocesses if item["stage"] == "training" and item["returncode"] == 0)
    inference_observations = sum(261422 for item in subprocesses if item["stage"] == "training" and item["returncode"] == 0)
    write_json(run_dir / "metrics/summary.json", {"schema_version": "gse_circular_peak_geometry_three_seed_training_outer_v2", "overall_status": overall, "scientific_pass": overall == PASS, "error": error, "subprocesses": subprocesses, "selection": selection, "duration_seconds": time.monotonic() - started, "source_unchanged": bool(before and before == after), "optimizer_steps": total_steps, "model_inference_observations": inference_observations, "c08_checkpoint_observations": 0, "c09_worlds_read": 0, "c10_worlds_read": 0, "mtare_worlds_read": 0, "graph_replays": 0, "planner_calls": 0})
    ...
    print(json.dumps({"overall_status": overall, "error": error, "optimizer_steps": total_steps, "decision": selection.get("decision"), "seal_entries": entries}, indent=2))
