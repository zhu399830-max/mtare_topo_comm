...
from mtare_topo.governance_anchor_diagnostic import SCHEMA,SLUG,POLICY,validate_card
...
def freeze():
    ...
    command=["env","CUDA_VISIBLE_DEVICES=","OMP_NUM_THREADS=1","OPENBLAS_NUM_THREADS=1","MKL_NUM_THREADS=1","PYTHONHASHSEED=20260906",PYTHON,
        "tools/v3/surface_anchor_diagnostic_v1.py","--spec",str(root/SPEC),"--run-dir",str(root/run)]
...
def execute(spec,run):
    ...
    from mtare_topo.teacher.gse_anchor_diagnostic_v1 import diagnose_observation
