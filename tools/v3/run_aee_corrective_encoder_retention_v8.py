...
RUN_ID = "gate2_20260822_aee_corrective_encoder_retention_v8_seed20260822"
TRAINER = PROJECT_ROOT / "tools/v3/train_aee_corrective_encoder_retention_v8.py"
STATUS_PASS = "PASS_AEE_CORRECTIVE_ENCODER_RETENTION_V8"
STATUS_FAIL = "FAIL_AEE_CORRECTIVE_ENCODER_RETENTION_V8"
...
def validate_seed_summary(summary: dict[str, Any], seed: int) -> None:
    ...
    if not isinstance(summary.get("gate"), dict) or set(summary["gate"]) != expected:
        raise RuntimeError(f"V8 gate schema drift: {seed}")
...
def main() -> int:
    ...
    try:
        ...
        if card.get("status") != "APPROVED_FOR_AEE_CORRECTIVE_ENCODER_RETENTION_V8":
