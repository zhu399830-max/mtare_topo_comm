...
from mtare_topo.governance_bidirectional_relation_train_v1 import SCHEMA,SLUG,POLICY,validate_card
...
def freeze():
    ...
    approval=dict(status='APPROVED',approved_by='user-standing-scope-authorization',approved_at='2026-09-09',
        ...
        scope='Exactly860 existing observations282fit578calibration; C0/C1 sameR2 each2000 scheduled batches micro1accum4. Preserve96 fit observations without positive anchors; conditional unknown policy unchanged; log actual optimizer steps. No independent development population.',
        confirmation_reference='User standing autonomous development authorization; current PLAN authorizes paired training after completed860 authenticated reader probe. Same-R2 relation ablation, original corrective losses and geometry-only scoring unchanged; exact860 source scope, no new labels or protected worlds; calibration is not independent evaluation.')
    ...
    command=['env','OMP_NUM_THREADS=1','OPENBLAS_NUM_THREADS=1','MKL_NUM_THREADS=1','PYTHONHASHSEED=0','CUBLAS_WORKSPACE_CONFIG=:4096:8',PYTHON,
             'tools/v3/bidirectional_relation_train_v1.py','--spec',str(ROOT/SPEC),'--run-dir',str(ROOT/RUN)]
    spec=dict(schema_version='v3_run_spec_v1',gate=3,date='20260909',slug=SLUG,seed=0,operation='training',
        ...
        wall_time_cap_s=43200,estimated_cost=dict(compute='5090D4000scheduledbatches plus initial/final860observations per method; zero encoder inference',host_ram_gb=32,gpu_vram_gb=28,disk_gb=30,wall_time_hours=12),
        ...
        expected_evidence=['Population and source manifests,initial state,shared schedule,initial/final raw predictions and parent-scored diagnostics,4000scheduled-batch logs with actual-step counts,two final states,previews,summary,RUN_STATE,seal.'],
