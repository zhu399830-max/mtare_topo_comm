#!/usr/bin/env python3
"""Run and seal the approved Gate-4 C08 causal topology replay."""
...
RUN_ID = "gate4_20260817_cano_c08_route_conditioned_causal_topology_replay_v2r_seed0"
E1 = Path("/tmp/mtare_gate4_meshing_sidecar_v1/bin/python")
TORCH = Path("/tmp/mtare_gate4_torch290_cu129_zarr2187/bin/python")
STAGES = (
    ("01_sensor", E1, PROJECT_ROOT / "tools/v3/execute_cano_c08_causal_sensor_stage_v1.py", 2400),
    ("02_frozen_inference", TORCH, PROJECT_ROOT / "tools/v3/execute_cano_c08_frozen_inference_stage_v1.py", 600),
    ("03_causal_graph", TORCH, PROJECT_ROOT / "tools/v3/execute_cano_c08_causal_graph_stage_v1.py", 1200),
)
...
def main() -> int:
    ...
    for name, executable in (("sensor", E1), ("inference_graph", TORCH)):
        ...
        check = subprocess.run(
            [str(executable), "-m", "pip", "check"], text=True,
    ...
    stage_records = []; all_passed = True; started = time.monotonic()
    for name, executable, script, timeout_s in STAGES:
        argv = [str(executable), str(script), "--run-dir", str(run_dir)]
        stage_started = time.monotonic()
        completed = subprocess.run(argv, cwd=PROJECT_ROOT, env=environment, text=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, timeout=timeout_s, check=False)
    ...
    passed = bool(
        all_passed and sensor.get("overall_status") == "PASS_C08_CAUSAL_SENSOR_STAGE_V1"
        and sensor.get("frames") == 4773 and sensor.get("dual_scene_full_scan_rays") == 109969920
        ...
        and inference.get("overall_status") == "PASS_C08_FROZEN_INFERENCE_STAGE_V1"
        and inference.get("model_inference_frames") == 14319
        and graph.get("overall_status") == "PASS_C08_CAUSAL_GRAPH_SWEEP_AND_FREEZE_V1"
        and graph.get("grid_configurations") == 243 and graph.get("grid_graph_replays") == 3645
        and graph.get("oracle_integrity_passed_for_all_grid_rows") is True
        and graph.get("c09_worlds_read") == graph.get("c10_worlds_read") == graph.get("mtare_worlds_read") == 0
    )
    overall = "PASS_CANO_C08_ROUTE_CONDITIONED_CAUSAL_TOPOLOGY_REPLAY_V2R" if passed else "FAIL_CANO_C08_ROUTE_CONDITIONED_CAUSAL_TOPOLOGY_REPLAY_V2R"
    write_json(run_dir / "metrics/summary.json", {
        "schema_version": "cano_c08_route_conditioned_causal_topology_replay_runner_v2r", "overall_status": overall,
        ...
        "auxiliary_primary_rays": sensor.get("auxiliary_primary_clearance_rays", 0) + sensor.get("auxiliary_primary_branch_los_rays", 0),
        "model_inference_frames": inference.get("model_inference_frames", 0),
        "grid_graph_replays": graph.get("grid_graph_replays", 0),
        "selected_parameter_id": graph.get("selected_parameter_id"),
        ...
        "claim_boundary": "C08 development parameter freeze only; no C09 validation, C10 test, planner or M-TARE closed loop.",
    })
