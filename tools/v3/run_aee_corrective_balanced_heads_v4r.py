...
RUN_ID = "gate2_20260822_aee_corrective_balanced_heads_v4r_seed20260822"
TRAINER = PROJECT_ROOT / "tools/v3/train_aee_corrective_balanced_heads_v4.py"
STATUS_PASS = "PASS_AEE_CORRECTIVE_BALANCED_HEADS_V4R"
STATUS_FAIL = "FAIL_AEE_CORRECTIVE_BALANCED_HEADS_V4R"
...
def validate_seed_summary(summary: dict[str, Any], seed: int) -> None:
    if summary.get("status") != "COMPLETED_AEE_CORRECTIVE_BALANCED_HEADS_SEED_V4":
        raise RuntimeError(f"V4R seed {seed} did not complete")
    if summary.get("seed") != seed or summary.get("epochs_completed") != 10:
        raise RuntimeError(f"V4R seed/epoch identity drift: {seed}")
    if summary.get("independent_train_samples_per_epoch") != {"cano": 5000, "aee": 1000}:
        raise RuntimeError(f"V4R independent sample drift: {seed}")
    if summary.get("training_views_per_epoch") != {"cano_dense": 5000, "cano_mask_matched": 5000, "aee": 1000}:
        raise RuntimeError(f"V4R view count drift: {seed}")
    if summary.get("direction_loss") != "per_frame_equal_positive_negative_soft_mass_v1":
        raise RuntimeError(f"V4R direction-loss contract drift: {seed}")
    if summary.get("representation_contract") != "encoder_and_embedding_frozen_exact":
        raise RuntimeError(f"V4R representation contract drift: {seed}")
    if any(summary.get(key) != 0 for key in ("c09_frames_read", "c10_frames_read", "formal_benchmark_frames_read")):
        raise RuntimeError(f"V4R forbidden data read: {seed}")
    expected = {
    ...
    if not isinstance(summary.get("gate"), dict) or set(summary["gate"]) != expected:
        raise RuntimeError(f"V4R gate schema drift: {seed}")
...
def main() -> int:
    ...
    try:
        ...
        if card.get("status") != "APPROVED_FOR_AEE_CORRECTIVE_BALANCED_HEADS_V4R":
