#!/usr/bin/env python3
"""Generate 100 frozen objective labels for one AEE corrective trajectory."""
...
STATUS_PASS = "PASS_AEE_CORRECTIVE_OBJECTIVE_TEACHER_SHARD_V1"
...
def main() -> int:
    ...
    audit = audit_corrective_sensor_shard(sensor, AEE_FRAMES_PER_TRAJECTORY)
    if not audit["passed"]:
        raise RuntimeError(f"corrective sensor shard audit failed: {audit}")
    if not np.array_equal(sensor.get("raw_frame_index"), aee_corrective_frame_indices()):
        raise RuntimeError("corrective raw frame indices drift")
    ...
    for index, (xyz, yaw) in enumerate(zip(sensor["sensor_xyz_m"], sensor["yaw_deg"])):
        ...
        maximum_reachable_layers = max(maximum_reachable_layers, evidence.maximum_reachable_layers_per_cell)
    payload = {
    ...
    summary = {
        "schema_version": "aee_corrective_objective_teacher_shard_summary_v1",
        "status": STATUS_PASS,
