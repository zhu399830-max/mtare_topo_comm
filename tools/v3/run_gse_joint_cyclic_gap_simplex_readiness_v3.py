#!/usr/bin/env python3
"""Requalify JCGS after correcting singleton circular closure."""
...
PASS = "PASS_GSE_JOINT_CYCLIC_GAP_SIMPLEX_READINESS_V3"
FAIL = "FAIL_GSE_JOINT_CYCLIC_GAP_SIMPLEX_READINESS_V3"
INNER_PASS = "PASS_GSE_JOINT_CYCLIC_GAP_SIMPLEX_READINESS_V2"
CARD_STATUS = "APPROVED_FOR_ONE_IMMUTABLE_GSE_JOINT_CYCLIC_GAP_SIMPLEX_READINESS_V3"
...
def main() -> int:
    ...
    if run.name != run_id or load_json(run / "RUN_STATE.json").get("state") != "CREATED_NOT_EXECUTED":
        raise RuntimeError("gap-simplex V3 readiness executes exactly once")
    ...
    try:
        ...
        with (run / "logs/00_singleton_closure_regression.log").open("w", encoding="utf-8") as stream:
        ...
        if regression_returncode != 0:
            raise RuntimeError("singleton closure regression failed")
        ...
        if executor_returncode not in (0, 2):
            raise RuntimeError(f"gap-simplex V3 executor system failure: {executor_returncode}")
        ...
        if (executor_returncode == 0) != passed:
            raise RuntimeError("gap-simplex V3 status/return mismatch")
        ...
        if any(result.get(name) != 0 for name in forbidden):
            raise RuntimeError("gap-simplex V3 forbidden operation")
        ...
        if any(not path.is_file() or path.stat().st_size == 0 for path in required):
            raise RuntimeError("gap-simplex V3 evidence incomplete")
        after = {relative: sha256(PROJECT_ROOT / relative) for relative in before}
        if before != after:
            raise RuntimeError("gap-simplex V3 source changed")
    ...
    write_json(run / "metrics/summary.json", {
        "schema_version": "gse_joint_cyclic_gap_simplex_readiness_outer_v3",
        "overall_status": overall,
        "scientific_pass": overall == PASS,
        "error": error,
        "singleton_gap_closure_deg": 360.0 if regression_returncode == 0 else None,
    ...
    entries = seal(run)
    print(json.dumps({"overall_status": overall, "error": error, "singleton_gap_closure_deg": 360.0 if regression_returncode == 0 else None, "seal_entries": entries}, indent=2))
