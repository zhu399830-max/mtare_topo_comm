...
from mtare_topo.governance_opening_collision_probe_v1 import SCHEMA,SLUG,POLICY,validate_card
...
def freeze():
    ...
    command=["env","CUDA_VISIBLE_DEVICES=","OMP_NUM_THREADS=1","OPENBLAS_NUM_THREADS=1","MKL_NUM_THREADS=1","PYTHONHASHSEED=20260906",PYTHON,
        "tools/v3/opening_collision_probe_v1.py","--spec",str(root/SPEC),"--run-dir",str(root/run)]
    ...
    spec=dict(schema_version="v3_run_spec_v1",gate=3,date="20260907",slug=SLUG,seed=20260906,operation="data_export",
        ...
        question="Are duplicate opening coordinates caused by multiple contours in one source section or by different source references?",
        method="Original opening proposal diagnostic on failed observation39551; retain all supported duplicate groups, exact source arc identities and complete triangle-plane section loops; no deduplication or labels.",
...
def execute(spec,run):
    ...
    from mtare_topo.teacher.gse_opening_collision_probe_v1 import inspect_collision
        ...
        with (run/"logs/observations.jsonl").open("x") as log:
            for row in card["scope"]["entries"]:
                ...
                for bundle in bundles:
                    ...
                    rows.append(inspect_collision(bundle))
