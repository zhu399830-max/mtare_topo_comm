...
from mtare_topo.governance_continuous_model_input_v1 import SCHEMA,SLUG,validate_card
from mtare_topo.data.continuous_model_input_scope_v1 import compile_scope
from mtare_topo.data.continuous_input_scope_v1 import SELECTION
from mtare_topo.data.continuous_model_input_export_v1 import export_population
...
def freeze():
    ...
    authority=dict(status='APPROVED',approved_by='user-standing-development-authorization',approved_at='2026-09-09',
        ...
        scope='One fixed C04 traversal three variants30windows42frames; exact six model input fields; no training, test data or teacher labels.',
        confirmation_reference='User continuous autonomous authorization; PLAN fixed continuous input selection and one six-field input export. No absolute pose input to any model.')
    ...
    command=['env','OMP_NUM_THREADS=1','OPENBLAS_NUM_THREADS=1',PYTHON,'tools/v3/continuous_model_input_export_v1.py',
    ...
    spec=dict(schema_version='v3_run_spec_v1',gate=3,date='20260909',slug=SLUG,seed=20260906,operation='data_export',
        ...
        question='Export exact original scan and relative-motion inputs for30 continuous windows without teacher labels.',
        ...
        wall_time_cap_s=600,estimated_cost=dict(compute='CPU only;5.62MB decoded source chunks;0model/optimizer',disk_gb=.1,wall_time_hours=1/6,host_ram_gb=4,gpu_vram_gb=0),
        acceptance_criteria=['Exact30windows42frames with original source seals and no out-of-scope reads.',
            ...
            'Three six-field NPZ packages bind all30 encoder inputs; overlapping scan bytes agree; no invented safety/graph success.'],
