#!/usr/bin/env python3
"""Single frozen archived-grid exclusion diagnostic; no label promotion."""
import argparse
import hashlib
import json
...
from mtare_topo.governance_reference_exclusion_v1 import SCHEMA,SLUG,POLICY,validate_card
from mtare_topo.governance_surface_selection import digest
from mtare_topo.governance_surface_material import read_pinned
from mtare_topo.data.gse_reference_exclusion_scope_v1 import compile_scope
...
def freeze():
    ...
    approval=dict(status="APPROVED",approved_by="user-standing-scope-authorization",approved_at="2026-09-07",
        ...
        scope="Fixed30C01 observations150frames1920 fixed queries; archived grids and original source binding; conditional reference exclusions only; no semantic labels or training",
    ...
    command=["env","CUDA_VISIBLE_DEVICES=","OMP_NUM_THREADS=1","OPENBLAS_NUM_THREADS=1","MKL_NUM_THREADS=1","PYTHONHASHSEED=20260906",PYTHON,
        "tools/v3/reference_exclusion_v1.py","--spec",str(root/SPEC),"--run-dir",str(root/run)]
    ...
    spec=dict(schema_version="v3_run_spec_v1",gate=3,date="20260907",slug=SLUG,seed=20260906,operation="data_export",
        ...
        question="Do fixed observed locations admit conditional exclusion from the complete frozen construction-anchor reference universe?",
        method="64 fixed Cartesian query locations per observation; validate sealed observed-grid and original causal projection hashes; exclude only observed locations farther than4m from ALL degree1/degree>=3 construction references, including hidden/outside references. No global semantic completeness.",
        ...
        expected_evidence=["Source snapshot,command/environment,per-observation fixed query states,conditional exclusions,source frames,unknown reasons,raw log,summary,RUN_STATE,SHA256 seal;failure preserves active task."],
    ...
    print(json.dumps({"spec":SPEC,"counts":dict(observations=30,frames=150,queries=1920),"source_files":len(files),"executed":False}))


def execute(spec,run):
    from mtare_topo.data.gse_surface_teacher_reader_v1 import SurfaceTeacherReader
    from mtare_topo.teacher.gse_reference_exclusion_real_v1 import evaluate_archived
    ...
    try:
        ...
        if digest(compile_scope(root))!=card["scope_sha256"]:raise ValueError("scope drift")
        reader=SurfaceTeacherReader(root,card["scope"]["teacher_source_scope"])
        with (run/"logs/observations.jsonl").open("x") as log:
            for row in card["scope"]["observations"]:
                ...
                output=evaluate_archived(root,row,bundle,card["scope"]["query_policy"],reader.opened)
                write(run/"artifacts"/(hashlib.sha256(active.encode()).hexdigest()[:16]+".json"),output)
                entry=dict(task=active,queries=64,conditional_negatives=sum(output["reference_negative_mask"]),unobserved=output["reason"].count("UNOBSERVED_LOCATION"),near_reference=output["reason"].count("POSSIBLE_REFERENCE_MATCH_KEEP_UNKNOWN"),references=output["reference_count"])
        ...
        summary=dict(status="FAILED" if error else "REFERENCE_EXCLUSION_DIAGNOSTIC_COMPLETE",error=error,active_task=active,
