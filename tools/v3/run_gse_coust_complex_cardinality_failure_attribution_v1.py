#!/usr/bin/env python3
"""Execute and seal the frozen COUST complex-cardinality attribution."""
...
FAIL = "FAIL_GSE_COUST_COMPLEX_CARDINALITY_FAILURE_ATTRIBUTION_V1"
CARD_STATUS = "APPROVED_FOR_ONE_IMMUTABLE_GSE_COUST_COMPLEX_CARDINALITY_FAILURE_ATTRIBUTION_V1"
PYTHON = Path("/home/zeng-workstation/.local/share/mtare_topo_comm/envs/phase3_torch290_cu129_zarr2187_v1/bin/python")
TRAINING = PROJECT_ROOT / "results/gate3_semantics/gate3_20260829_gse_cyclic_ordered_unimodal_slot_transport_three_seed_training_v1_seed0"
...
def main() -> int:
    ...
    if run.name != run_id or load_json(run / "RUN_STATE.json").get("state") != "CREATED_NOT_EXECUTED":
        raise RuntimeError("COUST complex-cardinality attribution executes exactly once")
    ...
    try:
        ...
        command = [
            str(PYTHON), str(PROJECT_ROOT / "tools/v3/execute_gse_coust_complex_cardinality_failure_attribution_v1.py"),
        ...
        if returncode != 0:
            raise RuntimeError("COUST attribution subprocess failed")
        attribution = load_json(output / "summary.json")
        if attribution.get("status") != PASS or attribution.get("scientific_pass") is not True:
            raise RuntimeError("COUST attribution result contract failed")
        required = [output / "summary.json", output / "figure_source.json", output / "complex_cardinality_table.csv", *[output / f"gse_coust_complex_cardinality_failure_attribution_v1.{suffix}" for suffix in ("png", "pdf", "svg")]]
        if any(not path.is_file() or path.stat().st_size == 0 for path in required):
            raise RuntimeError("COUST attribution evidence incomplete")
        if attribution.get("c07", {}).get("observations") != 21548 or attribution.get("c08", {}).get("observations") != 24394:
            raise RuntimeError("COUST attribution observation population drift")
        if attribution.get("c07", {}).get("visible_exits") != 45504 or attribution.get("c08", {}).get("visible_exits") != 51537:
            raise RuntimeError("COUST attribution exit population drift")
        after = {relative: sha256(PROJECT_ROOT / relative) for relative in before}
        if before != after:
            raise RuntimeError("frozen COUST attribution source changed")
        overall = PASS
    ...
    write_json(run / "metrics/summary.json", {
        "schema_version": "gse_coust_complex_cardinality_failure_attribution_outer_v1",
