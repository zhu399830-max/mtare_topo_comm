#!/usr/bin/env python3
"""Train one local composition-slot head with a frozen observable backbone."""
...
EXPECTED_TOTAL_PARAMETERS = 3_637_138
...
def load_frozen_model(
    ...
    *,
    seed: int,
) -> tuple[FrozenObservableLocalCompositionSlotNet, dict]:
    ...
    if (
        ...
        raise RuntimeError("local composition-slot source checkpoint identity drift")
    backbone = ObservableSparsePortRelationNet()
    backbone.load_state_dict(checkpoint["model_state_dict"], strict=True)
    torch.manual_seed(2_026_090_310 + seed)
    return FrozenObservableLocalCompositionSlotNet(backbone), checkpoint
...
def evaluate_losses(
    ...
    *,
    ...
    model.eval(); sums = defaultdict(float); rows = batches = 0
    cluster_count = clustered_endpoints = dustbin_endpoints = 0
    for numpy_batch in loader.iter_epoch(
        batch_size=EVALUATION_BATCH_SIZE, seed=0, epoch=0, shuffle=False,
    ):
        losses, _, targets = batch_losses(model, numpy_batch, device=device)
        row_count = len(numpy_batch.base.range_valid)
        for name in LOSS_NAMES:
            value = float(losses[name].detach())
            if not np.isfinite(value):
                raise RuntimeError(f"non-finite local composition-slot C07 loss: {name}")
            sums[name] += value * row_count
        cluster_count += int(targets.cluster_count.sum())
        clustered_endpoints += int((targets.labels >= 0).sum())
        dustbin_endpoints += int((targets.labels == -1).sum())
        rows += row_count; batches += 1
    if rows != EXPECTED_C07_ROWS or batches != EXPECTED_C07_BATCHES:
        raise RuntimeError(f"local composition-slot C07 population drift: {rows}/{batches}")
    return {
        ...
        **{name: sums[name] / rows for name in LOSS_NAMES},
        ...
        "dustbin_endpoints": dustbin_endpoints,
    }
...
def _save_checkpoint(
    ...
    *,
    ...
    payload = {
        "schema_version": "primitive_local_composition_slot_checkpoint_v1",
        ...
        "training_contract": {
            ...
            "selection_metric": "mean_C07_local_composition_slot_total_loss",
            "teacher_attachment_validity": "dual_endpoint_observed",
            "teacher_relation_representation": "lossless_exchangeable_composition_cliques",
            "composition_slot_count": 32,
            "dustbin": True,
            "frozen_observable_backbone": True,
            "joint_slot_loss_from_epoch_zero": True,
        },
    }
...
def main() -> int:
    ...
    if (
        ...
        raise ValueError("formal local composition-slot hyperparameter drift")
    output = args.output_dir.resolve()
    if output.exists():
        raise RuntimeError("local composition-slot seed output exists")
    output.mkdir(parents=True); started = time.monotonic(); _configure_determinism(args.seed)
    if not torch.cuda.is_available():
        raise RuntimeError("formal local composition-slot training requires CUDA")
    device = torch.device("cuda")
    fit = ObservablePrimitiveRelationBatchLoader(
        args.fit_sensor_root, args.fit_teacher_root, args.fit_observability_root,
    )
    c07 = ObservablePrimitiveRelationBatchLoader(
        args.c07_sensor_root, args.c07_teacher_root, args.c07_observability_root,
    )
    if len(fit) != EXPECTED_FIT_ROWS or len(fit.task_names) != EXPECTED_FIT_TASKS:
        raise RuntimeError("local composition-slot fit population drift")
    if len(c07) != EXPECTED_C07_ROWS or len(c07.task_names) != EXPECTED_C07_TASKS:
        raise RuntimeError("local composition-slot C07 population drift")
    if any("_C07__" in name or "_C08__" in name for name in fit.task_names):
        raise RuntimeError("selection task leaked into local composition-slot fit")
    if any("_C07__" not in name for name in c07.task_names):
        raise RuntimeError("local composition-slot C07 loader contains non-C07 task")
    ...
    model, source_payload = load_frozen_model(args.source_checkpoint, seed=args.seed)
    ...
    if (
        ...
        raise RuntimeError("local composition-slot parameter/freeze boundary drift")
    frozen_digest_before = _state_digest(model.backbone)
    optimizer = torch.optim.AdamW(trainable, lr=LEARNING_RATE, weight_decay=WEIGHT_DECAY)
    history = []; best_epoch = -1; best_selection_loss = float("inf")
    optimizer_steps = 0; peak_process_memory = 0
    torch.cuda.reset_peak_memory_stats(device)

    for epoch in range(TRAINING_EPOCHS):
        epoch_started = time.monotonic(); model.train(); sums = defaultdict(float)
        rows = batches = clusters = clustered_endpoints = dustbin_endpoints = 0
        for numpy_batch in fit.iter_epoch(
            batch_size=TRAINING_BATCH_SIZE, seed=args.seed, epoch=epoch, shuffle=True,
        ):
            optimizer.zero_grad(set_to_none=True)
            losses, _, targets = batch_losses(model, numpy_batch, device=device)
            objective = losses["total"]
            if not bool(torch.isfinite(objective)):
                raise RuntimeError("non-finite local composition-slot training objective")
            objective.backward()
            if any(
                value.grad is None or not bool(torch.isfinite(value.grad).all())
                or not bool((value.grad != 0).any()) for value in trainable
            ):
                raise RuntimeError("missing/non-finite/zero local composition-slot head gradient")
            if any(value.grad is not None for value in frozen):
                raise RuntimeError("frozen local composition-slot backbone received gradient")
            ...
            clustered_endpoints += int((targets.labels >= 0).sum())
            dustbin_endpoints += int((targets.labels == -1).sum())
            rows += row_count; batches += 1; optimizer_steps += 1
            if batches % 200 == 0:
                ...
                if process_memory > MAXIMUM_GPU_PROCESS_BYTES:
                    raise RuntimeError("local composition-slot GPU process memory exceeds 16 GiB")
        if rows != EXPECTED_FIT_ROWS or batches != EXPECTED_FIT_BATCHES:
            raise RuntimeError(f"local composition-slot fit epoch drift: {rows}/{batches}")
        training_metrics = {
            "rows": rows, "batches": batches, "clusters": clusters,
            "clustered_endpoints": clustered_endpoints,
            "dustbin_endpoints": dustbin_endpoints,
            **{name: sums[name] / rows for name in (*LOSS_NAMES, "gradient_norm")},
        }
        selection = evaluate_losses(model, c07, device=device)
        process_memory = _process_memory_bytes(); peak_process_memory = max(peak_process_memory, process_memory)
        if process_memory > MAXIMUM_GPU_PROCESS_BYTES:
            raise RuntimeError("local composition-slot C07 GPU memory exceeds 16 GiB")
        if _state_digest(model.backbone) != frozen_digest_before:
            raise RuntimeError("local composition-slot frozen backbone changed")
        record = {
            "epoch": epoch, "training": training_metrics, "selection": selection,
        ...
        _save_checkpoint(
            output / f"epoch_{epoch:02d}.pt", model=model, optimizer=optimizer,
            seed=args.seed, epoch=epoch, source_checkpoint=args.source_checkpoint,
            training_metrics=training_metrics, selection_metrics=selection,
        )
        if float(selection["selection_loss"]) < best_selection_loss:
            best_selection_loss = float(selection["selection_loss"]); best_epoch = epoch
        _write_json(output / "history.json", history)
        print(json.dumps(record, sort_keys=True), flush=True); torch.cuda.empty_cache()

    expected_steps = EXPECTED_FIT_BATCHES * TRAINING_EPOCHS
    if optimizer_steps != expected_steps or best_epoch < 0:
        raise RuntimeError("local composition-slot optimizer-step/checkpoint selection drift")
    ...
    if frozen_digest_after != frozen_digest_before:
        raise RuntimeError("local composition-slot frozen state changed after training")
    summary = {
        "schema_version": "primitive_local_composition_slot_seed_training_v1",
        "seed": args.seed, "parameter_count": EXPECTED_TOTAL_PARAMETERS,
        ...
        "epochs": TRAINING_EPOCHS, "optimizer_steps": optimizer_steps,
        "best_epoch": best_epoch, "best_c07_selection_loss": best_selection_loss,
    ...
    _write_json(output / "summary.json", summary); print(json.dumps(summary, indent=2), flush=True)
    return 0
