...
RUN_ID = "gate2_20260822_aee_corrective_staged_balanced_heads_v5_seed20260822"
TRAINER = PROJECT_ROOT / "tools/v3/train_aee_corrective_staged_balanced_heads_v5.py"
STATUS_PASS = "PASS_AEE_CORRECTIVE_STAGED_BALANCED_HEADS_V5"
STATUS_FAIL = "FAIL_AEE_CORRECTIVE_STAGED_BALANCED_HEADS_V5"
...
def validate_seed_summary(summary: dict[str, Any], seed: int) -> None:
    if summary.get("status") != "COMPLETED_AEE_CORRECTIVE_STAGED_BALANCED_HEADS_SEED_V5":
        raise RuntimeError(f"V5 seed {seed} did not complete")
    if summary.get("seed") != seed or summary.get("epochs_completed") != 10:
        raise RuntimeError(f"V5 seed/epoch identity drift: {seed}")
    if summary.get("independent_train_samples_per_epoch") != {"cano": 5000, "aee": 1000}:
        raise RuntimeError(f"V5 independent sample drift: {seed}")
    if summary.get("training_views_per_epoch") != {"cano_dense": 5000, "cano_mask_matched": 5000, "aee": 1000}:
        raise RuntimeError(f"V5 view count drift: {seed}")
    if summary.get("count_effective_mass") != [1244, 6793, 1329, 504, 105, 25] or summary.get("role_effective_mass") != [6495, 2105, 1400]:
        raise RuntimeError(f"V5 effective-class-mass drift: {seed}")
    if summary.get("staged_update") != {"direction_epochs": 1, "count_role_epochs": 10, "steps_per_epoch": 47}:
        raise RuntimeError(f"V5 staged-update drift: {seed}")
    if summary.get("representation_contract") != "encoder_and_embedding_frozen_exact":
        raise RuntimeError(f"V5 representation contract drift: {seed}")
    if any(summary.get(key) != 0 for key in ("c09_frames_read", "c10_frames_read", "formal_benchmark_frames_read")):
        raise RuntimeError(f"V5 forbidden data read: {seed}")
    expected = {
    ...
    if not isinstance(summary.get("gate"), dict) or set(summary["gate"]) != expected:
        raise RuntimeError(f"V5 gate schema drift: {seed}")
...
def main() -> int:
    ...
    try:
        ...
        if card.get("status") != "APPROVED_FOR_AEE_CORRECTIVE_STAGED_BALANCED_HEADS_V5":
