...
from mtare_topo.governance_supplement_support_v1r import SCHEMA,SLUG,POLICY,validate_card
...
def freeze():
    ...
    approval=dict(status="APPROVED",approved_by="user-standing-scope-authorization",approved_at="2026-09-07",
        ...
        scope="Fixed2676C01-C07observations;original fiveframe input caches,poses,source codes and constructions;terminal cap evidence and junction source presence only;no labels or training",
        confirmation_reference="User authorized same-C01-C07 supplementary sampling (那你想办法呀, 继续执行) and autonomous local teacher development. Exact missing2676 input export is sealed; this one storage-only corrective checks the same observed source evidence with lossless gzip after the JSON output-cap failure. Standing autonomous implementation-fix authority, no resampling, raised cap, label promotion or overwrite of prior run.")
    ...
    command=["env","CUDA_VISIBLE_DEVICES=","OMP_NUM_THREADS=1","OPENBLAS_NUM_THREADS=1","MKL_NUM_THREADS=1","PYTHONHASHSEED=20260906",PYTHON,
        "tools/v3/supplement_support_v1r.py","--spec",str(root/SPEC),"--run-dir",str(root/run)]
    ...
    spec=dict(schema_version="v3_run_spec_v1",gate=3,date="20260907",slug=SLUG,seed=20260906,operation="data_export",
        ...
        question="Which fixed2676 observations support a source terminal cap with a finite uniquely owned first return?",
        method="Storage-only corrective: original JSON bytes losslessly gzip6-compressed with raw and compressed SHA; no witness discarded. Original degree-one .05m/64 cap faces; original normalize64/cast32 ray inputs and Open3D exact t_hit equality; unique saved source; junction per-incidence source presence (not branch visibility); open10m ROI; no tolerance enlargement or labels.",
        ...
        wall_time_cap_s=1800,estimated_cost=dict(compute="CPU only;2676 fiveframe observations,13374 selected frames,210 source tasks; cached scans only",host_ram_gb=4,gpu_vram_gb=0,disk_gb=.5,wall_time_hours=.5),
        acceptance_criteria=["Exact fixed2676 selection,source hashes,poses and membership; no C08-C10, new scan rendering, models, labels or training.",
...
def execute(spec,run):
    ...
    from mtare_topo.teacher.gse_supplement_support_v1 import diagnose_observation
    ...
    try:
        ...
        with (run/"logs/observations.jsonl").open("x") as log:
            for row in card["scope"]["entries"]:
                ...
                for bundle in bundles:
                    rows.append(diagnose_observation(bundle,range_error_bound_m=POLICY["extra_range_error_m"]))
                storage=write_evidence(run/"artifacts"/(active+".json.gz"),dict(task=active,observations=rows))
                entry=dict(task=active,observations=len(rows),local_terminals=sum(r["local_terminal_count"] for r in rows),observed_terminals=sum(r["observed_terminal_count"] for r in rows))
        ...
        summary=dict(status="FAILED" if error else "SUPPLEMENT_SUPPORT_DIAGNOSTIC_COMPLETE",error=error,active_task=active,
