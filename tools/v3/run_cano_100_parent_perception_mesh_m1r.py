#!/usr/bin/env python3
"""Run and seal the approved full-100 sanitized perception-asset batch once."""
...
from execute_cano_100_parent_perception_mesh_m1r import EXPECTED_SCOPE
...
RUN_ID = "gate0_20260811_cano_100_parent_perception_mesh_m1r_sanitized_assets_seed0"
EXECUTOR = PROJECT_ROOT / "tools/v3/execute_cano_100_parent_perception_mesh_m1r.py"
SOURCE_M1 = PROJECT_ROOT / "results/gate0_baseline/gate0_20260811_cano_100_parent_perception_mesh_m1_immutable_assets_seed0"
...
def _run(run_dir: Path) -> tuple[int, float]:
    ...
    try:
        ...
        output += f"\nTIMEOUT: frozen {TIME_LIMIT_SECONDS} second M1R limit reached.\n"
    duration = time.monotonic() - started
    (run_dir / "logs/01_cano_100_parent_perception_mesh_m1r.log").write_text(
...
def main() -> int:
    ...
    if (
        ...
        raise RuntimeError("approval or M1R scope mismatch")
    ...
    if (
        ...
        raise RuntimeError("approved M1R proposal/card missing")
    ...
    if set(spec.get("frozen_tools", {})) != set(paths):
        raise RuntimeError("frozen M1R tool set mismatch")
    ...
    m1_state = load_json(SOURCE_M1 / "RUN_STATE.json")
    if m1_state.get("state") != "FAILED" or m1_state.get("overall_status") != "FAIL_CANO_100_PARENT_PERCEPTION_MESH_M1":
        raise RuntimeError("M1 failure route evidence invalid")
    ...
    if shutil.disk_usage(PROJECT_ROOT).free < 6 * 1024**3:
        raise RuntimeError("less than 6 GiB free before M1R")

    for name, value in (
        ("tool_hashes", observed), ("upstream_identity", upstream), ("e1_environment", environment),
        ("subt_proc_gen_import_identity", imports), ("m1_source_seal", m1_seal),
        ("m0f_source_seal", m0f_seal), ("v2r_source_seal", v2r_seal), ("v2_source_seal", v2_seal),
    ):
        write_json(run_dir / f"config/{name}.json", value)
    shutil.copy2(card_path, run_dir / "config/topology_card.json")
    write_json(run_dir / "RUN_STATE.json", {"schema_version": "v3_run_state_v1", "run_id": RUN_ID, "state": "RUNNING", "note": "Approved M1R full-100 sanitized assets; 80 M1R train previews; zero replay/data/training."})
    ...
    sanitation = sorted(run_dir.glob("artifacts/meshes/S*_C*/primary/sanitation.json"))
    ...
    passed = bool(
        code == 0 and duration <= TIME_LIMIT_SECONDS and summary
        and summary.get("overall_status") == "PASS_CANO_100_PARENT_PERCEPTION_MESH_M1R"
        and summary.get("scope") == EXPECTED_SCOPE
        and summary.get("batch_audit", {}).get("passed") is True
        and summary.get("sanitation_batch", {}).get("passed") is True
        and len(primary) == len(sanitation) == len(metrics) == 100
        and len(maps) == 80 and len(replay) == 0
        and (run_dir / "artifacts/mesh_manifest.json").is_file()
        and (run_dir / "previews/provenance.json").is_file()
        and size <= DISK_LIMIT_BYTES
    )
    overall = "PASS_CANO_100_PARENT_PERCEPTION_MESH_M1R" if passed else "FAIL_CANO_100_PARENT_PERCEPTION_MESH_M1R"
    boundary = "100 sanitized immutable perception assets only; zero replay, LiDAR, labels, formal data, training, models, trajectories and M-TARE changes."
    runner_summary = {
        "schema_version": "cano_100_parent_perception_mesh_runner_summary_m1r",
        "run_id": RUN_ID,
        "overall_status": overall,
        "executor_exit_code": code,
        "duration_seconds": duration,
        "counts": {"primary_meshes": len(primary), "sanitation_records": len(sanitation), "replay_meshes": len(replay), "parent_metrics": len(metrics), "train_complete_previews": len(maps)},
        "source_seals": {"m1": m1_seal, "m0f": m0f_seal, "v2r": v2r_seal, "v2": v2_seal},
