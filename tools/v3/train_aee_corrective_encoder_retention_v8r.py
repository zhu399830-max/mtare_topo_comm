#!/usr/bin/env python3
"""V8R: adapt the final encoder block under frozen-teacher direction retention."""
...
def make_encoder_retention_trainable(model):
    ...
    if not _DIRECTION_PARAMETERS or not _STAGE2_PARAMETERS:
        raise RuntimeError("V8R staged parameter contract is empty")
    if any(not name.startswith(base.SEMANTIC_HEAD_PREFIXES) for name in trainable):
        raise RuntimeError("V8R stage-1 trainable contract failed")
...
class EncoderRetentionAdamW(torch.optim.AdamW):
    ...
    def step(self, closure=None):
        ...
        if self.contract_step == STEPS_PER_EPOCH:
            if _MODEL is None:
                raise RuntimeError("V8R model unavailable at stage boundary")
...
def retained_balanced_loss(outputs, direction_target, count_target, role_target, loss_weight, role_weights=None):
    ...
    if teacher is not None:
        ...
        if int(cano.sum()) == 0:
            raise RuntimeError("V8R batch has no Cano retention views")
...
def main() -> int:
    ...
    checkpoint["mode"] = "M1D_AEE_CORRECTIVE_ENCODER_RETENTION_V8R"
    ...
    summary.update(
        schema_version="aee_corrective_encoder_retention_seed_summary_v8r",
        status="COMPLETED_AEE_CORRECTIVE_ENCODER_RETENTION_SEED_V8R",
