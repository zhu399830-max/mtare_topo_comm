...
from mtare_topo.governance_opening_collision_probe_v2 import SCHEMA,SLUG,POLICY,validate_card
...
def freeze():
    ...
    approval=dict(status="APPROVED",approved_by="user-standing-scope-authorization",approved_at="2026-09-07",
        ...
        scope="One failed observation39551 in S04_3d_unicyclic_small_C03__c1_mixed; existing reader decodes its10 cached observations50frames; only selected observation diagnosed; no labels or training",
        confirmation_reference="Standing user authorization for in-scope diagnosis of the observed failure; no whole-run retry, sample deletion, label change or protected data.")
    ...
    command=["env","CUDA_VISIBLE_DEVICES=","OMP_NUM_THREADS=1","OPENBLAS_NUM_THREADS=1","MKL_NUM_THREADS=1","PYTHONHASHSEED=20260906",PYTHON,
        "tools/v3/opening_collision_probe_v2.py","--spec",str(root/SPEC),"--run-dir",str(root/run)]
    ...
    spec=dict(schema_version="v3_run_spec_v1",gate=3,date="20260907",slug=SLUG,seed=20260906,operation="data_export",
        ...
        question="Does unique axis-contour ownership resolve the same failed observation without duplicate centers, data changes or lost diagnostic evidence?",
        method="Same failed observation39551: retain original duplicate/section diagnostic; generate jointV4 partial targets using unique axis-contour ownership; nonowning contours remain unknown provenance; no coordinate changes or training.",
        ...
        wall_time_cap_s=1800,estimated_cost=dict(compute="CPU only;1 analyzed observation5frames; collateral cached10 observations50frames from1task",host_ram_gb=4,gpu_vram_gb=0,disk_gb=.5,wall_time_hours=.5),
        acceptance_criteria=["Exact failed observation39551; fixed source hashes,poses and membership; no C08-C10, new scan rendering, models, labels or training.",
...
def execute(spec,run):
    ...
    from mtare_topo.teacher.gse_opening_collision_probe_v1 import inspect_corrected_collision
    ...
    try:
        ...
        with (run/"logs/observations.jsonl").open("x") as log:
            for row in card["scope"]["entries"]:
                ...
                for bundle in bundles:
                    ...
                    rows.append(inspect_corrected_collision(bundle))
                storage=write_evidence(run/"artifacts"/(active+".json.gz"),dict(task=active,observations=rows))
                entry=dict(task=active,observations=len(rows),duplicate_groups=sum(len(r["duplicate_supported_proposal_indices"]) for r in rows))
        ...
        summary=dict(status="FAILED" if error else "OPENING_COLLISION_PROBE_COMPLETE",error=error,active_task=active,
