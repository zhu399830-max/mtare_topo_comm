#!/usr/bin/env python3
"""Run and seal the approved built-in Isaac lidar Writer runtime control."""
...
RUN_ID = "gate0_20260810_isaac_official_lidar_writer_control_v1_seed0"
...
def _official_reference_hash() -> str:
    raw = subprocess.check_output(
        ["docker", "run", "--rm", "--entrypoint", "sha256sum", IMAGE, OFFICIAL_REFERENCE],
        text=True,
        stderr=subprocess.STDOUT,
    )
...
def main() -> int:
    ...
    if spec.get("operation") != "sensor_smoke" or spec.get("seed") != 0:
        raise RuntimeError("spec is not the approved official Writer runtime control")
    scope = spec.get("data_scope", {})
    if (
        scope.get("official_control_worlds") != 1
        or scope.get("cano_worlds") != 0
        or scope.get("built_in_control_sensors") != 1
        or scope.get("formal_dataset_samples") != 0
    ):
        raise RuntimeError("official control scope drifted")

    image_identity = _image_identity()
    official_reference_hash = _official_reference_hash()
    tool_paths = {
        "runner": Path(__file__).resolve(),
        "isaac_probe": PROJECT_ROOT / "tools/v3/isaac/probe_official_lidar_writer_runtime.py",
        "unit_test": PROJECT_ROOT / "tests/v3/unit/test_official_lidar_writer_control.py",
    }
    ...
    write_json(
        run_dir / "RUN_STATE.json",
        {
            ...
            "run_id": RUN_ID,
            "state": "RUNNING",
            "note": "Official built-in Writer runtime health control; zero project data/training/model counts.",
        },
    )
    ...
    command = [
        ...
        "/isaac-sim/python.sh",
        IMAGE,
        "/workspace/tools/v3/isaac/probe_official_lidar_writer_runtime.py",
        "--control-output-dir",
    ...
    capture_rc, duration = _run_logged(
        command, run_dir / "logs/01_official_writer_runtime_control.log", 600
    )
    subprocess.run(
        [
            "docker", "run", "--rm", "--user", "0:0", "-v", f"{run_dir}:/target:rw",
            "--entrypoint", "chown", IMAGE, "-R", f"{os.getuid()}:{os.getgid()}", "/target",
        ],
        check=True,
    ...
    log_text = (run_dir / "logs/01_official_writer_runtime_control.log").read_text(
        encoding="utf-8", errors="replace"
    )
    drain_timeout = "Timed out while waiting for pending Replicator writer schedules to drain" in log_text
    if runtime is None:
        overall = "FAIL_OFFICIAL_WRITER_RUNTIME_CONTROL_NO_SUMMARY"
        classification = "CONTROL_IMPLEMENTATION_OR_RUNTIME_EXCEPTION"
    elif runtime.get("overall_status") == "PASS_OFFICIAL_WRITER_RUNTIME_HEALTH":
        overall = "PASS_OFFICIAL_WRITER_RUNTIME_CONTROL"
        classification = "CUSTOM_SENSOR_OR_SCENE_COUPLING_REMAINS"
    else:
        overall = "FAIL_OFFICIAL_WRITER_RUNTIME_CONTROL"
        classification = runtime.get("classification", "UNKNOWN_RUNTIME_FAILURE")

    required = [
        runtime_path,
        run_dir / "artifacts/runtime/official_control_stage.usda",
    ]
    missing = [str(path.relative_to(run_dir)) for path in required if not path.is_file()]
    if missing:
        overall = "FAIL_OFFICIAL_WRITER_RUNTIME_CONTROL_MISSING_EVIDENCE"

    summary = {
        "schema_version": "official_lidar_writer_runtime_control_summary_v1",
        "run_id": RUN_ID,
        ...
        "claim_boundary": (
            "This result classifies only the pinned headless Isaac Writer runtime. The built-in "
            "control sensor is forbidden from replacing the project sensor or entering any dataset."
        ),
    }
    ...
    print(json.dumps({"overall_status": overall, "classification": classification, "sealed_files": sealed_files}, indent=2))
