...
from mtare_topo.governance_supplement_joint_v1 import SCHEMA,SLUG,POLICY,validate_card
...
def freeze():
    ...
    approval=dict(status="APPROVED",approved_by="user-standing-scope-authorization",approved_at="2026-09-07",
        ...
        scope="Fixed2676 C01-C07 fiveframe observations; original cached scans, poses, constructions and source codes; jointV3 partial anchors/openings/membership with full interface provenance; no qualified labels, model inference or training",
    ...
    command=["env","CUDA_VISIBLE_DEVICES=","OMP_NUM_THREADS=1","OPENBLAS_NUM_THREADS=1","MKL_NUM_THREADS=1","PYTHONHASHSEED=20260906",PYTHON,
        "tools/v3/supplement_joint_v1.py","--spec",str(root/SPEC),"--run-dir",str(root/run)]
    ...
    spec=dict(schema_version="v3_run_spec_v1",gate=3,date="20260907",slug=SLUG,seed=20260906,operation="data_export",
        ...
        question="Does jointV3 partial structural supervision cover the existing supplemental population across independent parents and entities?",
        method="Original source-interface geometry diagnostics plus jointV3 partial junction/terminal/window targets; missing witnesses not contradictions, other-node evidence vetoes; per-observation full raw interfaces and provenance stored as lossless gzip; no whole-region label promotion.",
        ...
        with (run/"logs/observations.jsonl").open("x") as log:
            for row in card["scope"]["entries"]:
                ...
                for bundle in bundles:
                    ...
                    storage=write_evidence(run/"artifacts"/name,dict(raw_interfaces=raw,produced_targets=output))
                    item=dict(source=source,anchors=len(output["record"]["anchors"]),openings=len(output["record"]["openings"]),
                        ...
                        target_record_sha256=output["target_record_sha256"],storage=storage)
                    rows.append(item)
