#!/usr/bin/env python3
"""Execute and seal three-seed relational exit-token transport training."""
...
PASS = "PASS_GSE_RELATIONAL_EXIT_TRANSPORT_TRAINING_V1"
FAIL = "FAIL_GSE_RELATIONAL_EXIT_TRANSPORT_TRAINING_V1"
...
BASELINE = PROJECT_ROOT / "results/gate3_semantics/gate3_20260828_gse_action_set_node_selection_corrective_v1_seed0"
...
def main() -> int:
    ...
    if run_dir.name != run_id or load_json(run_dir / "RUN_STATE.json").get("state") != "CREATED_NOT_EXECUTED":
        raise RuntimeError("relational training executes exactly once")
    ...
    try:
        ...
        if not validation.passed or card.get("status") != "APPROVED_FOR_ONE_IMMUTABLE_GSE_RELATIONAL_EXIT_TRANSPORT_TRAINING_V1" or card.get("approval", {}).get("authorized_operations") != ["training"]:
            raise RuntimeError(f"relational training Data Card invalid: {validation.errors}")
        ...
        if environment != expected:
            raise RuntimeError(f"relational training environment drift: {environment}")
        ...
        for seed in range(3):
            command = [str(PYTHON), str(PROJECT_ROOT / "tools/v3/train_gse_relational_exit_transport_event_v1.py"), "--cache-dir", str(ACTION / "scratch/action_set_cache"), "--output-dir", str(models / f"seed{seed}"), "--seed", str(seed), "--epochs", "6", "--batch-size", "128", "--evaluation-batch-size", "512", "--learning-rate", "0.0003", "--weight-decay", "0.0001"]
            ...
            if code != 0:
                raise RuntimeError(f"relational seed{seed} training failed")
            seed_summary = load_json(models / f"seed{seed}/summary.json")
            if seed_summary.get("optimizer_steps") != 6_666 or seed_summary.get("parameters") != 240_101 or seed_summary.get("perception_backbone_optimizer_steps") != 0:
                raise RuntimeError(f"relational seed{seed} optimization contract drift")
        evaluation = run_dir / "metrics/selection"
        command = [str(PYTHON), str(PROJECT_ROOT / "tools/v3/evaluate_gse_relational_exit_transport_selection_v1.py"), "--cache-dir", str(ACTION / "scratch/action_set_cache")]
        for seed in range(3):
            command.extend((f"--seed{seed}", str(models / f"seed{seed}/selection_outputs.npz")))
        command.extend(("--output-dir", str(evaluation), "--baseline-summary", str(BASELINE / "metrics/summary.json")))
        ...
        if code not in (0, 2):
            raise RuntimeError("relational selection evaluator program failure")
        selection = load_json(evaluation / "summary.json")
        scientific_pass = selection.get("status") == "PASS_GSE_RELATIONAL_EXIT_TRANSPORT_SELECTION_V1"
        if (code == 0) != scientific_pass:
            raise RuntimeError("relational selection status/return mismatch")
        required = [
            *[models / f"seed{seed}/{name}" for seed in range(3) for name in ("best.pt", "history.json", "selection_outputs.npz", "summary.json")],
            evaluation / "summary.json", evaluation / "selection_ensemble_outputs.npz", evaluation / "figure_source.json",
            *[evaluation / f"gse_relational_exit_transport_selection_v1.{suffix}" for suffix in ("png", "pdf", "svg")],
        ]
        if any(not path.is_file() or path.stat().st_size == 0 for path in required):
            raise RuntimeError("relational training evidence incomplete")
        after = {relative: sha256(PROJECT_ROOT / relative) for relative in before}
        if before != after:
            raise RuntimeError("relational training changed a frozen source")
    ...
    write_json(run_dir / "metrics/summary.json", {"schema_version": "gse_relational_exit_transport_training_outer_v1", "overall_status": overall, "scientific_pass": overall == PASS, "error": error, "subprocesses": subprocesses, "selection": selection, "duration_seconds": time.monotonic() - started, "source_unchanged": bool(before and before == after), "optimizer_steps": total_steps, "perception_backbone_optimizer_steps": 0, "c09_worlds_read": 0, "c10_worlds_read": 0, "mtare_worlds_read": 0})
