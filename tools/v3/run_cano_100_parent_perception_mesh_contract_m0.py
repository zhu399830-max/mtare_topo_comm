#!/usr/bin/env python3
"""Run and seal the approved Cano perception-mesh M0 contract once."""
...
RUN_ID = "gate0_20260811_cano_100_parent_perception_mesh_contract_m0_seed0"
EXECUTOR = PROJECT_ROOT / "tools/v3/execute_cano_100_parent_perception_mesh_contract_m0.py"
...
def _verify_seal(run_dir: Path, expected_count: int) -> dict:
    ...
    if len(lines) != expected_count or mismatches:
        raise RuntimeError(f"source seal verification failed: {result}")
    return result
...
def _run_executor(run_dir: Path) -> tuple[int, float]:
    ...
    try:
        ...
        output += "\nTIMEOUT: frozen 3600 second M0 limit reached.\n"
    duration = time.monotonic() - started
    (run_dir / "logs/01_cano_100_parent_perception_mesh_contract_m0.log").write_text(
...
def main() -> int:
    ...
    if (
        ...
        raise RuntimeError("spec approval or M0 scope is incompatible")
    ...
    if (
        ...
        raise RuntimeError("approved M0 proposal/card is missing")
    ...
    if set(spec.get("frozen_tools", {})) != set(tool_paths):
        raise RuntimeError("frozen tool set is not exact")
    ...
    if (
        load_json(SOURCE_V2R / "RUN_STATE.json").get("state") != "COMPLETED"
        or load_json(SOURCE_V2R / "metrics/summary.json").get("overall_status")
        != "PASS_CANO_100_TOPOLOGY_PARENT_RECIPE_RECLASSIFICATION_V2R"
    ):
        raise RuntimeError("V2R source is not a completed PASS")
    ...
    if shutil.disk_usage(PROJECT_ROOT).free < 2 * 1024**3:
        raise RuntimeError("less than 2 GiB free disk before M0")
    ...
    write_json(
        run_dir / "RUN_STATE.json",
        {
            ...
            "run_id": RUN_ID,
            "state": "RUNNING",
            "note": "Approved train-only M0: 20 native meshes, zero LiDAR/labels/training.",
        },
    )
    ...
    passed = bool(
        ...
        and summary.get("overall_status") == "PASS_CANO_100_PARENT_PERCEPTION_MESH_CONTRACT_M0"
    ...
    runner_summary = {
        "schema_version": "cano_100_parent_perception_mesh_contract_runner_summary_m0",
        "run_id": RUN_ID,
        ...
        "source_seals": {"v2r": v2r_seal, "v2": v2_seal},
        ...
        "claim_boundary": "Train-only native perception-mesh materialization/replay; zero LiDAR, labels, formal data, training, models, trajectories and M-TARE changes.",
    }
