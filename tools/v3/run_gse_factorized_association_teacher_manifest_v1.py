#!/usr/bin/env python3
"""Execute and seal one immutable factorized association Teacher manifest proof."""
...
CARD_STATUS = "APPROVED_FOR_ONE_IMMUTABLE_GSE_FACTORIZED_ASSOCIATION_TEACHER_MANIFEST_V1"
PYTHON = Path("/home/zeng-workstation/.local/share/mtare_topo_comm/envs/phase3_torch290_cu129_zarr2187_v1/bin/python")
EXECUTOR = PROJECT_ROOT / "tools/v3/execute_gse_factorized_association_teacher_manifest_v1.py"
...
INVENTORY = PROJECT_ROOT / "results/gate3_semantics/gate3_20260827_gse_factorized_association_inventory_v1_seed0"
...
def _sources() -> dict:
    return {
        ...
        "frozen_components": verify_failed_component_run_seal(
            PROJECT_ROOT, VERIFIER, "FAIL_GSE_EXIT_TOKEN_ASSOCIATION_CORRECTIVE_V2"
        ),
        "inventory_defect": verify_failed_component_run_seal(
            PROJECT_ROOT, INVENTORY, "FAIL_GSE_FACTORIZED_ASSOCIATION_INVENTORY_V1"
        ),
    }
...
def main() -> int:
    ...
    if run_dir.name != RUN_ID or load_json(run_dir / "RUN_STATE.json").get("state") != "CREATED_NOT_EXECUTED":
        raise RuntimeError("factorized association Teacher manifest may execute only once")
    ...
    try:
        if spec.get("gate") != 3 or spec.get("operation") != "audit" or spec.get("seed") != 0:
            raise RuntimeError("factorized association Teacher manifest scope drift")
        card = load_json(PROJECT_ROOT / spec["data_card"])
        if card.get("status") != CARD_STATUS or card.get("approval", {}).get("status") != "APPROVED":
            raise RuntimeError("factorized association Teacher manifest Data Card drift")
        for name, record in spec["frozen_tools"].items():
            if _sha256(PROJECT_ROOT / record["path"]) != record["sha256"]:
                raise RuntimeError(f"frozen Teacher manifest tool drift: {name}")
        for relative, expected in spec["frozen_inputs"].items():
            actual = _sha256(PROJECT_ROOT / relative)
            if actual != expected:
                raise RuntimeError(f"frozen Teacher manifest input drift: {relative}")
        ...
        expected_environment = {"python": "3.13.5", "numpy": "2.1.3", "matplotlib": "3.10.0"}
        if environment != expected_environment:
            raise RuntimeError(f"factorized association Teacher manifest sidecar drift: {environment}")
        ...
        write_json(run_dir / "RUN_STATE.json", {
            ...
            "note": "C01-C08 identity-balanced decision association Teacher manifest; zero training/inference.",
        })
        command = [
        ...
        log = run_dir / "logs/00_factorized_association_teacher_manifest.log"
        ...
        metric_path = run_dir / "metrics/factorized_association_teacher_manifest.json"
        result = load_json(metric_path) if metric_path.is_file() else {}
        sources_after = _sources()
        for relative, expected in before.items():
            actual = _sha256(PROJECT_ROOT / relative)
            if actual != expected:
                raise RuntimeError(f"source changed during Teacher manifest proof: {relative}")
            after[relative] = actual
        required = [
            run_dir / "artifacts/identity_balanced_association_manifest.jsonl",
            run_dir / "artifacts/identity_balanced_association_manifest.csv",
            run_dir / "previews/gse_factorized_association_teacher_manifest.png",
            run_dir / "previews/gse_factorized_association_teacher_manifest.pdf",
            run_dir / "previews/gse_factorized_association_teacher_manifest.svg",
            run_dir / "previews/gse_factorized_association_teacher_manifest_source.json",
        ]
        ...
        if (
            ...
            or output_bytes > 256 * 1024**2 or not all(item.is_file() for item in required)
        ):
            raise RuntimeError("factorized association Teacher manifest evidence contract drift")
    ...
    write_json(run_dir / "metrics/summary.json", {
        "schema_version": "gse_factorized_association_teacher_manifest_outer_v1",
        ...
        "source_unchanged": bool(before and before == after), "manifest": result,
