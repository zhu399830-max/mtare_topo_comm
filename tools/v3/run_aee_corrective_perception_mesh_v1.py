#!/usr/bin/env python3
"""Run and seal one approved corrective 20-parent perception-mesh batch."""
...
RUN_ID = "gate2_20260821_aee_corrective_perception_mesh_v1_seed20260821"
...
def main() -> int:
    ...
    paths = {
        ...
        "data_card": card_path,
        "environment": _approved_project_file(spec.get("environment", {}).get("path"), "environment.path"),
    }
    ...
    if (
        source_state.get("overall_status") != "PASS_AEE_CORRECTIVE_TOPOLOGY_CANDIDATE_AUDIT_V1"
    ...
    write_json(run_dir / "RUN_STATE.json", {"schema_version":"v3_run_state_v1","run_id":RUN_ID,"state":"RUNNING","note":"Twenty corrective perception meshes only."})
    ...
    output = ""
    try:
        done = subprocess.run(argv, cwd=PROJECT_ROOT, text=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, timeout=TIME_LIMIT_SECONDS, check=False)
    ...
    passed = bool(code == 0 and summary and summary.get("overall_status") == "PASS_AEE_CORRECTIVE_PERCEPTION_MESH_V1" and summary.get("scope") == EXPECTED_SCOPE and summary.get("checks") and all(summary["checks"].values()) and len(meshes) == len(sanitation) == len(metrics) == 20 and len(previews) == 10 and duration <= TIME_LIMIT_SECONDS and size <= DISK_LIMIT_BYTES)
    overall = "PASS_AEE_CORRECTIVE_PERCEPTION_MESH_V1" if passed else "FAIL_AEE_CORRECTIVE_PERCEPTION_MESH_V1"
    boundary = "Twenty immutable perception meshes only; zero LiDAR, teacher, formal data, training, C09/C10 or planner change."
    write_json(run_dir / "metrics/runner_summary.json", {"schema_version":"aee_corrective_perception_mesh_runner_v1","run_id":RUN_ID,"overall_status":overall,"executor_exit_code":code,"duration_seconds":duration,"counts":{"meshes":len(meshes),"sanitation":len(sanitation),"metrics":len(metrics),"train_previews":len(previews)},"result_bytes_before_seal":size,"disk_limit_bytes":DISK_LIMIT_BYTES,"host":{"platform":platform.platform()},"claim_boundary":boundary})
