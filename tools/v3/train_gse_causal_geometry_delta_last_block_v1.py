#!/usr/bin/env python3
"""Train the bounded last-encoder-block causal geometry-delta fallback."""
...
def _forward(backbone, head, student) -> dict[str, torch.Tensor]:
    return forward_last_block_adaptation(backbone, head, student)
...
def train(args: argparse.Namespace) -> dict[str, Any]:
    ...
    for seed in (0, 1, 2):
        ...
        trainability = configure_last_block_adaptation(backbone, head)
        last_block_parameters, head_parameters = trainable_parameters(backbone, head)
        frozen_digest_before = _parameter_digest(
            (name, parameter)
            for name, parameter in backbone.named_parameters()
            if not parameter.requires_grad
        )
        last_block_digest_before = _parameter_digest(
            (name, parameter)
            for name, parameter in backbone.named_parameters()
        ...
        optimizer = torch.optim.AdamW(
            [
                ...
                {"params": last_block_parameters, "lr": args.last_block_learning_rate},
            ],
        ...
        with epoch_file.open("w", encoding="utf-8") as stream:
            for epoch in range(1, args.epochs + 1):
                ...
                # cuDNN requires the frozen GRU to run in training mode when
                # gradients pass through it to encoder[-1].  Its weights stay
                # outside the optimizer and the one-layer GRU has no dropout;
                # the per-seed digest below independently proves immutability.
                backbone.train()
                head.train()
                ...
                for regression_batch, event_batch in zip(regression_chunks, event_chunks, strict=True):
                    ...
                    norm = torch.nn.utils.clip_grad_norm_(
                        [*head_parameters, *last_block_parameters], 5.0
                    )
                    if not torch.isfinite(norm):
                        raise RuntimeError("nonfinite last-block geometry-delta multitask gradient")
                    gradient_audit = audit_last_block_gradients(backbone, head)
                    optimizer.step()
                    optimizer_steps += 1
                ...
                if epoch % args.evaluate_every == 0 or epoch == args.epochs:
                    ...
                    if best_rank is None or rank > best_rank:
                        ...
                        torch.save(
                            {
                                "schema_version": "gse_causal_geometry_delta_last_block_checkpoint_v1",
                                "seed": seed,
                                "epoch": epoch,
                                "head": {name: value.detach().cpu() for name, value in head.state_dict().items()},
                                "last_encoder_block": {
                                    name: value.detach().cpu()
                                    for name, value in backbone.encoder[-1].state_dict().items()
                                },
                                "source_backbone_checkpoint": str(checkpoint_path.relative_to(PROJECT_ROOT)),
                                "source_backbone_epoch": int(source["epoch"]),
                                "optimizer_steps": optimizer_steps,
                                "architecture": "bounded_last_encoder_block_shared_directional_change_explicit_delta_plus_event_v1",
                                "delta_scale": GEOMETRY_DELTA_SCALE,
                                "trainability": trainability,
                                "optimizer_learning_rates": {
                                    "head": args.head_learning_rate,
                                    "last_encoder_block": args.last_block_learning_rate,
                                },
                            },
                            seed_dir / "best.pt",
                        )
        ...
        frozen_digest_after = _parameter_digest(
            (name, parameter)
            for name, parameter in backbone.named_parameters()
            if not parameter.requires_grad
        )
        last_block_digest_after = _parameter_digest(
            (name, parameter)
            for name, parameter in backbone.named_parameters()
            if parameter.requires_grad
        )
        if frozen_digest_before != frozen_digest_after:
            raise RuntimeError("frozen backbone parameter digest changed")
        if last_block_digest_before == last_block_digest_after:
            raise RuntimeError("last encoder block did not update")
        seed_summary = {
            "seed": seed,
            "best_epoch": best_epoch,
            "optimizer_steps": optimizer_steps,
            "last_encoder_block_optimizer_steps": optimizer_steps,
            ...
            "parameter_update_audit": {
                "frozen_backbone_digest_before": frozen_digest_before,
                "frozen_backbone_digest_after": frozen_digest_after,
                "frozen_backbone_unchanged": True,
                "last_encoder_block_digest_before": last_block_digest_before,
                "last_encoder_block_digest_after": last_block_digest_after,
                "last_encoder_block_changed": True,
            },
        }
    ...
    summary = {
        "schema_version": "gse_causal_geometry_delta_last_block_training_v1",
        "overall_status": PASS_STATUS if scientific_pass else FAIL_STATUS,
        "scientific_pass": scientific_pass,
        "method": "three_seed_bounded_last_encoder_block_shared_directional_change_explicit_delta_plus_event",
        "fit_worlds": 60,
        ...
        "last_encoder_block_optimizer_steps": int(
            sum(value["last_encoder_block_optimizer_steps"] for value in seed_summaries.values())
        ),
        "frozen_backbone_optimizer_steps": 0,
...
def main() -> int:
    ...
    parser.add_argument("--last-block-learning-rate", type=float, default=1e-4)
    ...
    if (
        ...
        or not math.isclose(args.head_learning_rate, 1e-3)
        or not math.isclose(args.last_block_learning_rate, 1e-4)
        or not math.isclose(args.weight_decay, 1e-4)
    ):
        raise RuntimeError("geometry-delta last-block V1 hyperparameters are frozen")
    result = train(args)
