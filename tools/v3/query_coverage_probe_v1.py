...
from mtare_topo.governance_query_coverage_probe_v1 import SCHEMA,SLUG,POLICY,validate_card
...
def freeze():
    ...
    approval=dict(status="APPROVED",approved_by="user-standing-scope-authorization",approved_at="2026-09-08",
        ...
        scope="One existing C01 observation225,frames289-293; reader decodes7 cached observations35frames of S01_flat_tree_small_C01__c1_mixed only. Reuse sealed V6 partial targets; fixed 64 spatial queries; no model predictions, full labels or training",
    ...
    command=["env","CUDA_VISIBLE_DEVICES=","OMP_NUM_THREADS=1","OPENBLAS_NUM_THREADS=1","MKL_NUM_THREADS=1","PYTHONHASHSEED=20260906",PYTHON,
        "tools/v3/query_coverage_probe_v1.py","--spec",str(root/SPEC),"--run-dir",str(root/run)]
    ...
    spec=dict(schema_version="v3_run_spec_v1",gate=3,date="20260908",slug=SLUG,seed=20260906,operation="data_export",
        ...
        question="Which fixed diagnostic queries have conditional reference coverage on sealed C01 observation225?",
        method="Rebuild one fixed ray grid from original five scans, bind sealed V6 targets; score a predetermined 64-position lattice with 1/2/4m diagnostic radii, retaining unknown. Not model predictions or calibration.",
...
def execute(spec,run):
    ...
    from mtare_topo.teacher.gse_query_coverage_probe_v1 import inspect_coverage
