...
from mtare_topo.governance_supplement_joint_v3 import SCHEMA,SLUG,POLICY,validate_card
from mtare_topo.governance_surface_selection import digest
from mtare_topo.governance_surface_material import read_pinned
from mtare_topo.data.gse_supplement_teacher_scope_v1 import compile_supplement_teacher_scope
from mtare_topo.data.gse_joint_v4_cache_v1 import compile_cache,read_v4,compare_versions
...
def freeze():
    ...
    approval=dict(status="APPROVED",approved_by="user-standing-scope-authorization",approved_at="2026-09-08",
        ...
        scope="Fixed2676 C01-C07 fiveframe observations; original cached scans, poses, constructions and source codes; jointV6 partial anchors/openings/membership with full interface provenance; no qualified labels, model inference or training",
    ...
    command=["env","CUDA_VISIBLE_DEVICES=","OMP_NUM_THREADS=1","OPENBLAS_NUM_THREADS=1","MKL_NUM_THREADS=1","PYTHONHASHSEED=20260906",PYTHON,
        "tools/v3/supplement_joint_v3.py","--spec",str(root/SPEC),"--run-dir",str(root/run)]
    ...
    spec=dict(schema_version="v3_run_spec_v1",gate=3,date="20260908",slug=SLUG,seed=20260906,operation="data_export",
        ...
        question="Does jointV6 partial structural supervision cover the existing supplemental population across independent parents and entities?",
        method="Reuse all2676 sealed V4 raw interfaces without ray recasting; regenerate jointV6 targets and record every opening/membership change; fixed anchors and scoring region cannot change; partial junction/terminal/window targets; missing witnesses not contradictions, other-node evidence vetoes; per-observation full raw interfaces and provenance stored as lossless gzip; no whole-region label promotion.",
...
def execute(spec,run):
    ...
    from mtare_topo.teacher.gse_joint_reference_targets_v6 import produce_joint_reference_targets
