#!/usr/bin/env python3
"""Publish the paper-ready GSE perception comparison from sealed evidence."""
...
def publish(run_dir: Path, destination: Path) -> dict:
    ...
    sources = (
        ...
        run_dir / "artifacts/calibration/summary.json",
    )
    for path in sources:
    ...
    axes[0].bar(seeds + width / 2, [row["gse_calibrated_macro_f1"] for row in event["per_seed"]], width, color="#2A9D8F", label="GSE-Graph")
    ...
    write_json(
        destination / f"{FIGURE_ID}_provenance.json",
        {
            ...
            "selection_rule": "all three predeclared seeds and all four continuous geometry fields; no cherry-picking",
