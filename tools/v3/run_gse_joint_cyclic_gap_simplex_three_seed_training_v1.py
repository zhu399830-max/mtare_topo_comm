#!/usr/bin/env python3
"""Execute and seal three-seed Joint Cyclic Gap Simplex training."""
...
PASS = "PASS_GSE_JOINT_CYCLIC_GAP_SIMPLEX_THREE_SEED_TRAINING_V1"
FAIL = "FAIL_GSE_JOINT_CYCLIC_GAP_SIMPLEX_THREE_SEED_TRAINING_V1"
INNER_PASS = "PASS_GSE_JOINT_CYCLIC_GAP_SIMPLEX_SELECTION_V1"
CARD_STATUS = "APPROVED_FOR_ONE_IMMUTABLE_GSE_JOINT_CYCLIC_GAP_SIMPLEX_THREE_SEED_TRAINING_V1"
...
def main() -> int:
    ...
    if run.name != run_id or load_json(run / "RUN_STATE.json").get("state") != "CREATED_NOT_EXECUTED":
        raise RuntimeError("JCGS training executes exactly once")
    started = time.monotonic()
    overall, error, selection = FAIL, None, {}
    before, after, processes, commands = {}, {}, [], []
    try:
        ...
        for seed in range(3):
            command = [
                str(PYTHON), str(PROJECT_ROOT / "tools/v3/train_gse_joint_cyclic_gap_simplex_v1.py"),
            ...
            if code != 0:
                raise RuntimeError(f"seed{seed} JCGS training failed")
            summary = load_json(models / f"seed{seed}/summary.json")
            if (summary.get("schema_version") != "gse_joint_cyclic_gap_simplex_training_seed_v1" or summary.get("optimizer_steps") != 11370 or summary.get("parameters") != 789650 or summary.get("c08_checkpoint_observations") != 0 or summary.get("development_output_observations") != 45942):
                raise RuntimeError(f"seed{seed} split/optimization drift")
        evaluation = run / "metrics/selection"
        command = [str(PYTHON), str(PROJECT_ROOT / "tools/v3/evaluate_gse_joint_cyclic_gap_simplex_selection_v1.py"), "--teacher-root", str(TEACHER / "artifacts/export/teacher"), "--source-root", str(DATASET / "artifacts/dataset/train"), "--output-dir", str(evaluation)]
        for seed in range(3):
        ...
        if code not in (0, 2):
            raise RuntimeError("JCGS evaluator program failure")
        ...
        if (code == 0) != passed:
            raise RuntimeError("JCGS selection status/return mismatch")
        ...
        required = [
            ...
            *[evaluation / f"gse_joint_cyclic_gap_simplex_selection_v1.{suffix}" for suffix in ("png", "pdf", "svg")],
        ]
        if any(not path.is_file() or path.stat().st_size == 0 for path in required):
            raise RuntimeError("JCGS training evidence incomplete")
    ...
    write_json(run / "metrics/summary.json", {
        "schema_version": "gse_joint_cyclic_gap_simplex_three_seed_training_outer_v1",
