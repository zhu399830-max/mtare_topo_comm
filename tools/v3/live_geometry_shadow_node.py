...
def main():
    ...
    parser.add_argument('--learned-socket',type=Path)
    parser.add_argument('--checkpoint-sha256')
    parser.add_argument('--policy',type=Path,required=True,
                        help='Frozen JSON with composition_policy, anchor_spacing_m, lookahead_m')
    args=parser.parse_args()
    policy=json.loads(args.policy.read_text())
    if args.learned_socket:
        from mtare_topo.integration.local_geometry_inference import LocalGeometryPredictor
        policy.update(predictor=LocalGeometryPredictor(args.learned_socket),checkpoint_sha256=args.checkpoint_sha256)
    ...
    def odometry(message):
        # Inference must not starve incoming odometry behind the scan lock.
        with pose_lock:
        ...
        with lock:
            ...
            try:
                ...
                if executor is not None and result is not None:
                    if args.learned_socket and (rospy.Time.now()-scan.header.stamp).to_sec()>.5:
                        raise ValueError('learned result exceeds 0.5 second live age limit')
                ...
                failed=True
                if args.learned_socket and waypoints is not None and poses:
                    # Best-effort hold; supervisor also terminates this simulator
                    # on node failure. Never keep commanding a stale model goal.
                    hold=PointStamped();hold.header.frame_id='map';hold.header.stamp=rospy.Time.now()
                    with pose_lock:
                        body=poses[-1].pose.pose.position
                    hold.point.x,hold.point.y,hold.point.z=body.x,body.y,body.z
                    waypoints.publish(hold)
