...
CARD_STATUS = "APPROVED_FOR_ONE_IMMUTABLE_GSE_SPARSE_RELATION_OBJECTIVE_RECALL_CORRECTIVE_V1R"
...
def main() -> int:
    ...
    try:
        ...
        command = [str(PYTHON), str(PROJECT_ROOT / "tools/v3/execute_gse_sparse_relation_objective_recall_corrective_v1r.py"), "--dataset-root", str(PROJECT_ROOT / "results/gate2_representation/gate2_20260824_gse_deduplicated_dataset_export_v1_seed0/artifacts/dataset/train"), "--sequence-manifest", str(PROJECT_ROOT / "results/gate2_representation/gate2_20260824_gse_deduplicated_dataset_export_v1_seed0/artifacts/sequence_manifest.jsonl"), "--output-dir", str(run / "metrics/corrective")]
        ...
        required = [run / f"metrics/corrective/gse_sparse_relation_objective_recall_corrective_v1r.{suffix}" for suffix in ("png", "pdf", "svg")]
    ...
    summary = {"schema_version": "gse_sparse_relation_objective_recall_corrective_outer_v1r", "overall_status": overall, "scientific_pass": overall == PASS, "error": error, "result": result, "duration_seconds": time.monotonic() - started, "source_unchanged": bool(before and before == after), "optimizer_steps": 0, "model_forward_observations": 0, "checkpoints_written": 0, "c09_worlds_read": 0, "c10_worlds_read": 0, "mtare_worlds_read": 0, "graph_replays": 0, "planner_calls": 0}
