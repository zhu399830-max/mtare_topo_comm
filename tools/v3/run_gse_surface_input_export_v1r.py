#!/usr/bin/env python3
"""V1R input export: preserve virtual-environment invocation; no other contract change."""
import argparse
...
from freeze_gse_surface_input_export_v1r import current_environment
...
def execute(spec, run, root=PROJECT_ROOT):
    ...
    try:
        ...
        executable = invoked_executable.resolve(strict=True)
        env = current_environment()
        freeze = subprocess.check_output([str(invoked_executable), "-m", "pip", "freeze", "--all"], text=True)
        ...
        write(run / "config/execution_environment.json", {**env, "platform": platform.platform(),
            "invoked_executable": str(invoked_executable), "executable": str(executable), "executable_sha256": sha(executable),
