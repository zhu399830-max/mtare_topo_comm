"""Single CPU export of833 paired voxel/native-SPG partitions."""
...
from mtare_topo.governance_bidirectional_partitions_v1 import SCHEMA,SLUG,validate_card
...
def freeze():
    ...
    approval=dict(status='APPROVED',approved_by='user-standing-scope-authorization',approved_at='2026-09-09',
                  ...
                  scope='833 existing C01-C07 compact student caches,fixed bidirectional fit/calibration population, exact source frame count in scope;CPU partitions only.',
                  confirmation_reference='User standing autonomous C01-C07 scope authorization; PLAN reuse sealed860 shared feature population, export missing833 only and cached features for paired representations; no training or new labels.')
    ...
    command=['env','OMP_NUM_THREADS=1','OPENBLAS_NUM_THREADS=1','PYTHONHASHSEED=0',
             ...
             'tools/v3/bidirectional_partitions_v1.py','--spec',str(ROOT/SPEC),'--run-dir',str(ROOT/run)]
    spec=dict(schema_version='v3_run_spec_v1',gate=3,date='20260909',slug=SLUG,seed=0,operation='data_export',
              ...
              question='Can all833 fixed observations provide identical-point voxel/SPG inputs for the paired training?',
              ...
              wall_time_cap_s=3600,estimated_cost=dict(compute='CPU833 paired partitions; no GPU',host_ram_gb=4,gpu_vram_gb=0,disk_gb=2,wall_time_hours=1),
              acceptance_criteria=['Exactly833 original source identities and full ROI ray mappings; no point deletion or missing representation substitution.',
                                   'Finite bounded partitions,833 immutable packets,complete source hashes and seal; no training or semantic success claim.'],
    ...
    try:
        ...
        with (run/'logs/observations.jsonl').open('x') as log:
            for row in card['scope']['observations']:
                ...
                print(json.dumps(dict(completed=len(entries),total=833,groups=entry['groups'],elapsed_s=time.monotonic()-start)),flush=True)
                if sum(p.stat().st_size for p in run.rglob('*') if p.is_file())>2*1024**3:raise RuntimeError('output cap')
        if len(entries)!=833:raise ValueError('population incomplete')
