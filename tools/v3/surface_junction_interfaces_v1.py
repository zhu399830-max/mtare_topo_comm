...
from mtare_topo.governance_junction_interfaces_v1 import SCHEMA,SLUG,POLICY,validate_card
...
def freeze():
    ...
    approval=dict(status="APPROVED",approved_by="user-standing-scope-authorization",approved_at="2026-09-07",
        ...
        scope="Fixed30C01observations150frames;ordered source interface crossings before first return only;no semantic labels or training",
    ...
    command=["env","CUDA_VISIBLE_DEVICES=","OMP_NUM_THREADS=1","OPENBLAS_NUM_THREADS=1","MKL_NUM_THREADS=1","PYTHONHASHSEED=20260906",PYTHON,
        "tools/v3/surface_junction_interfaces_v1.py","--spec",str(root/SPEC),"--run-dir",str(root/run)]
    ...
    spec=dict(schema_version="v3_run_spec_v1",gate=3,date="20260907",slug=SLUG,seed=20260906,operation="data_export",
        ...
        question="Which original nearby junction interfaces have ordered same-ray intersections before saved first return inside the fixed ROI?",
        method="Original .05m/64 closed source meshes intersected separately; incident endpoint cap faces retained independently; original float32 caster parameters; strict pre-return open10m crossings; coincident groups remain unordered; no semantic or traversability claims.",
    ...
    try:
        ...
        with (run/"logs/observations.jsonl").open("x") as log:
            for row in card["scope"]["selected"]:
                ...
                output=diagnose_observation(bundle)
                write(run/"artifacts"/(row["view_id"]+".json"),output)
                entry=dict(task=active,local_junction_references=output["nearby_junction_references"],interfaces=len(output["interfaces_teacher_only"]),raw_intersections=len(output["raw_interface_intersections"]),rays_with_crossings=len(output["rays"]),rays_with_ordered_pairs=sum(bool(r["ordered_adjacent_crossings"]) for r in output["rays"]))
        ...
        summary=dict(status="FAILED" if error else "JUNCTION_INTERFACE_DIAGNOSTIC_COMPLETE",error=error,active_task=active,
