...
from mtare_topo.governance_continuous_features_v1 import SCHEMA, SLUG, POLICY, validate_card
...
def freeze():
    ...
    approval = dict(status="APPROVED", approved_by="user-standing-scope-authorization", approved_at="2026-09-09",
        ...
        confirmation_reference="User standing autonomous authorization; PLAN allows same frozen encoder features for exact30 continuous windows in C04, no training or new labels.",
        scope="Exact30 fiveframe inputs; same historical seed0epoch2 frozen encoder; no labels,selection,training,C08-C10 or graphs")
    ...
    command = ["env", "OMP_NUM_THREADS=1", "OPENBLAS_NUM_THREADS=1", "MKL_NUM_THREADS=1", "PYTHONHASHSEED=0",
               PYTHON, "tools/v3/continuous_features_v1.py", "--spec", str(root / SPEC), "--run-dir", str(root / run)]
    spec = dict(schema_version="v3_run_spec_v1", gate=3, date="20260909", slug=SLUG, seed=0, operation="data_export",
        ...
        question="Extract the original frozen encoder features for30 fixed rolling windows, shared by downstream C0/C1 diagnostics without teacher input.",
        ...
        wall_time_cap_s=1800, estimated_cost=dict(compute="5090D inference only30windows", host_ram_gb=32, gpu_vram_gb=28, disk_gb=8, wall_time_hours=.5),
        acceptance_criteria=["All30 exact source windows exported once; no source IDs or labels in encoder input.",
        ...
        expected_evidence=["30compactcaches,source/weight hashes,per-observation provenance,environment,logs,summary,RUN_STATE,seal"],
...
def execute(spec, run):
    ...
    try:
        ...
        with (run / "logs/observations.jsonl").open("x") as log:
            for task in scope["tasks"]:
                ...
                for row in task["observations"]:
                    observation = bind_feature_input(raw, expected_sha256=task["input_sha256"], task=task["task"],
                        row=row["input_row"], source_sequence_id=row["source_sequence_id"], frame_rows=row["frame_rows"], observation_count=10)
            ...
            if len(entries) != 30 or module_state_sha256(bound.backbone) != bound.state_sha256:
