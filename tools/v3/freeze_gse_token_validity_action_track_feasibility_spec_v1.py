#!/usr/bin/env python3
"""Freeze one immutable frozen-token validity/action-track audit."""
...
RUN_ID = "gate3_20260829_gse_token_validity_action_track_feasibility_v1_seed0"
SPEC = PROJECT_ROOT / "configs/v3/gate3/gse_token_validity_action_track_feasibility_v1.json"
DATA_CARD = "configs/v3/gate3/data_cards/gse_token_validity_action_track_feasibility_v1.json"
...
def main() -> int:
    if SPEC.exists():
        raise RuntimeError("token-validity feasibility spec already exists")
    ...
    inputs.extend(f"{cache}/{name}.npy" for name in (
        "raw_tokens", "partition_code", "parent_id", "traversal_id", "sequence_index", "global_sequence_index",
    ))
    inputs.append(f"{cache}/manifest.json")
    tools = {
        ...
        "freezer": "tools/v3/freeze_gse_token_validity_action_track_feasibility_spec_v1.py",
    ...
    spec = {
        ...
        "date": "20260829", "slug": "gse_token_validity_action_track_feasibility_v1", "seed": 0,
        ...
        "method": "Verify all C01-C08 sources; attach Teacher validity for scoring only; evaluate tied-threshold current confidence, past-only five-frame descriptor-track mean and Teacher-free three-seed heading/width/profile consensus.",
        ...
        "user_authorization": {"status": "APPROVED", "approved_by": "user-standing-authorization", "approved_at": "2026-08-29T01:30:00+08:00", "authorized_gates": [3], "authorized_operations": ["audit"], "scope": "One immutable zero-training C01-C08 frozen-token validity audit.", "confirmation_reference": "User explicitly delegated best-method choices and continuous execution."},
        "acceptance_criteria": [
            "Exact 80 worlds, 252430 frames, 188126 observations, C07/C08 21548/24394 and per-seed 396913 positive plus 731843 negative slots.",
            "Retained Teacher action-set and all-seed descriptor transport metrics remain at least 0.98.",
            "Every seed reaches token recall at least 0.50 at tied-threshold precision at least 0.995.",
            "Every seed's best causal/consensus score improves AP at least 0.05 and retains high-precision recall at least 0.50.",
            "Zero optimizer/new inference/threshold fitting/graph/C09/C10/M-TARE; source unchanged and complete evidence."
        ],
