...
RUN_ID = "gate2_20260822_aee_corrective_encoder_retention_v8r_seed20260822"
TRAINER = PROJECT_ROOT / "tools/v3/train_aee_corrective_encoder_retention_v8r.py"
STATUS_PASS = "PASS_AEE_CORRECTIVE_ENCODER_RETENTION_V8R"
STATUS_FAIL = "FAIL_AEE_CORRECTIVE_ENCODER_RETENTION_V8R"
...
def validate_seed_summary(summary: dict[str, Any], seed: int) -> None:
    ...
    if not isinstance(summary.get("gate"), dict) or set(summary["gate"]) != expected:
        raise RuntimeError(f"V8R gate schema drift: {seed}")
...
def main() -> int:
    ...
    try:
        ...
        if card.get("status") != "APPROVED_FOR_AEE_CORRECTIVE_ENCODER_RETENTION_V8R":
        ...
        system_predecessor = (PROJECT_ROOT / spec["system_predecessor"]["run"]).resolve()
        system_predecessor.relative_to(PROJECT_ROOT)
        system_entries = base.verify_seal(system_predecessor, spec["system_predecessor"]["seal_sha256"])
        system_summary = load_json(system_predecessor / "metrics/summary.json")
        if system_summary.get("overall_status") != "FAIL_AEE_CORRECTIVE_ENCODER_RETENTION_V8" or system_summary.get("failure_reason") != "V8 effective-class-mass drift: 0" or system_summary.get("completed_seeds") != 0:
            raise RuntimeError("V8 system predecessor drift")
        ...
        write_json(run_dir / "config/input_integrity.json", {"source_dataset": {"run": str(source.relative_to(PROJECT_ROOT)), "seal_entries": seal_entries, "seal_sha256": spec["source_dataset_seal_sha256"]}, "scientific_predecessor": {"run": str(predecessor.relative_to(PROJECT_ROOT)), "seal_entries": predecessor_entries, "seal_sha256": spec["scientific_predecessor_seal_sha256"], "status": spec["scientific_predecessor_status"], "partial_reuse": False}, "system_predecessor": {"run": str(system_predecessor.relative_to(PROJECT_ROOT)), "seal_entries": system_entries, "seal_sha256": spec["system_predecessor"]["seal_sha256"], "status": "FAIL_AEE_CORRECTIVE_ENCODER_RETENTION_V8", "partial_reuse": False}, "checkpoints": checkpoints, "environment": environment, "python_executable": str(python), "live_pip_freeze_sha256": freeze["pip_freeze_sha256"], "pip_check": pip_evidence, "host": platform.platform()})
