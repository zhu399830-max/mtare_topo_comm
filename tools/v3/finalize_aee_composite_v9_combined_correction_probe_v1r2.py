...
"""Bind the Python-3.8-compatible V1R2 V5 probe to the V1R audit."""
...
import run_aee_composite_v9_combined_correction_probe_v1r2 as probe_wrapper
from run_mtare_single_robot_stochastic_v1 import sha256


probe = probe_wrapper.base
PROPOSAL_SPEC = Path("configs/v3/gate6/aee_composite_v9_combined_correction_probe_v1r2.proposal.json")
PROPOSAL_CARD = Path("configs/v3/gate6/data_cards/aee_composite_v9_combined_correction_probe_v1r2.proposal.json")
FINAL_SPEC = Path("configs/v3/gate6/aee_composite_v9_combined_correction_probe_v1r2.json")
FINAL_CARD = Path("configs/v3/gate6/data_cards/aee_composite_v9_combined_correction_probe_v1r2.json")
...
def finalize(
    *,
    ...
    if final_spec_path.exists() or final_card_path.exists():
        raise RuntimeError("final V1R2 combined-correction probe card/spec already exists")
    proposal = load_json(proposal_spec_path)
    card_proposal = load_json(proposal_card_path)
    pending = "PENDING_PY38_COMPAT_SOURCE_BINDING_NOT_EXECUTABLE"
    if proposal.get("status") != pending or card_proposal.get("status") != pending:
        raise RuntimeError("V1R2 combined-correction probe is not a pending proposal")
    for name, item in proposal["frozen_tools"].items():
        if sha256(_within(root, item["path"])) != item["sha256"]:
            raise RuntimeError(f"V1R2 combined-correction probe tool drift: {name}")

    failed_probe = _within(root, proposal["failed_probe_run"])
    failed_seal_path = failed_probe / "artifacts/evidence_sha256.txt"
    if not failed_seal_path.is_file() or sha256(failed_seal_path) != proposal["failed_probe_seal_sha256"]:
        raise RuntimeError("V1R2 failed-probe seal identity drift")
    failed_state = load_json(failed_probe / "RUN_STATE.json")
    failed_summary = load_json(failed_probe / "metrics/summary.json")
    if (
        failed_state.get("state") != "FAILED"
        or failed_state.get("overall_status")
        != "FAIL_AEE_COMPOSITE_V9_COMBINED_CORRECTION_PROBE_V1R"
        or failed_summary.get("failure_reason")
        != "readiness case failed with exit 1: aee-v5-combined-correction-probe"
        or failed_summary.get("completed_cases") != 0
    ):
        raise RuntimeError("V1R2 predecessor is not the exact Python-3.8 entry failure")

    audit_root = _within(root, proposal["mechanism_audit_run"])
    audit_seal_path = audit_root / "artifacts/evidence_sha256.txt"
    if not audit_seal_path.is_file():
        raise RuntimeError("V1R2 mechanism audit has no final seal")
    audit_seal = sha256(audit_seal_path)
    ...
    probe_case = {
        "case_id": f"{selected['world']}_env{selected['environment_seed']}_m1d_seed{selected['checkpoint_seed']}_combined_v5_probe",
    ...
    final["frozen_tools"]["data_card"] = {"path": final_card_path.relative_to(root).as_posix(), "sha256": sha256(final_card_path)}
    finalizer = Path(__file__).resolve()
    final["frozen_tools"]["proposal_finalizer"] = {"path": finalizer.relative_to(root).as_posix(), "sha256": sha256(finalizer)}
