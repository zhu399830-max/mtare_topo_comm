...
from mtare_topo.governance_supplement_support_v1 import SCHEMA,SLUG,POLICY,validate_card
...
def freeze():
    ...
    approval=dict(status="APPROVED",approved_by="user-standing-scope-authorization",approved_at="2026-09-07",
        ...
        confirmation_reference="User authorized same-C01-C07 supplementary sampling (那你想办法呀, 继续执行) and autonomous local teacher development. Exact missing2676 input export is sealed; this operation checks its observed source evidence, no resampling or label promotion.")
    ...
    command=["env","CUDA_VISIBLE_DEVICES=","OMP_NUM_THREADS=1","OPENBLAS_NUM_THREADS=1","MKL_NUM_THREADS=1","PYTHONHASHSEED=20260906",PYTHON,
        "tools/v3/supplement_support_v1.py","--spec",str(root/SPEC),"--run-dir",str(root/run)]
    ...
    spec=dict(schema_version="v3_run_spec_v1",gate=3,date="20260907",slug=SLUG,seed=20260906,operation="data_export",
        ...
        method="Original degree-one .05m/64 cap faces; original normalize64/cast32 ray inputs and Open3D exact t_hit equality; unique saved source; junction per-incidence source presence (not branch visibility); open10m ROI; no tolerance enlargement or labels.",
