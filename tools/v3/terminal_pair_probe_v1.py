...
from mtare_topo.governance_terminal_pair_probe_v1 import SCHEMA,SLUG,POLICY,validate_card
...
def freeze():
    ...
    approval=dict(status="APPROVED",approved_by="user-standing-scope-authorization",approved_at="2026-09-08",
        ...
        scope="One existing C01 observation225,frames289-293; reader decodes7 cached observations35frames of S01_flat_tree_small_C01__c1_mixed only. Pair sealed V4 with V6 partial targets; no full labels or training",
    ...
    command=["env","CUDA_VISIBLE_DEVICES=","OMP_NUM_THREADS=1","OPENBLAS_NUM_THREADS=1","MKL_NUM_THREADS=1","PYTHONHASHSEED=20260906",PYTHON,
        "tools/v3/terminal_pair_probe_v1.py","--spec",str(root/SPEC),"--run-dir",str(root/run)]
    ...
    spec=dict(schema_version="v3_run_spec_v1",gate=3,date="20260908",slug=SLUG,seed=20260906,operation="data_export",
        ...
        question="Does V6 restore witnessed terminal correspondence on sealed C01 observation225 without changing anchor/opening geometry?",
        method="Reuse sealed original interfaces; compare exact V4/V6 source and anchor/opening geometry, retain unknown and full provenance; output new version without overwriting old targets.",
        ...
        acceptance_criteria=["Exact observation225; fixed sources/poses and geometry; only evidence-backed terminal membership may change; no C08-C10, new scans, models or training.",
...
def execute(spec,run):
    ...
    from mtare_topo.teacher.gse_terminal_pair_probe_v1 import inspect_pair
    ...
    try:
        ...
        with (run/"logs/observations.jsonl").open("x") as log:
            for row in card["scope"]["entries"]:
                ...
                for bundle in bundles:
                    ...
                    rows.append(inspect_pair(bundle, root, reader.opened))
                storage=write_evidence(run/"artifacts"/(active+".json.gz"),dict(task=active,observations=rows))
                entry=dict(task=active,observations=len(rows),positive_memberships=sum(r["new_positive_memberships"] for r in rows))
        ...
        summary=dict(status="FAILED" if error else "TERMINAL_PAIR_PROBE_COMPLETE",error=error,active_task=active,
