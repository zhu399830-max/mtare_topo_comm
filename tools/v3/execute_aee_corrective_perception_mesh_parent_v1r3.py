#!/usr/bin/env python3
"""Materialize exactly one sealed corrective parent in a fresh process."""
...
from execute_aee_corrective_perception_mesh_v1 import (
    SOURCE_RUN,
    _parents,
    _strata,
)
...
def execute_parent(run_dir: Path, parent_index: int, expected_parent_id: str) -> dict:
    ...
    primary, vertices = m1r._sanitize_materialization(
        root, primary, graph, splines
    )
    record = {
    ...
    if parent["split"] == "corrective_train":
        m0._render_complete_train_map(
            ...
            splines,
            preview_path,
            stage_label="AEE CORRECTIVE MESH V1R3",
        )
    ...
    receipt = {
        "schema_version": "aee_corrective_perception_mesh_parent_receipt_v1r3",
        "parent_index": parent_index,
        "parent_id": parent["parent_id"],
        "split": parent["split"],
        "mesh_sha256": primary["mesh_sha256"],
