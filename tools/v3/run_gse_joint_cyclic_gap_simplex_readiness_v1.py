#!/usr/bin/env python3
"""Execute and seal the immutable Joint Cyclic Gap Simplex readiness."""
...
CARD_STATUS = "APPROVED_FOR_ONE_IMMUTABLE_GSE_JOINT_CYCLIC_GAP_SIMPLEX_READINESS_V1"
...
def main() -> int:
    ...
    if run.name != run_id or load_json(run / "RUN_STATE.json").get("state") != "CREATED_NOT_EXECUTED":
        raise RuntimeError("gap-simplex readiness executes exactly once")
    ...
    try:
        ...
        command = [str(PYTHON), str(PROJECT_ROOT / "tools/v3/execute_gse_joint_cyclic_gap_simplex_readiness_v1.py"), "--teacher-root", str(TEACHER / "artifacts/export/teacher"), "--source-root", str(DATASET / "artifacts/dataset/train"), "--output-dir", str(run / "artifacts/readiness")]
        ...
        if returncode not in (0, 2):
            raise RuntimeError(f"gap-simplex executor system failure: {returncode}")
        ...
        if (returncode == 0) != passed:
            raise RuntimeError("gap-simplex status/return mismatch")
    ...
    entries = seal(run); print(json.dumps({"overall_status": overall, "error": error, "decision": result.get("decision"), "seal_entries": entries}, indent=2))
