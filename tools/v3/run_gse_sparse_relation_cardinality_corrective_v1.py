#!/usr/bin/env python3
"""Execute and seal the sparse-relation cardinality corrective exactly once."""
...
FAIL = "FAIL_GSE_SPARSE_RELATION_CARDINALITY_CORRECTIVE_V1"
CARD_STATUS = "APPROVED_FOR_ONE_IMMUTABLE_GSE_SPARSE_RELATION_CARDINALITY_CORRECTIVE_V1"
...
PREDECESSOR = PROJECT_ROOT / "results/gate3_semantics/gate3_20260829_gse_axis_anchored_event_relation_readiness_v2_seed0"
SPARSE = PROJECT_ROOT / "results/gate3_semantics/gate3_20260829_gse_sparse_circular_relation_transport_readiness_v1_seed0"
...
def main() -> int:
    ...
    if run.name != run_id or load_json(run / "RUN_STATE.json").get("state") != "CREATED_NOT_EXECUTED":
        raise RuntimeError("cardinality corrective executes exactly once")
    ...
    try:
        ...
        with unit_log.open("w", encoding="utf-8") as stream:
            ...
            ], cwd=PROJECT_ROOT, env=process_environment, stdout=stream, stderr=subprocess.STDOUT, text=True, timeout=600, check=False)
        subprocesses.append({"stage": "unit_tests", "returncode": int(unit.returncode)})
        if unit.returncode != 0:
            raise RuntimeError("cardinality corrective unit tests failed")
        output = run / "metrics/corrective"
        command = [
            str(PYTHON), str(PROJECT_ROOT / "tools/v3/execute_gse_sparse_relation_cardinality_corrective_v1.py"),
            "--dataset-root", str(DATASET / "artifacts/dataset/train"),
            "--sequence-manifest", str(DATASET / "artifacts/sequence_manifest.jsonl"),
            "--predecessor-readiness-summary", str(PREDECESSOR / "metrics/summary.json"),
            "--predecessor-checkpoint", str(SLOT / "artifacts/models/seed0/best.pt"),
            "--sealed-sparse-readiness-summary", str(SPARSE / "metrics/readiness/summary.json"),
        ...
        subprocesses.append({"stage": "corrective", "returncode": int(completed.returncode), "peak_host_rss_kib": peak_rss_kib})
        if completed.returncode != 0:
            raise RuntimeError("cardinality corrective subprocess failed")
        result = load_json(output / "summary.json")
        if result.get("status") != PASS or result.get("scientific_pass") is not True:
            raise RuntimeError("cardinality corrective result contract failed")
        required = [
            output / "summary.json", output / "figure_source.json", output / "corrective_checks.csv",
            *[output / f"gse_sparse_relation_cardinality_corrective_v1.{suffix}" for suffix in ("png", "pdf", "svg")],
        ]
        if any(not path.is_file() or path.stat().st_size == 0 for path in required):
            raise RuntimeError("cardinality corrective evidence incomplete")
        after = {relative: sha256(PROJECT_ROOT / relative) for relative in before}
        if before != after:
            raise RuntimeError("frozen cardinality corrective source changed")
        overall = PASS
    ...
    write_json(run / "metrics/summary.json", {
        "schema_version": "gse_sparse_relation_cardinality_corrective_outer_v1",
        "overall_status": overall, "scientific_pass": overall == PASS, "error": error,
        "subprocesses": subprocesses, "peak_host_rss_kib": peak_rss_kib, "result": result,
