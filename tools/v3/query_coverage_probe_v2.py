...
from mtare_topo.governance_query_coverage_probe_v2 import SCHEMA,SLUG,POLICY,validate_card
...
def freeze():
    ...
    approval=dict(status="APPROVED",approved_by="user-standing-scope-authorization",approved_at="2026-09-08",
        ...
        scope="One existing C01 observation225,frames289-293; reader decodes7 cached observations35frames of S01_flat_tree_small_C01__c1_mixed only. Reuse sealed V6 partial targets; four fixed-rule target-relative diagnostic queries; no model predictions, full labels or training",
    ...
    command=["env","CUDA_VISIBLE_DEVICES=","OMP_NUM_THREADS=1","OPENBLAS_NUM_THREADS=1","MKL_NUM_THREADS=1","PYTHONHASHSEED=20260906",PYTHON,
        "tools/v3/query_coverage_probe_v2.py","--spec",str(root/SPEC),"--run-dir",str(root/run)]
    ...
    spec=dict(schema_version="v3_run_spec_v1",gate=3,date="20260908",slug=SLUG,seed=20260906,operation="data_export",
        ...
        question="Does independent confirmed-structure support resolve duplicate query coverage on sealed C01 observation225 without changing unknown competition or targets?",
        method="Rebuild one fixed ray grid from original five scans, bind sealed V6 targets; pair V1/V2 on exact target and fixed +0.1m X-offset queries for anchor and opening with 1/2/4m diagnostic radii, retaining unknown. Not model predictions or calibration.",
        ...
        wall_time_cap_s=1800,estimated_cost=dict(compute="CPU only;1 analyzed observation5frames; collateral cached7 observations35frames from1task",host_ram_gb=4,gpu_vram_gb=0,disk_gb=.5,wall_time_hours=.5),
...
def execute(spec,run):
    ...
    from mtare_topo.teacher.gse_query_coverage_probe_v2 import inspect_coverage
    ...
    try:
        ...
        with (run/"logs/observations.jsonl").open("x") as log:
            for row in card["scope"]["entries"]:
                ...
                for bundle in bundles:
                    ...
                    rows.append(inspect_coverage(bundle, root, reader.opened))
                storage=write_evidence(run/"artifacts"/(active+".json.gz"),dict(task=active,observations=rows))
                entry=dict(task=active,observations=len(rows),diagnostic_queries=sum(r["query_count"] for r in rows))
        ...
        summary=dict(status="FAILED" if error else "QUERY_COVERAGE_PROBE_COMPLETE",error=error,active_task=active,
