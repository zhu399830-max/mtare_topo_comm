#!/usr/bin/env python3
"""Run and seal the fast structural-node evidence funnel."""
...
EXECUTOR = PROJECT_ROOT / "tools/v3/execute_gse_structural_node_evidence_funnel_v1.py"
...
PASS = "PASS_GSE_STRUCTURAL_NODE_EVIDENCE_FUNNEL_V1"
FAIL = "FAIL_GSE_STRUCTURAL_NODE_EVIDENCE_FUNNEL_V1"
...
def _verify(spec: dict) -> dict[str, str]:
    ...
    for relative, expected in spec["frozen_inputs"].items():
        ...
        if actual != expected:
            raise RuntimeError(f"structural-node frozen input drift: {relative}")
...
def main() -> int:
    ...
    if run_dir.name != run_id or state.get("state") != "CREATED_NOT_EXECUTED":
        raise RuntimeError("structural-node evidence audit may execute only once")
    ...
    try:
        if spec.get("gate") != 3 or spec.get("operation") != "audit":
            raise RuntimeError("structural-node evidence scope drift")
        for record in spec["frozen_tools"].values():
            if _sha(PROJECT_ROOT / record["path"]) != record["sha256"]:
                raise RuntimeError(f"structural-node frozen tool drift: {record['path']}")
        ...
        command = [
            ...
            "--output-dir", str(run_dir / "metrics/funnel"),
        ]
        ...
        if returncode not in (0, 2):
            raise RuntimeError("structural-node evidence executor failed as a program")
        result = load_json(run_dir / "metrics/funnel/summary.json")
        scientific_pass = result.get("status") == PASS
        if (returncode == 0) != scientific_pass:
            raise RuntimeError("structural-node evidence status/return mismatch")
        source_after = _verify(spec)
        if source_before != source_after:
            raise RuntimeError("structural-node evidence source changed during audit")
    ...
    summary = {
        "schema_version": "gse_structural_node_evidence_funnel_outer_v1",
