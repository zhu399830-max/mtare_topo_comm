"""Single immutable conversion of sealed rays; never train or rerender."""
import argparse
import json
...
from mtare_topo.data.covered_sensor_conversion import convert, file_sha
...
def execute(spec,run):
    ...
    try:
        ...
        with (run/'logs/progress.jsonl').open('x') as log:
            ...
            metrics=convert(ROOT,run,card['scope'],progress)
        verify()
    except Exception:
        error=traceback.format_exc();(run/'logs/error.log').write_text(error)
    finally: signal.alarm(0)
    summary=dict(metrics,state='CONVERSION_PASS' if error is None else 'CONVERSION_FAIL',
