...
from calibration_opposite_teacher_scope_v1 import compile_scope
from request_scoped_teacher_client_v1 import RequestScopedTeacherClient

base=original.base;ROOT=base.ROOT
SLUG='gse_v8_opposite289_v1';SCHEMA='v3_v8_opposite289_card_v1'
...
def freeze():
    ...
    a=dict(status='APPROVED',approved_by='user-standing-development-authorization',approved_at='2026-09-09',
        ...
        scope='Exact289 opposite-direction calibration observations,4 C07 parents,12 variant tasks; historical-route diagnostic, not fitting or threshold selection.',
        confirmation_reference='User standing autonomous permission for evidence-backed execution improvements; current PLAN permits opposite289 teacher diagnosis after scan/motion verification; no automatic training qualification; standing authorization covers scoped development execution after preflight. No old run overwrite, no training/test or threshold changes.')
    ...
    spec.update(slug=SLUG,data_card=CARD,config_path=CARD,user_authorization=a,
        question='Does unchanged V8 provide multi-position observed junction references on the fixed289 opposite-direction observations, including approaching junctions?',
        ...
        baseline='Original-direction289 V8 coverage: all157 positive anchors behind sensor. Opposite observations are distinct inputs, so compare visibility and position coverage, not pointwise paired accuracy.',
        ...
        acceptance_criteria=['Exactly289 calibration observations397 variant frames4 parents; no fit, development-evaluation or test payload.',
            'Keep all original sources in order, same frozen archive, original geometry and unknown masks.',
            'All289 finish under the original cap, no dropped observations or retries; no training or threshold calibration.'],
        expected_evidence=['289 full outputs, source hashes, logs, runtime, memory, RUN_STATE, seal'],
        environment=base.environment(),teacher_environment=base.old_environment(),
        estimated_cost=dict(compute='CPU only289 calibration diagnostic observations; no sensor rerendering/model/training',host_ram_gb=4,gpu_vram_gb=0,disk_gb=30,wall_time_hours=3),
        execution_correction=dict(index=1,only_override='request_object_lifetime',batching=False,explicit_gc=False,
            ...
            fresh_worker_confound=True,reused_input_observations=289))
    spec['command']=['env','MALLOC_ARENA_MAX=2','CUDA_VISIBLE_DEVICES=','OMP_NUM_THREADS=1','OPENBLAS_NUM_THREADS=1',
        'PYTHONHASHSEED=20260906',base.PYTHON,'tools/v3/v8_opposite289_run_v1.py','--spec',str(ROOT/SPEC),'--run-dir',str(ROOT/RUN)]
