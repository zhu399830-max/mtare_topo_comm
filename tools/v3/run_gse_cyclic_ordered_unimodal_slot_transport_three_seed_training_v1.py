#!/usr/bin/env python3
"""Execute and seal three-seed COUST training."""
...
PASS = "PASS_GSE_CYCLIC_ORDERED_UNIMODAL_SLOT_TRANSPORT_THREE_SEED_TRAINING_V1"
FAIL = "FAIL_GSE_CYCLIC_ORDERED_UNIMODAL_SLOT_TRANSPORT_THREE_SEED_TRAINING_V1"
INNER_PASS = "PASS_GSE_CYCLIC_ORDERED_UNIMODAL_SLOT_TRANSPORT_SELECTION_V1"
CARD_STATUS = "APPROVED_FOR_ONE_IMMUTABLE_GSE_CYCLIC_ORDERED_UNIMODAL_SLOT_TRANSPORT_THREE_SEED_TRAINING_V1"
...
def main() -> int:
    ...
    if run.name != run_id or load_json(run / "RUN_STATE.json").get("state") != "CREATED_NOT_EXECUTED":
        raise RuntimeError("COUST training executes exactly once")
    ...
    try:
        ...
        for seed in range(3):
            command = [
                str(PYTHON), str(PROJECT_ROOT / "tools/v3/train_gse_cyclic_ordered_unimodal_slot_transport_v1.py"),
            ...
            if code != 0:
                raise RuntimeError(f"seed{seed} COUST training failed")
            summary = load_json(models / f"seed{seed}/summary.json")
            if (summary.get("schema_version") != "gse_cyclic_ordered_unimodal_slot_transport_training_seed_v1"
                    or summary.get("optimizer_steps") != 11370 or summary.get("parameters") != 788618
        ...
        command = [
            str(PYTHON), str(PROJECT_ROOT / "tools/v3/evaluate_gse_cyclic_ordered_unimodal_slot_transport_selection_v1.py"),
            "--teacher-root", str(TEACHER / "artifacts/export/teacher"),
        ...
        if code not in (0, 2):
            raise RuntimeError("COUST evaluator program failure")
        ...
        if (code == 0) != passed:
            raise RuntimeError("COUST selection status/return mismatch")
        ...
        required = [
            ...
            *[evaluation / f"gse_cyclic_ordered_unimodal_slot_transport_selection_v1.{suffix}" for suffix in ("png", "pdf", "svg")],
        ]
        if any(not path.is_file() or path.stat().st_size == 0 for path in required):
            raise RuntimeError("COUST training evidence incomplete")
    ...
    write_json(run / "metrics/summary.json", {
        "schema_version": "gse_cyclic_ordered_unimodal_slot_transport_three_seed_training_outer_v1",
    ...
    entries = seal(run)
    print(json.dumps({"overall_status": overall, "error": error, "optimizer_steps": steps, "decision": selection.get("decision"), "seal_entries": entries}, indent=2))
