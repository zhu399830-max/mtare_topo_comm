#!/usr/bin/env python3
"""Materialize the frozen 10,000-frame corrective Cano sensor/teacher set."""
...
def main() -> int:
    ...
    for world_index, asset in enumerate(assets):
        ...
        selected, selector_audit = select_corrective_cano_clusters(candidates)
        replayed, replay_audit = select_corrective_cano_clusters(candidates)
        if selector_audit != replay_audit or [item["cluster_id"] for item in selected] != [item["cluster_id"] for item in replayed]:
            raise RuntimeError(f"{parent_id}: selector deterministic replay failed")
        ...
        for cluster in selected:
            ...
            for frame in frame_contract(cluster, splines_document, fta):
                ...
                if not replay_passed or not teacher_passed:
                    raise RuntimeError(
                        f"{parent_id}:{cluster['cluster_id']}:{frame['frame_index']} failed replay/teacher contract"
                    )
        ...
        totals.update({"worlds": 1, "clusters": len(selected), "frames": frame_index})
        print(json.dumps({"world": parent_id, "index": world_index + 1, "of": 20, "frames": frame_index}), flush=True)
    frame_stream.close()
    cluster_stream.close()
    if totals != Counter({"frames": 10_000, "clusters": 2_000, "worlds": 20}):
    ...
    summary = {
        ...
        "overall_status": "PASS_AEE_CORRECTIVE_CANO_DATASET_V1",
        "worlds": 20,
        "train_worlds": 10,
