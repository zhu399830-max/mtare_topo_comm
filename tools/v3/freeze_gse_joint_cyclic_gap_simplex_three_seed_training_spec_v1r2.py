#!/usr/bin/env python3
"""Freeze canonical-periodic JCGS V1R2 three-seed training."""
...
RUN_ID = "gate3_20260829_gse_joint_cyclic_gap_simplex_three_seed_training_v1r2_seed0"
SPEC = PROJECT_ROOT / "configs/v3/gate3/gse_joint_cyclic_gap_simplex_three_seed_training_v1r2.json"
DATA_CARD = "configs/v3/gate3/data_cards/gse_joint_cyclic_gap_simplex_three_seed_training_v1r2.json"
...
def main() -> int:
    if SPEC.exists():
        raise RuntimeError("JCGS V1R2 spec already exists")
    card = load_json(PROJECT_ROOT / DATA_CARD)
    validation = validate_data_card(card)
    if not validation.passed:
        raise RuntimeError(f"JCGS V1R2 card invalid: {validation.errors}")
    ...
    readiness = "results/gate3_semantics/gate3_20260829_gse_joint_cyclic_gap_simplex_readiness_v5r_seed0"
    ...
    baseline_attribution = "results/gate3_semantics/gate3_20260829_gse_coust_complex_cardinality_failure_attribution_v1_seed0"
    inputs = [
        DATA_CARD,
        ...
        f"{failed_v1}/RUN_STATE.json", f"{failed_v1}/metrics/summary.json", f"{failed_v1}/artifacts/evidence_sha256.txt",
        f"{failed_v1r}/RUN_STATE.json", f"{failed_v1r}/metrics/summary.json", f"{failed_v1r}/artifacts/evidence_sha256.txt",
        f"{baseline}/RUN_STATE.json", f"{baseline}/metrics/summary.json", f"{baseline}/artifacts/evidence_sha256.txt",
        f"{baseline_attribution}/RUN_STATE.json", f"{baseline_attribution}/metrics/summary.json", f"{baseline_attribution}/artifacts/evidence_sha256.txt",
    ...
    tools = {
        ...
        "base_runner_helpers": "tools/v3/run_gse_joint_cyclic_gap_simplex_three_seed_training_v1.py",
        "v1r_runner": "tools/v3/run_gse_joint_cyclic_gap_simplex_three_seed_training_v1r.py",
        "runner": "tools/v3/run_gse_joint_cyclic_gap_simplex_three_seed_training_v1r2.py",
        "freezer": "tools/v3/freeze_gse_joint_cyclic_gap_simplex_three_seed_training_spec_v1r2.py",
    ...
    spec = {
        ...
        "date": "20260829", "slug": "gse_joint_cyclic_gap_simplex_three_seed_training_v1r2", "seed": 0,
        "operation": "training", "data_card": DATA_CARD,
        "question": "Can correctly canonicalized JCGS recover strict K>=3 exit geometry and transferable safe confidence strongly enough to establish the GSE-Graph semantic observation method?",
        "method": "Exact V1/V1R schedule from scratch with canonical periodic endpoint indexing proven on the attributed failure batch; joint phase plus closed positive gaps, cyclic supervision, periodic exit geometry and one C07 refusal threshold.",
        "baseline": "COUST overall exact2 0.4156/0.3870, K3 0.00240/0.00179, K4 0/0.00433 on C07/C08; V1/V1R have no scientific result.",
        ...
        "acceptance_criteria": [
            "Three from-scratch seeds x ten epochs x 11370 steps; 789650 parameters and finite gradients.",
            "One C07 threshold yields precision >=0.995 and recall >=0.50 on C07/C08.",
            "Raw count >=0.80, deployed action macro-F1 >=0.80, overall raw/safe exact >=0.50.",
            "K3 raw/safe >=0.40/0.30 and K4 >=0.20/0.10 on both.",
            "Overall/K3/K4 exact2 gain >=0.05 over COUST on both.",
            "Exit bearing <=1deg, width <=3m, profile <=1m; global geometry including slope <=2deg.",
            "C08 checkpoint=0; C09/C10/M-TARE/graph/planner=0; sources unchanged."
        ],
        ...
        "command": ["/usr/bin/timeout", "86400s", PYTHON, "tools/v3/run_gse_joint_cyclic_gap_simplex_three_seed_training_v1r2.py", "--spec", str(SPEC), "--run-dir", str(PROJECT_ROOT / f"results/gate3_semantics/{RUN_ID}")]
    }
