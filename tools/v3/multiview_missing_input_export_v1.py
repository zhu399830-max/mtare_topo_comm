...
from mtare_topo.governance_multiview_missing_input_v1 import SCHEMA,SLUG,validate_card
from mtare_topo.data.multiview_input_reuse_v1 import compile_scope
SELECTION='configs/v3/gate3/multiview_input_reuse_inventory_v1.json'
from mtare_topo.data.multiview_missing_input_export_v1 import export_population
...
def freeze():
    ...
    authority=dict(status='APPROVED',approved_by='user-standing-development-authorization',approved_at='2026-09-09',
        ...
        scope='Missing2055 windows,fit1890/calibration165,65parents195tasks,exact six fields; existing204 packages unchanged; development135 excluded.',
    ...
    command=['env','OMP_NUM_THREADS=1','OPENBLAS_NUM_THREADS=1',PYTHON,'tools/v3/multiview_missing_input_export_v1.py',
    ...
    spec=dict(schema_version='v3_run_spec_v1',gate=3,date='20260909',slug=SLUG,seed=20260906,operation='data_export',
        ...
        question='Export exact original scan and relative-motion inputs for2055 missing multiview windows without teacher labels.',
        ...
        wall_time_cap_s=600,estimated_cost=dict(compute='CPU only;370593792 decoded padded bytes;0model/optimizer',disk_gb=2,wall_time_hours=1/6,host_ram_gb=4,gpu_vram_gb=0),
        acceptance_criteria=['Exact2055 missing windows2835 unique variant frames with original source seals and no out-of-scope reads.',
            'Six fields source-bound,finite and original dtype; exact current-origin zero and source-window identities.',
            '195 six-field NPZ packages bind all2055 encoder inputs; overlapping scan bytes agree; no invented safety/graph success.'],
...
def execute(spec,run):
    ...
    try:
        ...
        summary=dict(status='FAILED' if error else 'SIX_FIELD_INPUT_EXPORT_COMPLETE',error=error,
            observations=2055 if result else 0,elapsed_s=time.monotonic()-start,
