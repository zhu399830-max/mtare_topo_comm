...
RUN='results/gate3_semantics/gate3_20260909_gse_calibration_opposite_remaining_export_v1_seed20260906'
...
def check():
    ...
    inventory=json.loads(read('configs/v3/gate3/calibration_opposite_inventory_v1.json',
                             '6a7431c5cefe876ef07d2244d76d5334bb71325ca7ebc242448a19b5ea341e39'))
    seal=read(RUN+'/artifacts/evidence_sha256.txt','36515259846c86012937681d771c161b3bd92d71ed65c141e0c709b1544adeb7')
    ...
    for package in manifest['reused_inputs']:
        for i,source in enumerate(package['observations']):
            k=key(source)
            if k in index:raise ValueError('reused/new duplicate')
            index[k]=dict(path=package['path'],sha256=package['sha256'],row=i,
                          container_rows=len(package['observations']))
