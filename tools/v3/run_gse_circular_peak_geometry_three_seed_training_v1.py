#!/usr/bin/env python3
"""Execute and seal circular peak geometry three-seed training."""
...
PASS = "PASS_GSE_CIRCULAR_PEAK_GEOMETRY_THREE_SEED_TRAINING_V1"
FAIL = "FAIL_GSE_CIRCULAR_PEAK_GEOMETRY_THREE_SEED_TRAINING_V1"
...
def main() -> int:
    ...
    if run_dir.name != run_id or load_json(run_dir / "RUN_STATE.json").get("state") != "CREATED_NOT_EXECUTED":
        raise RuntimeError("circular peak training executes exactly once")
    ...
    try:
        ...
        if not validation.passed or card.get("status") != "APPROVED_FOR_ONE_IMMUTABLE_GSE_CIRCULAR_PEAK_GEOMETRY_THREE_SEED_TRAINING_V1" or card.get("approval", {}).get("authorized_operations") != ["training"] or card.get("approval", {}).get("authorized_gates") != [3]:
            raise RuntimeError(f"circular peak training Data Card invalid: {validation.errors}")
        ...
        expected = {"python": "3.13.5", "numpy": "2.1.3", "torch": "2.9.0+cu129", "cuda": "12.9", "zarr": "2.18.7", "gpu": "NVIDIA GeForce RTX 5090 D"}
        if environment != expected:
            raise RuntimeError(f"circular peak training environment drift: {environment}")
        ...
        for seed in range(3):
            command = [str(PYTHON), str(PROJECT_ROOT / "tools/v3/train_gse_circular_peak_geometry_v1.py"), "--teacher-root", str(TEACHER / "artifacts/export/teacher"), "--source-root", str(DATASET / "artifacts/dataset/train"), "--output-dir", str(models / f"seed{seed}"), "--seed", str(seed), "--epochs", "10", "--batch-size", "128", "--evaluation-batch-size", "256", "--learning-rate", "0.0003", "--weight-decay", "0.0001"]
            ...
            if code != 0:
                raise RuntimeError(f"circular peak seed{seed} training failed")
            summary = load_json(models / f"seed{seed}/summary.json")
            if summary.get("optimizer_steps") != 11370 or summary.get("parameters") != 769268 or summary.get("c08_checkpoint_observations") != 0 or summary.get("development_output_observations") != 45942:
                raise RuntimeError(f"circular peak seed{seed} optimization/split drift")
        ...
        if code not in (0, 2):
            raise RuntimeError("circular peak evaluator program failure")
        ...
        if (code == 0) != scientific_pass:
            raise RuntimeError("circular peak selection status/return mismatch")
        required = [
        ...
        if any(not path.is_file() or path.stat().st_size == 0 for path in required):
            raise RuntimeError("circular peak training evidence incomplete")
        after = {relative: sha256(PROJECT_ROOT / relative) for relative in before}
        if before != after:
            raise RuntimeError("circular peak training changed a frozen source")
    ...
    write_json(run_dir / "metrics/summary.json", {"schema_version": "gse_circular_peak_geometry_three_seed_training_outer_v1", "overall_status": overall, "scientific_pass": overall == PASS, "error": error, "subprocesses": subprocesses, "selection": selection, "duration_seconds": time.monotonic() - started, "source_unchanged": bool(before and before == after), "optimizer_steps": total_steps, "model_inference_observations": inference_observations, "c08_checkpoint_observations": 0, "c09_worlds_read": 0, "c10_worlds_read": 0, "mtare_worlds_read": 0, "graph_replays": 0, "planner_calls": 0})
