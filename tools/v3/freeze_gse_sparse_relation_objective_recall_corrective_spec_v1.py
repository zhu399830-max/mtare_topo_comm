...
RUN_ID = "gate3_20260829_gse_sparse_relation_objective_recall_corrective_v1_seed0"
SPEC = PROJECT_ROOT / "configs/v3/gate3/gse_sparse_relation_objective_recall_corrective_v1.json"
CARD = "configs/v3/gate3/data_cards/gse_sparse_relation_objective_recall_corrective_v1.json"
...
def main():
    if SPEC.exists():
        raise RuntimeError("objective-recall corrective spec exists")
    ...
    inputs = [
        ...
        *[item for run in (failed_v2, failed_v2r) for item in (f"{run}/RUN_STATE.json", f"{run}/metrics/summary.json", f"{run}/artifacts/evidence_sha256.txt")],
    ...
    tools = {
        ...
        "executor": "tools/v3/execute_gse_sparse_relation_objective_recall_corrective_v1.py",
        "runner": "tools/v3/run_gse_sparse_relation_objective_recall_corrective_v1.py",
        "freezer": "tools/v3/freeze_gse_sparse_relation_objective_recall_corrective_spec_v1.py",
    ...
    spec = {
        ...
        "execution_phase": 3,
        "date": "20260829",
        "slug": "gse_sparse_relation_objective_recall_corrective_v1",
        "seed": 0,
        "operation": "audit",
        "data_card": CARD,
        "question": "Does the sparse-token evaluator count every objective token and persistent/reveal/withdraw relation, including missed endpoints, in recall and false negatives?",
        "method": "Read all C01-C08 causal Teacher histories once, count objective set relations independently of model emissions, and prove with a missing-endpoint construction that precision stays candidate-based while recall/FN/F1 use the full Teacher denominator.",
        ...
        "acceptance_criteria": [
            ...
            "Token and relation FN, positives, recall and F1 use the objective denominator; all unit tests pass.",
        ...
        "command": ["/usr/bin/timeout", "1800s", PYTHON, "tools/v3/run_gse_sparse_relation_objective_recall_corrective_v1.py", "--spec", str(SPEC), "--run-dir", str(PROJECT_ROOT / f"results/gate3_semantics/{RUN_ID}")],
    }
