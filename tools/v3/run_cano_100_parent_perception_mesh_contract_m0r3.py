#!/usr/bin/env python3
"""Run and seal the approved Cano perception-mesh M0R3 exactly once."""
...
RUN_ID = (
    "gate0_20260811_cano_100_parent_perception_mesh_contract_"
    "m0r3_point_to_surface_seed0"
)
EXECUTOR = PROJECT_ROOT / "tools/v3/execute_cano_100_parent_perception_mesh_contract_m0r3.py"
FAILED_M0R2 = PROJECT_ROOT / (
    "results/gate0_baseline/"
    "gate0_20260811_cano_100_parent_perception_mesh_contract_"
    "m0r2_triangle_discretization_seed0"
)
...
def _run_executor(run_dir: Path) -> tuple[int, float]:
    ...
    try:
        ...
        output += "\nTIMEOUT: frozen 3600 second M0R3 limit reached.\n"
    duration = time.monotonic() - started
    (run_dir / "logs/01_cano_100_parent_perception_mesh_contract_m0r3.log").write_text(
...
def main() -> int:
    ...
    if (
        ...
        raise RuntimeError("spec approval or unchanged M0R3 scope is incompatible")
    ...
    if (
        ...
        raise RuntimeError("approved M0R3 proposal/card is missing")
    ...
    if set(spec.get("frozen_tools", {})) != set(tool_paths):
        raise RuntimeError("frozen M0R3 tool set is not exact")
    ...
    failed_state = load_json(FAILED_M0R2 / "RUN_STATE.json")
    if (
        failed_state.get("state") != "FAILED"
        or failed_state.get("overall_status")
        != "FAIL_CANO_100_PARENT_PERCEPTION_MESH_CONTRACT_M0R2"
    ):
        raise RuntimeError("M0R2 predecessor is not the sealed nearest-vertex FAIL")
    m0r2_seal = _verify_seal(FAILED_M0R2, 146)
    ...
    if shutil.disk_usage(PROJECT_ROOT).free < 2 * 1024**3:
        raise RuntimeError("less than 2 GiB free disk before M0R3")
    ...
    write_json(run_dir / "config/m0r2_source_seal.json", m0r2_seal)
    ...
    write_json(
        run_dir / "RUN_STATE.json",
        {
            ...
            "run_id": RUN_ID,
            "state": "RUNNING",
            "note": "Approved M0R3 point-to-surface replay; zero data/training.",
        },
    )
    ...
    passed = bool(
        ...
        and summary.get("overall_status") == "PASS_CANO_100_PARENT_PERCEPTION_MESH_CONTRACT_M0R3"
    ...
    overall = (
        ...
        if passed
        else "FAIL_CANO_100_PARENT_PERCEPTION_MESH_CONTRACT_M0R3"
    )
    runner_summary = {
        "schema_version": "cano_100_parent_perception_mesh_contract_runner_summary_m0r3",
        "run_id": RUN_ID,
        ...
        "source_seals": {"m0r2": m0r2_seal, "v2r": v2r_seal, "v2": v2_seal},
        ...
        "claim_boundary": "Train-only native perception-surface M0R3; zero LiDAR, labels, formal data, training, models, trajectories and M-TARE changes.",
    }
