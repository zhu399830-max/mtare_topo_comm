...
from mtare_topo.governance_training_join_probe_v1 import SCHEMA,SLUG,POLICY,validate_card
...
def freeze():
    ...
    command=["env","CUDA_VISIBLE_DEVICES=","OMP_NUM_THREADS=1","OPENBLAS_NUM_THREADS=1","MKL_NUM_THREADS=1","PYTHONHASHSEED=20260906",PYTHON,
        "tools/v3/training_join_probe_v1.py","--spec",str(root/SPEC),"--run-dir",str(root/run)]
    ...
    spec=dict(schema_version="v3_run_spec_v1",gate=3,date="20260908",slug=SLUG,seed=20260906,operation="data_export",
        ...
        method="Read pinned cache/input/target bytes; verify original point coordinates and identity, construct deterministic patches and grid, and bind the loss-only context. No inference or training; explicit1m opening parameter is diagnostic only.",
...
def execute(spec,run):
    ...
    from mtare_topo.teacher.gse_training_join_probe_v1 import inspect_join
