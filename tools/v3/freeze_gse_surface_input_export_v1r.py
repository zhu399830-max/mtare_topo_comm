...
CARD_REL = "configs/v3/gate3/data_cards/gse_surface_input_export_v1r.json"
SPEC_REL = "configs/v3/gate3/gse_surface_input_export_v1r.json"
...
def freeze(root=PROJECT_ROOT):
    ...
    approval = {"status": "APPROVED", "approved_by": "user-standing-scope-authorization", "approved_at": "2026-09-07",
        ...
        "confirmation_reference": "User2026-09-07 PLEASE IMPLEMENT THIS PLAN:GSE-Graph surface relation execution, followed by 设置一个目标一直执行; continuation uses prior standing authority. Same exact input scope as V1. Its software compared pip freeze through the resolved base interpreter rather than the invoked virtualenv; predeclared independent implementation correction only, not a data/environment/method/threshold change or retry of the sealed failed run. No training authority."}
    card = {"schema_version": SCHEMA, "card_id": CARD_ID, "operation": "data_export",
        "purpose": "V1R software-only correction of virtualenv child invocation. Identical selected LiDAR/relative motion and six-field scope; V1 failed before data reads and is preserved.",
    ...
    sources = [CARD_REL, "docs/GSE_SURFACE_RELATION_EXECUTION_V1.md", "docs/GSE_SURFACE_ROBOT_TEACHER_STATUS_V1.md",
        ...
        "tools/v3/freeze_gse_surface_input_export_v1r.py", "tools/v3/run_gse_surface_input_export_v1r.py"]
    for path in sources:
    ...
    run_rel = "results/gate3_semantics/gate3_20260907_gse_surface_input_export_v1r_seed20260906"
    command = ["env", "CUDA_VISIBLE_DEVICES=", "OMP_NUM_THREADS=1", "OPENBLAS_NUM_THREADS=1", "MKL_NUM_THREADS=1",
        "PYTHONHASHSEED=20260906", SIDECAR, "tools/v3/run_gse_surface_input_export_v1r.py",
        "--spec", str(root / SPEC_REL), "--run-dir", str(root / run_rel)]
    spec = {"schema_version": "v3_run_spec_v1", "gate": 3, "date": "20260907", "slug": "gse_surface_input_export_v1r",
