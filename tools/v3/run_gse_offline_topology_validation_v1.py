#!/usr/bin/env python3
"""Execute and seal one immutable GSE validation-topology comparison."""
...
def main() -> int:
    ...
    if card.get("status") != "APPROVED_FOR_ONE_IMMUTABLE_GSE_OFFLINE_TOPOLOGY_VALIDATION":
    ...
    try:
        ...
        command = [
            str(PYTHON),
            str(PROJECT_ROOT / "tools/v3/evaluate_gse_offline_topology_v1.py"),
            "--training-run", str(TRAINING),
            "--perception-run", str(PERCEPTION),
    ...
    summary = {
        "schema_version": "gse_offline_topology_run_v1",
