...
RUN_ID = "gate2_20260822_aee_corrective_composite_v9_seed20260822"
TRAINER = PROJECT_ROOT / "tools/v3/train_aee_corrective_composite_v9.py"
STATUS_PASS = "PASS_AEE_CORRECTIVE_COMPOSITE_V9"
STATUS_FAIL = "FAIL_AEE_CORRECTIVE_COMPOSITE_V9"
...
def validate_seed_summary(summary: dict[str, Any], seed: int) -> None:
    if summary.get("status") != "COMPLETED_AEE_CORRECTIVE_COMPOSITE_SEED_V9":
        raise RuntimeError(f"V9 seed {seed} did not complete")
    if summary.get("seed") != seed or summary.get("epochs_completed") != 10:
        raise RuntimeError(f"V9 seed/epoch identity drift: {seed}")
    if summary.get("independent_train_samples_per_epoch") != {"cano": 5000, "aee": 1000}:
        raise RuntimeError(f"V9 independent sample drift: {seed}")
    if summary.get("training_views_per_epoch") != {"cano_dense": 5000, "cano_mask_matched": 5000, "aee": 1000}:
        raise RuntimeError(f"V9 view count drift: {seed}")
    if summary.get("count_effective_mass") != [1244, 6793, 1329, 504, 105, 25] or summary.get("role_effective_mass") != [6495, 2105, 1400]:
        raise RuntimeError(f"V9 effective-class-mass drift: {seed}")
    if summary.get("staged_update") != {"direction_epochs": 1, "final_encoder_embedding_count_role_epochs": 9, "steps_per_epoch": 47}:
        raise RuntimeError(f"V9 staged-update drift: {seed}")
    if summary.get("representation_contract") != "encoder_0_to_4_frozen_encoder_5_adapted_same_view_direction_retention":
        raise RuntimeError(f"V9 representation contract drift: {seed}")
    if summary.get("direction_retention") != "same_view_cano_logit_mse_weight_1":
        raise RuntimeError(f"V9 direction-retention contract drift: {seed}")
    if any(summary.get(key) != 0 for key in ("c09_frames_read", "c10_frames_read", "formal_benchmark_frames_read")):
        raise RuntimeError(f"V9 forbidden data read: {seed}")
    expected = {
        "sparse_direction_improvement_0p05", "sparse_direction_vs_b0", "sparse_empty_rate",
        "composite_sparse_count_1_to_4_macro_f1", "composite_sparse_role_macro_f1", "dense_direction_retention",
        "all_model_tensors_finite", "encoder_changed", "embedding_changed", "semantic_heads_changed",
        "early_encoder_identity", "last_encoder_block_changed", "direction_head_stage1_identity",
    }
    if not isinstance(summary.get("gate"), dict) or set(summary["gate"]) != expected:
        raise RuntimeError(f"V9 gate schema drift: {seed}")
...
def aggregate(summaries: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "sparse_count_1_to_4_macro_f1_by_seed": [item["composite_sparse_validation"]["count"]["macro_f1_count_1_to_4"] for item in summaries],
        "sparse_role_macro_f1_by_seed": [item["composite_sparse_validation"]["role"]["macro_f1_present"] for item in summaries],
    }
...
def main() -> int:
    ...
    try:
        ...
        if card.get("status") != "APPROVED_FOR_AEE_CORRECTIVE_COMPOSITE_V9":
        ...
        predecessor = (PROJECT_ROOT / spec["scientific_predecessor"]).resolve()
        predecessor.relative_to(PROJECT_ROOT)
        predecessor_entries = base.verify_seal(predecessor, spec["scientific_predecessor_seal_sha256"])
        predecessor_summary = load_json(predecessor / "metrics/summary.json")
        if predecessor_summary.get("overall_status") != spec["scientific_predecessor_status"] or predecessor_summary.get("completed_seeds") != 3:
            raise RuntimeError("V7 scientific predecessor drift")
        observability = (PROJECT_ROOT / spec["observability_evidence"]["run"]).resolve()
        observability.relative_to(PROJECT_ROOT)
        observability_entries = base.verify_seal(observability, spec["observability_evidence"]["seal_sha256"])
        observability_summary = load_json(observability / "metrics/summary.json")
        if observability_summary.get("overall_status") != "COMPLETED_AEE_COUNT_OBSERVABILITY_AUDIT_V1" or observability_summary.get("frames") != 5000:
            raise RuntimeError("count-observability evidence drift")
        ...
        write_json(run_dir / "config/input_integrity.json", {"source_dataset": {"run": str(source.relative_to(PROJECT_ROOT)), "seal_entries": seal_entries, "seal_sha256": spec["source_dataset_seal_sha256"]}, "scientific_predecessor": {"run": str(predecessor.relative_to(PROJECT_ROOT)), "seal_entries": predecessor_entries, "seal_sha256": spec["scientific_predecessor_seal_sha256"], "status": spec["scientific_predecessor_status"], "partial_reuse": False}, "observability_evidence": {"run": str(observability.relative_to(PROJECT_ROOT)), "seal_entries": observability_entries, "seal_sha256": spec["observability_evidence"]["seal_sha256"], "partial_reuse": False}, "checkpoints": checkpoints, "environment": environment, "python_executable": str(python), "live_pip_freeze_sha256": freeze["pip_freeze_sha256"], "pip_check": pip_evidence, "host": platform.platform()})
