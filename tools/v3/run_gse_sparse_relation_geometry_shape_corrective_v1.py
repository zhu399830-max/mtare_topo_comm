#!/usr/bin/env python3
"""Execute and seal one sparse token-geometry shape corrective."""
...
FAIL = "FAIL_GSE_SPARSE_RELATION_GEOMETRY_SHAPE_CORRECTIVE_V1"
CARD_STATUS = "APPROVED_FOR_ONE_IMMUTABLE_GSE_SPARSE_RELATION_GEOMETRY_SHAPE_CORRECTIVE_V1"
...
CARDINALITY = PROJECT_ROOT / "results/gate3_semantics/gate3_20260829_gse_sparse_relation_cardinality_corrective_v1_seed0"
...
def main() -> int:
    ...
    if run.name != run_id or load_json(run / "RUN_STATE.json").get("state") != "CREATED_NOT_EXECUTED":
        raise RuntimeError("geometry shape corrective executes exactly once")
    ...
    try:
        ...
        environment_variables["CUBLAS_WORKSPACE_CONFIG"] = ":4096:8"
        with (run / "logs/00_unit_tests.log").open("w", encoding="utf-8") as stream:
            ...
            ], cwd=PROJECT_ROOT, env=environment_variables, stdout=stream, stderr=subprocess.STDOUT, text=True, timeout=600, check=False)
        subprocesses.append({"stage": "unit_tests", "returncode": int(unit.returncode)})
        if unit.returncode != 0:
            raise RuntimeError("geometry shape unit tests failed")
        output = run / "metrics/corrective"
        command = [
            str(PYTHON), str(PROJECT_ROOT / "tools/v3/execute_gse_sparse_relation_geometry_shape_corrective_v1.py"),
            "--dataset-root", str(DATASET / "artifacts/dataset/train"),
            "--sequence-manifest", str(DATASET / "artifacts/sequence_manifest.jsonl"),
            "--readiness-summary", str(READINESS / "metrics/summary.json"),
            "--cardinality-summary", str(CARDINALITY / "metrics/summary.json"),
            "--output-dir", str(output),
        ]
        log = run / "logs/01_corrective.log"
        with log.open("w", encoding="utf-8") as stream:
            completed = subprocess.run(["/usr/bin/time", "-v", *command], cwd=PROJECT_ROOT, env=environment_variables, stdout=stream, stderr=subprocess.STDOUT, text=True, timeout=1200, check=False)
        ...
        if completed.returncode != 0:
            raise RuntimeError("geometry shape corrective subprocess failed")
        result = load_json(output / "summary.json")
        if result.get("status") != PASS or result.get("scientific_pass") is not True:
            raise RuntimeError("geometry shape corrective result contract failed")
        required = [output / "summary.json", output / "figure_source.json", output / "corrective_checks.csv", *[output / f"gse_sparse_relation_geometry_shape_corrective_v1.{suffix}" for suffix in ("png", "pdf", "svg")]]
        if any(not path.is_file() or path.stat().st_size == 0 for path in required):
            raise RuntimeError("geometry shape evidence incomplete")
        after = {relative: sha256(PROJECT_ROOT / relative) for relative in before}
        if before != after:
            raise RuntimeError("frozen geometry shape inputs changed")
        overall = PASS
    except Exception as exc:
        error = f"{type(exc).__name__}: {exc}"
        (run / "logs/failure_traceback.log").write_text(traceback.format_exc(), encoding="utf-8")
    write_json(run / "metrics/summary.json", {"schema_version": "gse_sparse_relation_geometry_shape_corrective_outer_v1", "overall_status": overall, "scientific_pass": overall == PASS, "error": error, "subprocesses": subprocesses, "peak_host_rss_kib": peak_rss_kib, "result": result, "duration_seconds": time.monotonic() - started, "source_unchanged": bool(before and before == after), "optimizer_steps": 0, "checkpoints_created": 0, "c07_worlds_read": 0, "c08_worlds_read": 0, "c09_worlds_read": 0, "c10_worlds_read": 0, "mtare_worlds_read": 0, "graph_replays": 0, "planner_calls": 0})
