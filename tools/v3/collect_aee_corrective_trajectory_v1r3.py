#!/usr/bin/env python3
"""V1R3 AEE collector: monotonic timestamp pairing plus ownership handoff."""
...
import collect_aee_corrective_trajectory_v1 as implementation


def handoff_tree(output: Path, host_uid: int, host_gid: int) -> dict[str, int | str]:
    root = output.resolve()
    try:
        root.relative_to(Path("/evidence/aee_trajectories"))
    except ValueError as exc:
        raise RuntimeError("ownership handoff target escapes /evidence/aee_trajectories") from exc
    ...
    # The archive consumer owns this stable interface.  V1R3 changes scan
    # pairing only; it must not version or rename the ownership handoff.
    handoff = {
        "schema_version": "aee_corrective_host_ownership_handoff_v1",
        "status": "PASS_AEE_CORRECTIVE_HOST_OWNERSHIP_HANDOFF_V1",
        "host_uid": host_uid,
        "host_gid": host_gid,
        "entries": len(paths) + 1,
    }
