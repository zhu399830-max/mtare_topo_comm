#!/usr/bin/env python3
"""Execute and seal the one-shot ERCSS feasibility audit."""
...
RUN_ID = "gate3_20260829_gse_registered_skeleton_feasibility_v1_seed0"
...
def verify_mesh_assets() -> int:
    ...
    for primary in sorted((MESH / "artifacts/meshes").glob("*_C0[1-8]/primary")):
        for name in ("mesh.obj", "graph.json", "splines.json", "geometry_parameters.json"):
            ...
            count += 1
    if count != 80 * 4:
        raise RuntimeError(f"expected 320 C01-C08 mesh documents, got {count}")
    return count
...
def main() -> int:
    ...
    if run_dir.name != RUN_ID or load_json(run_dir / "RUN_STATE.json").get("state") != "CREATED_NOT_EXECUTED":
        raise RuntimeError("ERCSS feasibility executes exactly once")
    started = time.monotonic(); overall = FAIL; error = None; result: dict = {}; returncode = None; peak_rss = None
    before: dict[str, str] = {}; after: dict[str, str] = {}; verified_mesh_files = 0
    try:
        if spec.get("gate") != 3 or spec.get("operation") != "audit" or spec.get("seed") != 0:
            raise RuntimeError("ERCSS formal scope mismatch")
        ...
        if not report.passed or card.get("status") != CARD_STATUS or card.get("approval", {}).get("status") != "APPROVED":
            raise RuntimeError("ERCSS Data Card mismatch")
        ...
        with (run_dir / "logs/00_unit_tests.log").open("w", encoding="utf-8") as stream:
            tests = subprocess.run([str(PYTHON), "-m", "pytest", "-q", "tests/v3/unit/test_gse_registered_structural_skeleton.py", "tests/v3/unit/test_gse_registered_structural_skeleton_teacher.py"], cwd=PROJECT_ROOT, env=env, stdout=stream, stderr=subprocess.STDOUT, text=True, timeout=300, check=False)
        if tests.returncode: raise RuntimeError("ERCSS unit tests failed")
        output = run_dir / "metrics/registered_skeleton_feasibility"
        command = ["/usr/bin/time", "-v", str(PYTHON), str(EVALUATOR), "--dataset", str(DATASET), "--supervision", str(SUPERVISION), "--mesh-root", str(MESH / "artifacts/meshes"), "--output-dir", str(output)]
    ...
    summary = {"schema_version": "gse_registered_skeleton_feasibility_outer_v1", "overall_status": overall, "scientific_pass": overall == PASS and error is None, "error": error, "returncode": returncode, "peak_host_rss_kib": peak_rss, "verified_mesh_files": verified_mesh_files, "source_unchanged": bool(before and before == after), "registered_skeleton_feasibility": result, "optimizer_steps": 0, "model_inference_frames": 0, "c09_worlds_read": 0, "c10_worlds_read": 0, "mtare_worlds_read": 0, "graph_replays": 0, "duration_seconds": time.monotonic() - started}
