#!/usr/bin/env python3
"""Execute and seal one immutable GSE event-node ensemble calibration."""
...
CARD_STATUS = "APPROVED_FOR_ONE_IMMUTABLE_GSE_EVENT_NODE_ENSEMBLE_CALIBRATION_V1"
...
NODE = PROJECT_ROOT / "results/gate4_topology/gate4_20260826_gse_node_matchability_ensemble_calibration_v1_seed0"
EVALUATOR = PROJECT_ROOT / "tools/v3/evaluate_gse_event_node_ensemble_v1.py"
...
def _sources() -> dict:
    return {
        ...
        "node": verify_complete_run_seal(
            PROJECT_ROOT, NODE, "PASS_GSE_NODE_MATCHABILITY_ENSEMBLE_CALIBRATION_V1"
        ),
    }
...
def main() -> int:
    ...
    if run_dir.name != RUN_ID or load_json(run_dir / "RUN_STATE.json").get("state") != "CREATED_NOT_EXECUTED":
        raise RuntimeError("one-time event-node calibration identity mismatch")
    ...
    try:
        if spec.get("gate") != 4 or spec.get("operation") != "threshold_calibration":
            raise RuntimeError("event-node calibration scope mismatch")
        if spec.get("user_authorization", {}).get("status") != "APPROVED":
            raise RuntimeError("event-node calibration is not authorized")
        if load_json(PROJECT_ROOT / spec["data_card"]).get("status") != CARD_STATUS:
            raise RuntimeError("event-node Data Card status mismatch")
        for name, record in spec["frozen_tools"].items():
            if _sha256(PROJECT_ROOT / record["path"]) != record["sha256"]:
                raise RuntimeError(f"frozen event-node tool drift: {name}")
        for relative, expected in spec["frozen_inputs"].items():
            if _sha256(PROJECT_ROOT / relative) != expected:
                raise RuntimeError(f"frozen event-node input drift: {relative}")
        before = _sources()
        ...
        command = [
            ...
            "--verifier-run", str(VERIFIER), "--node-run", str(NODE),
        ...
        if (
            ...
            or selected.get("precision", 0.0) < 0.98
            or selected.get("false_accept_rate", 1.0) > 0.01
            or selected.get("recall", 0.0) < 0.40
            or len(selected.get("per_family", {})) != 10
            or any(result.get(name) != 0 for name in (
                "optimizer_steps", "backbone_optimizer_steps", "model_inference_frames",
                "c09_worlds_read", "strict_test_worlds_read", "mtare_worlds_read",
            ))
        ):
            raise RuntimeError("event-node calibration failed its frozen contract")
        write_json(run_dir / "metrics/calibration_summary.json", {
            **result,
            "peak_host_rss_kib": peak_rss,
            "summary_sha256": _sha256(output / "summary.json"),
            "outputs_sha256": _sha256(output / "event_node_ensemble_selection_outputs.npz"),
        })
        overall = PASS_STATUS
    ...
    write_json(run_dir / "metrics/summary.json", {
        "schema_version": "gse_event_node_ensemble_calibration_outer_v1",
