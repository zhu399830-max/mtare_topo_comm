...
from mtare_topo.governance_anchor_diagnostic_v2 import SCHEMA,SLUG,POLICY,validate_card
...
def freeze():
    ...
    approval=dict(status="APPROVED",approved_by="user-standing-scope-authorization",approved_at="2026-09-07",
        ...
        scope="Fixed30C01observations150frames;source anchor/incident-path observation support only;no semantic labels or training",
        confirmation_reference="User explicitly authorized additional same-scope parser compatibility repair: 允许啊这种问题没必要问我; 以后记得自己推进选择能一直干下去的. Same30 source scope and scientific limits; no invented annotation.")
    ...
    command=["env","CUDA_VISIBLE_DEVICES=","OMP_NUM_THREADS=1","OPENBLAS_NUM_THREADS=1","MKL_NUM_THREADS=1","PYTHONHASHSEED=20260906",PYTHON,
        "tools/v3/surface_anchor_diagnostic_v2.py","--spec",str(root/SPEC),"--run-dir",str(root/run)]
    ...
    spec=dict(schema_version="v3_run_spec_v1",gate=3,date="20260907",slug=SLUG,seed=20260906,operation="data_export",
        ...
        question="Which local source junction references have all incident paths supported by the same causal observation grid?",
        method="Source-owned anchor-endpoint-axis probe paths,first10mROI exit,student-exact observation grid.25m;all incident axis support;no semantic label promotion.",
...
def execute(spec,run):
    ...
    from mtare_topo.teacher.gse_anchor_diagnostic_v2 import diagnose_observation
    ...
    try:
        ...
        with (run/"logs/observations.jsonl").open("x") as log:
            for row in card["scope"]["selected"]:
                ...
                entry=dict(task=active,local_anchors=output["local_anchor_count"],reference_junctions=output["reference_junctions"],supported_reference_junctions=output["supported_reference_junctions"])
        ...
        summary=dict(status="FAILED" if error else "ANCHOR_DIAGNOSTIC_COMPLETE",error=error,active_task=active,
