#!/usr/bin/env python3
"""Execute and seal structured exact-one event-loss readiness."""
...
def main() -> int:
    ...
    if run_dir.name != run_id or load_json(run_dir / "RUN_STATE.json").get("state") != "CREATED_NOT_EXECUTED":
        raise RuntimeError("exact-one readiness executes exactly once")
    ...
    try:
        ...
        if not validation.passed or card.get("status") != "APPROVED_FOR_ONE_IMMUTABLE_GSE_STRUCTURED_EXACT_ONE_EVENT_LOSS_READINESS_V1" or card.get("approval", {}).get("authorized_operations") != ["audit"]:
            raise RuntimeError(f"exact-one readiness Data Card invalid: {validation.errors}")
        ...
        if environment != {"python": "3.13.5", "numpy": "2.1.3", "torch": "2.9.0+cu129"}:
            raise RuntimeError(f"exact-one readiness environment drift: {environment}")
        ...
        output = run_dir / "metrics/readiness"
        command = [
            str(PYTHON), str(PROJECT_ROOT / "tools/v3/execute_gse_structured_exact_one_event_loss_readiness_v1.py"),
            "--cache-dir", str(ACTION / "scratch/action_set_cache"), "--output-dir", str(output),
        ]
        for seed in range(3):
            command.extend(("--observation", str(UNIFIED / f"artifacts/unified_observation/seed{seed}_unified_observation_features.npy")))
        ...
        with (run_dir / "logs/00_readiness.log").open("w", encoding="utf-8") as stream:
            completed = subprocess.run(command, cwd=PROJECT_ROOT, env=environment_process, stdout=stream, stderr=subprocess.STDOUT, text=True, timeout=300, check=False)
        returncode = int(completed.returncode)
        if returncode not in (0, 2):
            raise RuntimeError("exact-one readiness executor program failure")
        ...
        if result.get("status") != expected_status or bool(result.get("scientific_pass")) != (returncode == 0):
            raise RuntimeError("exact-one readiness status/return mismatch")
        required = [
            output / "summary.json", output / "figure_source.json",
            *[output / f"gse_structured_exact_one_event_loss_readiness_v1.{suffix}" for suffix in ("png", "pdf", "svg")],
        ]
        if any(not path.is_file() or path.stat().st_size == 0 for path in required):
            raise RuntimeError("exact-one readiness evidence incomplete")
        after = {relative: sha256(PROJECT_ROOT / relative) for relative in before}
        if before != after:
            raise RuntimeError("exact-one readiness changed a frozen source")
    ...
    write_json(run_dir / "metrics/summary.json", {
        "schema_version": "gse_structured_exact_one_event_loss_readiness_outer_v1",
        ...
        "optimizer_steps": 0, "trained_model_inference_frames": 0,
