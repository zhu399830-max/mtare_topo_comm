#!/usr/bin/env python3
"""Execute and seal the immutable replacement COUST readiness."""
...
CARD_STATUS = "APPROVED_FOR_ONE_IMMUTABLE_GSE_CYCLIC_ORDERED_UNIMODAL_SLOT_TRANSPORT_READINESS_V1R"
...
def main() -> int:
    ...
    if run.name != run_id or load_json(run / "RUN_STATE.json").get("state") != "CREATED_NOT_EXECUTED":
        raise RuntimeError("replacement readiness executes exactly once")
    ...
    try:
        ...
        if not validation.passed or card.get("status") != CARD_STATUS:
            raise RuntimeError(f"replacement Data Card invalid: {validation.errors}")
        ...
        environment = json.loads(subprocess.check_output([
            str(PYTHON), "-c",
            "import json,numpy,torch,zarr,sys;print(json.dumps({'python':sys.version.split()[0],'numpy':numpy.__version__,'torch':torch.__version__,'zarr':zarr.__version__},sort_keys=True))",
        ], text=True))
        ...
        command = [
            str(PYTHON), str(PROJECT_ROOT / "tools/v3/execute_gse_cyclic_ordered_unimodal_slot_transport_readiness_v1r.py"),
        ...
        with (run / "logs/00_readiness.log").open("w", encoding="utf-8") as stream:
            returncode = subprocess.run(command, cwd=PROJECT_ROOT, env=environment_variables, stdout=stream, stderr=subprocess.STDOUT, text=True, timeout=1200, check=False).returncode
        if returncode not in (0, 2):
            raise RuntimeError(f"replacement executor system failure: {returncode}")
        ...
        if (returncode == 0) != passed:
            raise RuntimeError("replacement status/return mismatch")
        ...
        if any(result.get(name) != 0 for name in forbidden):
            raise RuntimeError("replacement forbidden operation")
        required = [run / "artifacts/readiness" / name for name in (
            "summary.json", "figure_source.json", "selected_real_rows.csv",
            "gse_cyclic_ordered_unimodal_slot_transport_readiness_v1.png",
            "gse_cyclic_ordered_unimodal_slot_transport_readiness_v1.pdf",
            "gse_cyclic_ordered_unimodal_slot_transport_readiness_v1.svg",
        )]
        if any(not path.is_file() or path.stat().st_size == 0 for path in required):
            raise RuntimeError("replacement evidence incomplete")
        after = {relative: sha256(PROJECT_ROOT / relative) for relative in before}
        if before != after:
            raise RuntimeError("replacement source changed")
    ...
    write_json(run / "metrics/summary.json", {
        "schema_version": "gse_cyclic_ordered_unimodal_slot_transport_readiness_outer_v1r",
