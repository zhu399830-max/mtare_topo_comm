...
from mtare_topo.governance_bidirectional_relation_resume_v1 import SCHEMA,SLUG,POLICY,validate_card,lineage,PARENT
...
def freeze():
    ...
    command=['env','OMP_NUM_THREADS=1','OPENBLAS_NUM_THREADS=1','MKL_NUM_THREADS=1','PYTHONHASHSEED=0','CUBLAS_WORKSPACE_CONFIG=:4096:8',PYTHON,
             'tools/v3/bidirectional_relation_resume_v1.py','--spec',str(ROOT/SPEC),'--run-dir',str(ROOT/RUN)]
...
def execute(spec,run):
    ...
    try:
        ...
        if inventory!=load_json(ROOT/PARENT/'config/population.json'):raise ValueError('parent population/order mismatch')
        write(run/'config/population.json',inventory,'x')
        schedule=batch_schedule(len(fit))
        if [list(b) for b in schedule]!=load_json(ROOT/PARENT/'config/schedule.json'):raise ValueError('parent schedule drift')
