#!/usr/bin/env python3
"""Run and seal the zero-training structured-node capacity audit."""
...
EXECUTOR = PROJECT_ROOT / "tools/v3/execute_gse_route_conditioned_structured_node_capacity_v1.py"
...
PASS = "PASS_GSE_ROUTE_CONDITIONED_STRUCTURED_NODE_CAPACITY_V1"
FAIL = "FAIL_GSE_ROUTE_CONDITIONED_STRUCTURED_NODE_CAPACITY_V1"
...
def _verify(spec: dict) -> dict:
    ...
    for relative, expected in spec["frozen_inputs"].items():
        ...
        if actual != expected:
            raise RuntimeError(f"structured-node frozen input drift: {relative}")
...
def main() -> int:
    ...
    if run_dir.name != RUN_ID or load_json(run_dir / "RUN_STATE.json").get("state") != "CREATED_NOT_EXECUTED":
        raise RuntimeError("structured-node capacity may execute only once")
    ...
    try:
        if spec.get("gate") != 3 or spec.get("operation") != "audit":
            raise RuntimeError("structured-node audit scope drift")
        for record in spec["frozen_tools"].values():
            if _sha(PROJECT_ROOT / record["path"]) != record["sha256"]:
                raise RuntimeError(f"structured-node frozen tool drift: {record['path']}")
        ...
        command = [str(PYTHON), str(EXECUTOR), "--teacher", str(TEACHER), "--pair-cache", str(SOURCE / "artifacts/pair_cache/pairs.npz")]
        for seed in (0, 1, 2):
            command.extend((f"--seed{seed}", str(SOURCE / f"artifacts/models/seed{seed}/frozen_exit_token_outputs.npz")))
        command.extend(("--output-dir", str(run_dir / "metrics/capacity")))
        ...
        if returncode not in (0, 2):
            raise RuntimeError("structured-node executor failed as a program")
        result = load_json(run_dir / "metrics/capacity/summary.json")
        scientific_pass = result.get("status") == PASS
        if (returncode == 0) != scientific_pass:
            raise RuntimeError("structured-node status/return mismatch")
        source_after = _verify(spec)
        if source_before != source_after:
            raise RuntimeError("structured-node source changed during audit")
    ...
    summary = {"schema_version": "gse_route_conditioned_structured_node_capacity_outer_v1", "overall_status": overall, "scientific_pass": overall == PASS, "error": error, "duration_seconds": time.monotonic() - started, "executor_returncode": returncode, "capacity": result, "source_unchanged": bool(source_before and source_before == source_after), "optimizer_steps": 0, "model_inference_frames": 0, "c09_worlds_read": 0, "c10_worlds_read": 0, "mtare_worlds_read": 0}
