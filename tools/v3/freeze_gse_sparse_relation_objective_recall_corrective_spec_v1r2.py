...
RUN_ID = "gate3_20260829_gse_sparse_relation_objective_recall_corrective_v1r2_seed0"
SPEC = PROJECT_ROOT / "configs/v3/gate3/gse_sparse_relation_objective_recall_corrective_v1r2.json"
CARD = "configs/v3/gate3/data_cards/gse_sparse_relation_objective_recall_corrective_v1r2.json"
...
def main():
    if SPEC.exists():
        raise RuntimeError("objective-recall corrective V1R2 spec exists")
    ...
    inputs = [
        ...
        *[item for run in (failed_v2, failed_v2r, failed_v2r2, corrective_v1, corrective_v1r) for item in (f"{run}/RUN_STATE.json", f"{run}/metrics/summary.json", f"{run}/artifacts/evidence_sha256.txt")],
    ...
    tools = {
        ...
        "executor": "tools/v3/execute_gse_sparse_relation_objective_recall_corrective_v1r2.py",
        "runner": "tools/v3/run_gse_sparse_relation_objective_recall_corrective_v1r2.py",
        "freezer": "tools/v3/freeze_gse_sparse_relation_objective_recall_corrective_spec_v1r2.py",
    ...
    spec = {
        ...
        "execution_phase": 3,
        "date": "20260829",
        "slug": "gse_sparse_relation_objective_recall_corrective_v1r2",
        "seed": 0,
        "operation": "audit",
        "data_card": CARD,
        "question": "Does the final evaluator use both the complete objective Teacher denominator and the continuous deployed token bearing for localization and relation matching?",
        "method": "Read all C01-C08 causal Teacher histories once, preserve all V1R denominator proofs, and prove that localization and truth matching consume token_bearing_deg/2 with exact circular wrap while token_bin_index is retained only for proposal confidence.",
        ...
        "acceptance_criteria": [
            ...
            "Token, relation and structural-refusal FN, positives, recall and F1 use the objective denominator; localization uses continuous deployed bearing with 360-degree wrap; all unit tests pass.",
        ...
        "command": ["/usr/bin/timeout", "1800s", PYTHON, "tools/v3/run_gse_sparse_relation_objective_recall_corrective_v1r2.py", "--spec", str(SPEC), "--run-dir", str(PROJECT_ROOT / f"results/gate3_semantics/{RUN_ID}")],
    }
