#!/usr/bin/env python3
"""Freeze the exact formal offline-topology spec after perception PASS."""
...
PERCEPTION = PROJECT_ROOT / "results/gate3_semantics/gate3_20260824_gse_perception_validation_v1_seed0"
...
CARD = PROJECT_ROOT / "configs/v3/gate4/data_cards/gse_offline_topology_validation_v1.json"
OUTPUT = PROJECT_ROOT / "configs/v3/gate4/gse_offline_topology_validation_v1.json"
RUN_ID = "gate4_20260824_gse_offline_topology_validation_v1_seed0"
PYTHON = "/home/zeng-workstation/.local/share/mtare_topo_comm/envs/phase3_torch290_cu129_zarr2187_v1/bin/python"
SOURCE_STATUS = {
    TRAINING: "PASS_GSE_GRAPH_THREE_SEED_TRAINING_V1R",
    PERCEPTION: "PASS_GSE_PERCEPTION_VALIDATION_V1",
    DATASET: "PASS_GSE_DEDUPLICATED_DATASET_EXPORT_V1",
    TEACHER: "PASS_GSE_TEACHER_MANIFEST_V1",
}
FROZEN_TOOLS = {
    "data_card": "configs/v3/gate4/data_cards/gse_offline_topology_validation_v1.json",
    "runner": "tools/v3/run_gse_offline_topology_validation_v1.py",
    "evaluator": "tools/v3/evaluate_gse_offline_topology_v1.py",
    "paper_publisher": "tools/v3/publish_gse_offline_topology_figure_v1.py",
    "topology_example_publisher": "tools/v3/publish_gse_topology_examples_v1.py",
    "complete_evidence_publisher": "tools/v3/publish_gse_offline_topology_evidence_v1.py",
...
def freeze(output: Path = OUTPUT) -> dict:
    ...
    for run, expected in SOURCE_STATUS.items():
        ...
        state = load_json(required[0])
        summary = load_json(required[1])
        if (
            state.get("state") != "COMPLETED"
            or state.get("overall_status") != expected
            or summary.get("overall_status") != expected
            or not _zero_forbidden_reads(summary)
        ):
            raise RuntimeError(f"offline topology source is not completed PASS: {run.name}")
        source_files.extend(required)
    card = load_json(CARD)
    report = validate_data_card(card)
    if not report.passed or card.get("status") != "APPROVED_FOR_ONE_IMMUTABLE_GSE_OFFLINE_TOPOLOGY_VALIDATION":
    ...
    spec = {
        ...
        "execution_phase": 3,
        "date": "20260824",
        "slug": "gse_offline_topology_validation_v1",
        "operation": "topology_replay",
        "question": "Do frozen GSE semantic observations produce more accurate causal structural nodes and execution-verified edges than both exit-only and deterministic geometry rule graphs on all ten C09 validation parents?",
        "method": "Replay every directed traversal in deterministic Euler order for four fixed methods over the same 243 structural settings, select by the pre-registered validation rule, retain all sweeps and apply node/edge, association and graph-invariant gates.",
        ...
        "user_authorization": {
            ...
            "scope": "One immutable 10-world C09 offline topology validation after perception PASS, with fixed methods/grid and zero model updates, C10 or M-TARE reads.",
        ...
        "command": [
            ...
            "29400s",
            PYTHON,
            "tools/v3/run_gse_offline_topology_validation_v1.py",
            "--spec",
            str(output),
            "--run-dir",
