#!/usr/bin/env python3
"""Execute and seal the immutable frozen-token validity audit."""
...
TRANSPORT = PROJECT_ROOT / "results/gate3_semantics/gate3_20260828_gse_exit_action_transport_feasibility_v1_seed0"
V1 = PROJECT_ROOT / "results/gate3_semantics/gate3_20260829_gse_relational_commit_policy_feasibility_v1_seed0"
V2 = PROJECT_ROOT / "results/gate3_semantics/gate3_20260829_gse_structured_exact_one_event_training_v1_seed0"
...
def main() -> int:
    ...
    if run_dir.name != run_id or load_json(run_dir / "RUN_STATE.json").get("state") != "CREATED_NOT_EXECUTED":
        raise RuntimeError("token-validity audit executes exactly once")
    ...
    try:
        ...
        if (
            ...
            or card.get("status") != "APPROVED_FOR_ONE_IMMUTABLE_GSE_TOKEN_VALIDITY_ACTION_TRACK_FEASIBILITY_V1"
            or card.get("approval", {}).get("authorized_operations") != ["audit"]
            or card.get("approval", {}).get("authorized_gates") != [3]
        ):
            raise RuntimeError(f"token-validity Data Card invalid: {validation.errors}")
        ...
        if environment != expected:
            raise RuntimeError(f"token-validity environment drift: {environment}")
        ...
        command = [
            str(PYTHON), str(PROJECT_ROOT / "tools/v3/execute_gse_token_validity_action_track_feasibility_v1.py"),
            ...
            "--transport-summary", str(TRANSPORT / "metrics/summary.json"),
            "--v1-summary", str(V1 / "metrics/summary.json"),
            "--v2-summary", str(V2 / "metrics/selection/summary.json"),
        ...
        with (run_dir / "logs/00_token_validity_audit.log").open("w", encoding="utf-8") as stream:
            returncode = subprocess.run(command, cwd=PROJECT_ROOT, env=environment_variables, stdout=stream, stderr=subprocess.STDOUT, text=True, timeout=900, check=False).returncode
        if returncode not in (0, 2):
            raise RuntimeError(f"token-validity executor program failure: {returncode}")
        ...
        if (returncode == 0) != scientific_pass:
            raise RuntimeError("token-validity status/return mismatch")
        if result.get("population") != {
            ...
            "c07_observations": 21548, "c08_observations": 24394,
            "slots_per_seed": 1128756, "positive_tokens_per_seed": 396913,
            "negative_tokens_per_seed": 731843, "seed_archives": 3,
        }:
            raise RuntimeError("token-validity result population drift")
        if any(result.get(name) != 0 for name in (
            "optimizer_steps", "model_inference_frames", "threshold_selection_steps", "graph_replays",
            "c09_worlds_read", "c10_worlds_read", "mtare_worlds_read",
        )):
            raise RuntimeError("forbidden token-validity operation occurred")
        required = [
            run_dir / "artifacts/audit/summary.json",
            run_dir / "artifacts/audit/figure_source.json",
            run_dir / "artifacts/audit/token_validity_metrics.csv",
            *[run_dir / f"artifacts/audit/gse_token_validity_action_track_feasibility_v1.{suffix}" for suffix in ("png", "pdf", "svg")],
        ]
        if any(not path.is_file() or path.stat().st_size == 0 for path in required):
            raise RuntimeError("token-validity evidence incomplete")
        after = {relative: sha256(PROJECT_ROOT / relative) for relative in before}
        if before != after:
            raise RuntimeError("token-validity audit changed a frozen source")
    ...
    write_json(run_dir / "metrics/summary.json", {
        "schema_version": "gse_token_validity_action_track_feasibility_outer_v1",
