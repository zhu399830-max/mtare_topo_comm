#!/usr/bin/env python3
"""Train, evaluate and graph-qualify one immutable three-seed route-conditioned event residual."""
...
PASS="PASS_GSE_ROUTE_CONDITIONED_EVENT_RESIDUAL_TRAINING_V1";FAIL="FAIL_GSE_ROUTE_CONDITIONED_EVENT_RESIDUAL_TRAINING_V1";PYTHON=Path("/home/zeng-workstation/.local/share/mtare_topo_comm/envs/phase3_torch290_cu129_zarr2187_v1/bin/python")
def _sha(path):
 h=hashlib.sha256()
...
def _verify(spec):
 ...
 for rel,expected in spec["frozen_inputs"].items():
  ...
  if actual!=expected:raise RuntimeError(f"route-conditioned event residual input drift: {rel}")
  out[rel]=actual
 return out
...
def main():
 ...
 if run.name!=expected_run_id or load_json(run/"RUN_STATE.json").get("state")!="CREATED_NOT_EXECUTED":raise RuntimeError("route-conditioned event residual run may execute only once")
 start=time.monotonic();overall=FAIL;error=None;ensemble={};graph={};processes=[];before=after={}
 try:
  if spec.get("gate")!=3 or spec.get("operation")!="training" or spec.get("seed")!=0:raise RuntimeError("route-conditioned event residual scope drift")
  card=load_json(PROJECT_ROOT/spec["data_card"])
  if not str(card.get("status","")).startswith("APPROVED_FOR_ONE_IMMUTABLE_GSE_ROUTE_CONDITIONED_EVENT_RESIDUAL_TRAINING_"):raise RuntimeError("route-conditioned event residual Data Card drift")
  for rec in spec["frozen_tools"].values():
   if _sha(PROJECT_ROOT/rec["path"])!=rec["sha256"]:raise RuntimeError(f"route-conditioned event residual tool drift: {rec['path']}")
  ...
  if versions!=expected:raise RuntimeError(f"route-conditioned event residual environment drift: {versions}")
  ...
  after=_verify(spec)
  if before!=after or _verify_axis_subset(dataset)!=axis_entries:raise RuntimeError("route-conditioned event residual sources changed")
  required=[evaluation/"corrected_route_conditioned_ensemble.npz",evaluation/"gse_route_conditioned_event_residual.png",graph_out/"summary.json",graph_out/"gse_endpoint_geometry_capacity.png"]
  if any(not p.is_file() for p in required):raise RuntimeError("route-conditioned event residual evidence incomplete")
 ...
 graph_gates=locals().get("graph_gates",{});summary={"schema_version":"gse_route_conditioned_event_residual_training_outer_v1","overall_status":overall,"scientific_pass":overall==PASS,"error":error,"ensemble":ensemble,"graph":graph,"graph_gates":graph_gates,"processes":processes,"duration_seconds":time.monotonic()-start,"source_unchanged":bool(before and before==after),"optimizer_steps":600 if len([p for p in processes if p["stage"]=="training" and p["returncode"]==0])==3 else None,"trainable_parameters_per_seed":643,"base_model_optimizer_steps":0,"threshold_selection_steps":0,"c09_worlds_read":0,"c10_worlds_read":0,"mtare_worlds_read":0};write_json(run/"metrics/summary.json",summary);write_json(run/"RUN_STATE.json",{"schema_version":"v3_run_state_v1","run_id":expected_run_id,"state":"COMPLETED" if error is None else "FAILED","overall_status":overall,"error":error});entries=_seal(run);print(json.dumps({"overall_status":overall,"error":error,"seal_entries":entries},indent=2));return 0 if overall==PASS and error is None else 2
