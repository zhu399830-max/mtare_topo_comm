...
PASS = "PASS_GSE_SPARSE_RELATION_OBJECTIVE_RECALL_CORRECTIVE_V1R2"
FAIL = "FAIL_GSE_SPARSE_RELATION_OBJECTIVE_RECALL_CORRECTIVE_V1R2"
EXPECTED = {
...
def main() -> int:
    ...
    deployment = {
        ...
        "token_bearing_deg": np.asarray([[[1.0, 80.0, 160.0, 200.0, 240.0, 359.0]]]),
    }
    deployed_coordinate = evaluation._deployed_token_coordinates_bins(deployment)
    identity = np.full(180, -1, dtype=np.int64)
    identity[0] = 10
    deployed_mapping, deployed_distance = evaluation._match_prediction_to_truth(
        deployed_coordinate[0, 0], 1, identity
    )
    synthetic = {
        ...
        "deployed_bearing": {
            "hard_bin_distance": float(evaluation._circular_distance(2, 0)),
            "continuous_bin_coordinate": float(deployed_coordinate[0, 0, 0]),
            "continuous_distance": float(deployed_distance[0]),
            "matched_identity": int(deployed_mapping[0]),
            "wrap_distance_359deg_to_0": float(evaluation._circular_distance(deployed_coordinate[0, 0, 5], 0)),
        },
    }
    ...
    checks = {
        ...
        "continuous_deployed_bearing_drives_localization": deployed_mapping[0] == 10 and deployed_distance[0] == 0.5 and evaluation._circular_distance(2, 0) == 2,
        "continuous_deployed_bearing_wraps_at_360": evaluation._circular_distance(deployed_coordinate[0, 0, 5], 0) == 0.5,
        "zero_training_inference_or_forbidden_worlds": True,
    }
    checks = {key: bool(value) for key, value in checks.items()}
    passed = all(checks.values())
    summary = {
        "schema_version": "gse_sparse_relation_objective_recall_corrective_v1r2",
        "status": PASS if passed else FAIL,
        "scientific_pass": passed,
        "decision": "ALLOW_FINAL_V2R3_THREE_SEED_TRAINING" if passed else "STOP_BEFORE_TRAINING",
        "totals": totals,
    ...
    fig.suptitle("Sparse relation metric corrective V1R2: deployed heading and complete Teacher denominators")
    for suffix in ("png", "pdf", "svg"):
        fig.savefig(output / f"gse_sparse_relation_objective_recall_corrective_v1r2.{suffix}", dpi=220)
    plt.close(fig)
