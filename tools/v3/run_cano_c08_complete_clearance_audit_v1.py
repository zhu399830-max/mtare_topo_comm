#!/usr/bin/env python3
"""Run and seal the approved complete C08 clearance-only audit."""
...
RUN_ID = "gate4_20260813_cano_c08_complete_clearance_audit_v1_seed0"
E1 = Path("/tmp/mtare_cano_e1_zarr2187/bin/python")
EXECUTOR = PROJECT_ROOT / "tools/v3/execute_cano_c08_complete_clearance_audit_v1.py"
...
def main() -> int:
    ...
    if spec.get("gate") != 4 or spec.get("operation") != "topology_replay" or spec.get("clearance_audit_only") is not True:
    ...
    identity = json.loads(subprocess.check_output([
        str(E1), "-c", "import json,sys,numpy,open3d,matplotlib;print(json.dumps({'python':sys.version.split()[0],'numpy':numpy.__version__,'open3d':open3d.__version__,'matplotlib':matplotlib.__version__}))"
    ], text=True))
    ...
    (run_dir / "logs/01_complete_clearance_audit.log").write_text(
    ...
    passed = bool(
        completed.returncode == 0 and summary.get("overall_status") == "PASS_CANO_C08_COMPLETE_CLEARANCE_AUDIT_V1"
        and summary.get("frames_audited") == 4773 and summary.get("horizontal_rays") == 3436560
        ...
        and len(list((run_dir / "previews").glob("*.png"))) == 6
    )
    overall = "PASS_CANO_C08_COMPLETE_CLEARANCE_AUDIT_V1" if passed else "FAIL_CANO_C08_COMPLETE_CLEARANCE_AUDIT_V1"
    write_json(run_dir / "metrics/runner_summary.json", {
        "schema_version": "cano_c08_complete_clearance_audit_runner_v1", "overall_status": overall,
        "executor_exit_code": completed.returncode, "duration_seconds": duration,
        "frames_audited": summary.get("frames_audited", 0), "horizontal_rays": summary.get("horizontal_rays", 0),
        "failed_frames": summary.get("clearance_failed_frames", 0), "risk_segments": summary.get("risk_segment_count", 0),
        "claim_boundary": "Complete C08 horizontal-clearance diagnosis only; PASS does not imply every trajectory pose is eligible.",
    })
