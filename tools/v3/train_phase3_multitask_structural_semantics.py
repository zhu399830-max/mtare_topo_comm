#!/usr/bin/env python3
"""Phase-3 B1/M1 trainer. Formal use requires an approved Gate-2 runner/spec."""
...
def main():
    p=argparse.ArgumentParser();p.add_argument("--dataset-run",type=Path,required=True);p.add_argument("--output-dir",type=Path,required=True);p.add_argument("--mode",choices=("B1","M1"),required=True);p.add_argument("--seed",type=int,required=True);p.add_argument("--epochs",type=int,default=30);p.add_argument("--batch-size",type=int,default=128);p.add_argument("--learning-rate",type=float,default=3e-4);p.add_argument("--weight-decay",type=float,default=1e-4);p.add_argument("--patience",type=int,default=6);p.add_argument("--workers",type=int,default=0);a=p.parse_args()
    ...
    model=StructuralSemanticNet().to(device);optimizer=torch.optim.AdamW(model.parameters(),lr=a.learning_rate,weight_decay=a.weight_decay);history=[];best=-math.inf;stale=0;started=time.monotonic()
    for epoch in range(1,a.epochs+1):
        sampler.set_epoch(epoch)
        model.train();sums={k:0.0 for k in ("total","direction","count","role")};seen=0
        for batch in train_loader:
            student=batch["student"].to(device,non_blocking=True);direction=batch["direction_target"].to(device);count=batch["count_target"].to(device);role=batch["role_target"].to(device);optimizer.zero_grad(set_to_none=True);outputs=model(student);losses=multitask_loss(outputs,direction,count,role,role_weights)
            objective=losses["direction"] if a.mode=="B1" else losses["total"];objective.backward();optimizer.step();size=len(student);seen+=size
    ...
    checkpoint=torch.load(out/"best.pt",map_location=device,weights_only=False);model.load_state_dict(checkpoint["model"]);final=evaluate(model,validation_loader,device,role_weights,preserve_arrays=True);arrays=final.pop("arrays");np.savez_compressed(out/"validation_outputs.npz",**arrays);stability=stability_audit(model,validation_loader,device);(out/"best_validation_metrics.json").write_text(json.dumps(final,indent=2)+"\n");(out/"representation_stability.json").write_text(json.dumps(stability,indent=2)+"\n")
    summary={"status":"COMPLETED_PHASE3_TRAINING_EXECUTOR","mode":a.mode,"seed":a.seed,"epochs_completed":len(history),"best_epoch":int(checkpoint["epoch"]),"best_direction_validation_f1":best,"best_validation":final,"stability":stability,"parameters":sum(p.numel() for p in model.parameters()),"duration_seconds":time.monotonic()-started,"train_frames":len(train),"validation_frames":len(validation),"strict_test_frames_read":0,"mtare_frames_read":0}
