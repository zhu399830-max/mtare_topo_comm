...
"""Bind the composed frontier-attempt audit to the completed V3R source seal."""
...
from run_composed_frontier_attempt_audit_v1 import load_composed_source
from run_stochastic_v2_compatibility_audit_v1 import sha256


PROPOSAL_SPEC = Path("configs/v3/gate6/aee_composite_v9_composed_frontier_attempt_audit_v1.proposal.json")
PROPOSAL_CARD = Path("configs/v3/gate6/data_cards/aee_composite_v9_composed_frontier_attempt_audit_v1.proposal.json")
FINAL_SPEC = Path("configs/v3/gate6/aee_composite_v9_composed_frontier_attempt_audit_v1.json")
FINAL_CARD = Path("configs/v3/gate6/data_cards/aee_composite_v9_composed_frontier_attempt_audit_v1.json")
...
def finalize(
    *, proposal_spec_path: Path = PROPOSAL_SPEC,
    ...
    if final_spec_path.exists() or final_card_path.exists():
        raise RuntimeError("final composed-audit card/spec already exists")
    proposal, card_proposal = load_json(proposal_spec_path), load_json(proposal_card_path)
    if proposal.get("status") != "PENDING_FINAL_SOURCE_SEAL_NOT_EXECUTABLE":
        raise RuntimeError("composed audit spec is not a pending proposal")
    if card_proposal.get("status") != "PENDING_FINAL_SOURCE_SEAL_NOT_EXECUTABLE":
        raise RuntimeError("composed audit card is not a pending proposal")
    for name, item in proposal["frozen_tools"].items():
        if sha256(_within(root, item["path"])) != item["sha256"]:
            raise RuntimeError(f"composed audit proposal tool drift: {name}")
    source_seal_path = _within(root, proposal["source_run"]) / "artifacts/evidence_sha256.txt"
    if not source_seal_path.is_file():
        raise RuntimeError("composed audit recovery source has no final seal")
    ...
    if len(summaries) != 90 or source["manifest"].get("source_counts") != {
        ...
        raise RuntimeError("composed source did not prove exact 69+21 identity")
