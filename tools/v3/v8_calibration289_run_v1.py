...
from calibration_teacher_scope_v1 import compile_scope
from request_scoped_teacher_client_v1 import RequestScopedTeacherClient

base=original.base;ROOT=base.ROOT
SLUG='gse_v8_calibration289_v1';SCHEMA='v3_v8_calibration289_card_v1'
...
def freeze():
    ...
    a=dict(status='APPROVED',approved_by='user-standing-development-authorization',approved_at='2026-09-09',
        ...
        scope='Exact289 calibration observations,4 C07 parents,12 variant tasks; historical-route diagnostic, not fitting or threshold selection.',
        confirmation_reference='User standing autonomous permission for evidence-backed execution improvements; current PLAN permits binding289 calibration teacher inputs after scan/motion verification; no automatic training qualification; standing authorization covers scoped development execution after preflight. No old run overwrite, no training/test or threshold changes.')
    ...
    spec.update(slug=SLUG,data_card=CARD,config_path=CARD,user_authorization=a,
        question='Does unchanged V8 provide multi-position observed junction references on the fixed289 calibration observations?',
        ...
        baseline='Old27 V6 calibrated-route positives; other262 have no old labels. Compare source overlap and reference coverage, not model accuracy.',
        ...
        execution_correction=dict(index=1,only_override='request_object_lifetime',batching=False,explicit_gc=False,
            ...
            fresh_worker_confound=True,reused_input_observations=27))
    spec['command']=['env','MALLOC_ARENA_MAX=2','CUDA_VISIBLE_DEVICES=','OMP_NUM_THREADS=1','OPENBLAS_NUM_THREADS=1',
        'PYTHONHASHSEED=20260906',base.PYTHON,'tools/v3/v8_calibration289_run_v1.py','--spec',str(ROOT/SPEC),'--run-dir',str(ROOT/RUN)]
