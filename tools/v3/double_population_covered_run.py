"""Sixty fixed frames in one immutable synthetic software diagnostic, not a sensor dataset export."""
import argparse
...
def execute(spec, run):
    ...
    try:
        ...
        from mtare_topo.evaluation.double_frame_diagnostic import score_return
        ...
        if input_manifest() != load_json(ROOT/spec['config_path']):
            raise ValueError('complete input manifest drift')
        last_case = None
        with (run/'metrics/rays.jsonl').open('x') as stream, (run/'logs/progress.jsonl').open('x') as log:
            for case, origin, directions, expected, manifest in frames():
                if last_case != case['case_id']:
                    diagnostic = None
                    _, primitives = load_p1a_realized_construction(construction_document(case))
                    diagnostic = CoveredReferenceDiagnostic([
                        mesh_swept_superellipse(p, axial_spacing_m=.05, angular_segments=64)
                        for p in primitives])
                ...
                for index, direction in enumerate(directions):
                    ...
                    stream.write(json.dumps({'case_id': last_case, 'frame_index': manifest['frame_index'],
                        'ray_index': index, 'direction': direction.tolist(),
                        'expected_m': float(expected[index]), 'result': result, 'score': score},
                        allow_nan=False)+'\n')
                    stream.flush()
                    if checked % 1024 == 0:
                        progress = {'checked': checked, 'failures': failures, 'counts': dict(counts),
                                    'case_id': last_case, 'frame_index': manifest['frame_index'],
                        ...
                        if sum(p.stat().st_size for p in run.rglob('*') if p.is_file()) > 2*1024**3:
                            raise RuntimeError('2GiB evidence cap')
                    if not score['passed']:
                        raise RuntimeError('first diagnostic failure: '+last_case+
                            '/frame'+str(manifest['frame_index'])+'/ray'+str(index))
        verify()
    except Exception:
        ...
        signal.alarm(0)
    passed = error is None and checked == 691200 and failures == 0
    summary = {'state': 'DIAGNOSTIC_PASS' if passed else 'DIAGNOSTIC_FAIL',
               'checked_rays': checked, 'planned_rays': 691200, 'failures': failures,
