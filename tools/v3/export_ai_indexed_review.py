...
NAME='gse_ai_indexed_review_v1r1'
...
def execute():
    ...
    try:
        ...
        write(run/'config/execution_environment.json',dict(python=platform.python_version(),torch=torch.__version__,numpy=np.__version__,executable=sys.executable))
        ...
        with (run/'logs/export.jsonl').open('x') as log:
            for i,e in enumerate(card['scope']['entries']):
                ...
                expected=card['scope']['valid_returns'][i]
                if expected is not None and len(slots)!=expected:raise ValueError('valid count drift')
                if expected is None and not np.array_equal(valid,np.asarray(s.valid_mask,dtype=bool).reshape(5,11520)):raise ValueError('raw valid mask drift')
