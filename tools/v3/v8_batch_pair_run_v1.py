"""Single two-observation scheduling diagnostic; preserves the failed run."""
import argparse
import gzip
import json
...
SLUG='gse_v8_batch_pair_v1';SCHEMA='v3_v8_batch_pair_card_v1'
...
def freeze():
    ...
    a=dict(status='APPROVED',approved_by='user-standing-development-authorization',approved_at='2026-09-09',
        ...
        scope='Two exact fit observations192653/192654, one S10_C04 mixed traversal; scheduling diagnostic only.',
        confirmation_reference='User standing autonomous permission for evidence-backed execution improvements; current PLAN permits a bounded completed/failed pair comparison after synthetic equivalence. No retry of old141 run, no training/test or threshold changes.')
    ...
    spec.update(slug=SLUG,data_card=CARD,config_path=CARD,user_authorization=a,
        question='Can fixed1152-ray batches evaluate the failed location under the original cap while preserving complete preceding output?',
        method='Exact frozen V8; sole lateral entry call is scheduled in1152-ray chunks, original kernel unmodified. Same original mesh/field, rays, uncertainty and precision. Fresh two-request worker; process age is an explicit confound.',
        ...
        fallback='Fail and seal; no further scheduling retry, raising cap, dropped rays or training. Passing pair does not complete141.',
        acceptance_criteria=['Exactly source192653 then192654;6 original frames, one fit parent; no calibration/test.',
            ...
            'Second finishes under same cap; preserve all witnesses. No attribution solely to batching because worker is fresh.'],
        ...
        execution_correction=dict(index=1,only_override='gse_lateral_branch_evidence_v1.observed_operand_entries',batch_rays=1152,
    ...
    spec['command']=['env','MALLOC_ARENA_MAX=2','CUDA_VISIBLE_DEVICES=','OMP_NUM_THREADS=1','OPENBLAS_NUM_THREADS=1',
        'PYTHONHASHSEED=20260906',base.PYTHON,'tools/v3/v8_batch_pair_run_v1.py','--spec',str(ROOT/SPEC),'--run-dir',str(ROOT/RUN)]
...
def execute(spec,run):
    ...
    class ComparingClient(V8BatchedTeacherClient):
        completed=0
        def request(self,bundle):
            response=super().request(bundle)
            if self.completed==0:
                for k in ('raw_interfaces','produced_targets'):
                    if response[k]!=prior[k]:raise ValueError('batch changed preceding complete output: '+k)
