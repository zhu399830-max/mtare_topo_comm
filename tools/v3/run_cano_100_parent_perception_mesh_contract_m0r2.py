#!/usr/bin/env python3
"""Run and seal the approved Cano perception-mesh M0R2 exactly once."""
...
EXECUTOR = (
    PROJECT_ROOT
    / "tools/v3/execute_cano_100_parent_perception_mesh_contract_m0r2.py"
)
FAILED_M0R = PROJECT_ROOT / (
    "results/gate0_baseline/"
    "gate0_20260811_cano_100_parent_perception_mesh_contract_"
    "m0r_geometric_replay_seed0"
)
...
def _run_executor(run_dir: Path) -> tuple[int, float]:
    ...
    try:
        ...
        output += "\nTIMEOUT: frozen 3600 second M0R2 limit reached.\n"
    duration = time.monotonic() - started
    (run_dir / "logs/01_cano_100_parent_perception_mesh_contract_m0r2.log").write_text(
...
def main() -> int:
    ...
    if (
        ...
        raise RuntimeError("spec approval or unchanged M0R2 scope is incompatible")
    ...
    if (
        ...
        raise RuntimeError("approved M0R2 proposal/card is missing")
    ...
    if set(spec.get("frozen_tools", {})) != set(tool_paths):
        raise RuntimeError("frozen M0R2 tool set is not exact")
    ...
    failed_state = load_json(FAILED_M0R / "RUN_STATE.json")
    if (
        failed_state.get("state") != "FAILED"
        or failed_state.get("overall_status")
        != "FAIL_CANO_100_PARENT_PERCEPTION_MESH_CONTRACT_M0R"
    ):
        raise RuntimeError("M0R predecessor is not the sealed triangle-count FAIL")
    m0r_seal = _verify_seal(FAILED_M0R, 50)
    ...
    if shutil.disk_usage(PROJECT_ROOT).free < 2 * 1024**3:
        raise RuntimeError("less than 2 GiB free disk before M0R2")
    ...
    write_json(run_dir / "config/m0r_source_seal.json", m0r_seal)
    ...
    write_json(
        run_dir / "RUN_STATE.json",
        {
            ...
            "run_id": RUN_ID,
            "state": "RUNNING",
            "note": "Approved M0R2 single correction: bounded Poisson triangle discretization; zero data/training.",
        },
    )
    ...
    passed = bool(
        ...
        and summary.get("overall_status")
    ...
    overall = (
        "PASS_CANO_100_PARENT_PERCEPTION_MESH_CONTRACT_M0R2"
        if passed
        else "FAIL_CANO_100_PARENT_PERCEPTION_MESH_CONTRACT_M0R2"
    )
    runner_summary = {
        "schema_version": "cano_100_parent_perception_mesh_contract_runner_summary_m0r2",
        "run_id": RUN_ID,
        ...
        "source_seals": {"m0r": m0r_seal, "v2r": v2r_seal, "v2": v2_seal},
        ...
        "claim_boundary": "Train-only native perception-mesh M0R2; zero LiDAR, labels, formal data, training, models, trajectories and M-TARE changes.",
    }
