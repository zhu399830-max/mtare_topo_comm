#!/usr/bin/env python3
"""Single frozen source-interface diagnostic, no training or label promotion."""
import argparse
import hashlib
import json
...
from mtare_topo.governance_joint_partial_targets_v3 import SCHEMA,SLUG,POLICY,validate_card
...
def freeze():
    ...
    archive="results/gate3_semantics/gate3_20260907_gse_surface_junction_interfaces_v1_seed20260906"
    seal=archive+"/artifacts/evidence_sha256.txt"
    hashes={p:h for h,p in (line.split("  ",1) for line in (root/seal).read_text().splitlines())}
    scope["raw_interface_files"]={r["task"]:dict(path=archive+"/artifacts/"+r["view_id"]+".json",
        sha256=hashes[archive+"/artifacts/"+r["view_id"]+".json"]) for r in scope["selected"]}
    scope["raw_interface_seal"]=dict(path=seal,sha256=sha(root/seal))
    approval=dict(status="APPROVED",approved_by="user-standing-scope-authorization",approved_at="2026-09-07",
        ...
        scope="Fixed30C01 observations150frames; frozen original interface records; jointV3 partial anchor/opening/membership target diagnostic export; no qualified labels or training",
        confirmation_reference="User explicitly authorized automatic local teacher development: 别阻断了直接开始吧. Fixed original30 source scope; no fabricated human review or training.")
    ...
    command=["env","CUDA_VISIBLE_DEVICES=","OMP_NUM_THREADS=1","OPENBLAS_NUM_THREADS=1","MKL_NUM_THREADS=1","PYTHONHASHSEED=20260906",PYTHON,
        "tools/v3/joint_partial_targets_v3.py","--spec",str(root/SPEC),"--run-dir",str(root/run)]
    ...
    spec=dict(schema_version="v3_run_spec_v1",gate=3,date="20260907",slug=SLUG,seed=20260906,operation="data_export",
        ...
        question="Does the jointV3 producer yield usable partial anchor, opening and membership targets on original30 observations?",
        method="Reuse sealed original interfaces; jointV3 entering/interior junction evidence, terminal return evidence, independent window opening surface/crossing evidence; missing ray witnesses are unknown not veto; other-node evidence still vetoes; source-bound record digests. Partial automatic supervision only.",
        ...
        expected_evidence=["Source snapshot,command/environment,per-observation interface crossings,source frames,ownership rejections,raw log,summary,RUN_STATE,SHA256 seal;failure preserves active task."],
...
def execute(spec,run):
    ...
    from mtare_topo.teacher.gse_joint_reference_targets_v3 import produce_joint_reference_targets
    ...
    try:
        ...
        source_scope={k:v for k,v in card["scope"].items() if k not in ("raw_interface_files","raw_interface_seal")}
        pin=card["scope"]["raw_interface_seal"];read_pinned(root,pin["path"],pin["sha256"])
        reader=SurfaceTeacherReader(root,source_scope)
        with (run/"logs/observations.jsonl").open("x") as log:
            for row in card["scope"]["selected"]:
                ...
                pin=card["scope"]["raw_interface_files"][active]
                raw=json.loads(read_pinned(root,pin["path"],pin["sha256"]))
                reader.opened[pin["path"]]=pin["sha256"]
                output=produce_joint_reference_targets(bundle,raw)
                write(run/"artifacts"/(row["view_id"]+".json"),output)
                entry=dict(task=active,anchors=len(output["record"]["anchors"]),openings=len(output["record"]["openings"]),
                    known_memberships=sum(v is not None for row_ in output["record"]["membership"] for v in row_),
                    positive_memberships=sum(v is True for row_ in output["record"]["membership"] for v in row_),
                    target_record_sha256=output["target_record_sha256"])
        ...
        summary=dict(status="FAILED" if error else "JOINT_PARTIAL_TARGET_DIAGNOSTIC_COMPLETE",error=error,active_task=active,
