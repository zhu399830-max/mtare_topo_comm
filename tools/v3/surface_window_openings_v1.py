...
from mtare_topo.governance_window_openings_v1 import SCHEMA,SLUG,POLICY,validate_card
...
def freeze():
    ...
    approval=dict(status="APPROVED",approved_by="user-standing-scope-authorization",approved_at="2026-09-07",
        ...
        scope="Fixed30C01observations150frames;positive-only window opening proposals with same-source surface support; no qualified labels;no semantic labels or training",
    ...
    command=["env","CUDA_VISIBLE_DEVICES=","OMP_NUM_THREADS=1","OPENBLAS_NUM_THREADS=1","MKL_NUM_THREADS=1","PYTHONHASHSEED=20260906",PYTHON,
        "tools/v3/surface_window_openings_v1.py","--spec",str(root/SPEC),"--run-dir",str(root/run)]
    ...
    spec=dict(schema_version="v3_run_spec_v1",gate=3,date="20260907",slug=SLUG,seed=20260906,operation="data_export",
        ...
        question="Which fixed ROI cross sections have same-source outward crossings and partial surface support after global geometric competition?",
        method="All original source meshes .05m/64; polyline crossings of fixed10m sphere; finite outward rays plus at least one same-source contour cell; all geometric candidates compete regardless of source support; unknown dimensions/membership/anchors; candidates only, no qualified labels.",
...
def execute(spec,run):
    ...
    from mtare_topo.teacher.gse_window_opening_diagnostic_v1 import diagnose_observation
    ...
    try:
        ...
        with (run/"logs/observations.jsonl").open("x") as log:
            for row in card["scope"]["selected"]:
                ...
                entry=dict(task=active,candidate_count=output["candidate_count"],supported_count=output["supported_count"])
        ...
        summary=dict(status="FAILED" if error else "WINDOW_OPENING_DIAGNOSTIC_COMPLETE",error=error,active_task=active,
