#!/usr/bin/env python3
"""V9 ROS global-planner node with verified-backtrack graph re-anchoring."""
...
class SemanticTopologyGlobalNodeV4(base.SemanticTopologyGlobalNodeV3):
    """Reuse the frozen V3 sensor/model interface and replace only its runtime."""

    def __init__(self, **kwargs) -> None:
        original_runtime = base.OnlineTopologyPlannerRuntime
        base.OnlineTopologyPlannerRuntime = OnlineTopologyPlannerRuntimeV2
        try:
            super().__init__(**kwargs)
        finally:
            base.OnlineTopologyPlannerRuntime = original_runtime
        if not isinstance(self.runtime, OnlineTopologyPlannerRuntimeV2):
            raise RuntimeError("V4 failed to establish the verified-reanchor runtime")
...
def main() -> int:
    ...
    rospy.init_node("semantic_topology_global_node_v4", anonymous=False)
    SemanticTopologyGlobalNodeV4(
    ...
    rospy.loginfo("semantic topology v4 started in %s mode", "shadow" if args.shadow else "closed_loop")
    rospy.spin()
    return 0
