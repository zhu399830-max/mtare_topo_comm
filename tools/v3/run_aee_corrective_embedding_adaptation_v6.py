...
RUN_ID = "gate2_20260822_aee_corrective_embedding_adaptation_v6_seed20260822"
TRAINER = PROJECT_ROOT / "tools/v3/train_aee_corrective_embedding_adaptation_v6.py"
STATUS_PASS = "PASS_AEE_CORRECTIVE_EMBEDDING_ADAPTATION_V6"
STATUS_FAIL = "FAIL_AEE_CORRECTIVE_EMBEDDING_ADAPTATION_V6"
...
def validate_seed_summary(summary: dict[str, Any], seed: int) -> None:
    if summary.get("status") != "COMPLETED_AEE_CORRECTIVE_EMBEDDING_ADAPTATION_SEED_V6":
        raise RuntimeError(f"V6 seed {seed} did not complete")
    if summary.get("seed") != seed or summary.get("epochs_completed") != 10:
        raise RuntimeError(f"V6 seed/epoch identity drift: {seed}")
    if summary.get("independent_train_samples_per_epoch") != {"cano": 5000, "aee": 1000}:
        raise RuntimeError(f"V6 independent sample drift: {seed}")
    if summary.get("training_views_per_epoch") != {"cano_dense": 5000, "cano_mask_matched": 5000, "aee": 1000}:
        raise RuntimeError(f"V6 view count drift: {seed}")
    if summary.get("count_effective_mass") != [1244, 6793, 1329, 504, 105, 25] or summary.get("role_effective_mass") != [6495, 2105, 1400]:
        raise RuntimeError(f"V6 effective-class-mass drift: {seed}")
    if summary.get("staged_update") != {"direction_epochs": 1, "embedding_count_role_epochs": 10, "steps_per_epoch": 47}:
        raise RuntimeError(f"V6 staged-update drift: {seed}")
    if summary.get("representation_contract") != "encoder_frozen_embedding_adapted_direction_stage1_exact":
        raise RuntimeError(f"V6 representation contract drift: {seed}")
    if summary.get("direction_stage1_identity") is not True or summary.get("direction_stage1_sha256") != summary.get("final_direction_sha256"):
        raise RuntimeError(f"V6 direction stage-boundary identity failed: {seed}")
    if any(summary.get(key) != 0 for key in ("c09_frames_read", "c10_frames_read", "formal_benchmark_frames_read")):
        raise RuntimeError(f"V6 forbidden data read: {seed}")
    expected = {
    ...
    if not isinstance(summary.get("gate"), dict) or set(summary["gate"]) != expected:
        raise RuntimeError(f"V6 gate schema drift: {seed}")
...
def main() -> int:
    ...
    try:
        ...
        if card.get("status") != "APPROVED_FOR_AEE_CORRECTIVE_EMBEDDING_ADAPTATION_V6":
        ...
        if predecessor_summary.get("overall_status") != spec["scientific_predecessor_status"] or predecessor_summary.get("completed_seeds") != 3:
            raise RuntimeError("V5 scientific predecessor drift")
