...
from mtare_topo.governance_terminal_diagnostic_v1 import SCHEMA,SLUG,POLICY,validate_card
...
def freeze():
    ...
    approval=dict(status="APPROVED",approved_by="user-standing-scope-authorization",approved_at="2026-09-07",
        ...
        scope="Fixed30C01observations150frames;source terminal cap/first-return correspondence only;no semantic labels or training",
    ...
    command=["env","CUDA_VISIBLE_DEVICES=","OMP_NUM_THREADS=1","OPENBLAS_NUM_THREADS=1","MKL_NUM_THREADS=1","PYTHONHASHSEED=20260906",PYTHON,
        "tools/v3/surface_terminal_diagnostic_v1.py","--spec",str(root/SPEC),"--run-dir",str(root/run)]
    ...
    spec=dict(schema_version="v3_run_spec_v1",gate=3,date="20260907",slug=SLUG,seed=20260906,operation="data_export",
        ...
        question="Which original degree-one endpoint caps have finite first returns with unique matching saved source provenance within the fixed local ROI?",
        method="Source-owned degree-one endpoint cap triangles from original .05m/64 mesh; finite first-return correspondence and unique saved operand provenance; open10m anchor ROI; partial evidence only.",
        ...
        expected_evidence=["Source snapshot,command/environment,per-observation cap witnesses,source frames,ownership rejections,raw log,summary,RUN_STATE,SHA256 seal;failure preserves active task."],
    ...
    try:
        ...
        with (run/"logs/observations.jsonl").open("x") as log:
            for row in card["scope"]["selected"]:
                ...
                entry=dict(task=active,local_terminals=output["local_terminal_count"],observed_terminals=output["observed_terminal_count"])
        ...
        summary=dict(status="FAILED" if error else "TERMINAL_DIAGNOSTIC_COMPLETE",error=error,active_task=active,
