...
from mtare_topo.governance_surface_coverage_v1 import SCHEMA,SLUG,POLICY,validate_card
from mtare_topo.governance_surface_selection import digest
from mtare_topo.governance_surface_material import read_pinned
from mtare_topo.data.gse_surface_coverage_scope_v1 import compile_coverage_scope
...
def freeze():
    ...
    scope=compile_coverage_scope(root)
    approval=dict(status="APPROVED",approved_by="user-standing-scope-authorization",approved_at="2026-09-07",
        ...
        scope="Fixed3360C01-C07observations;construction and current XYZ spatial coverage only;no scans or labels or training",
        confirmation_reference="User explicitly authorized automatic local teacher development: 别阻断了直接开始吧. User approved fixed3360 C01-C07 development population and autonomous in-scope diagnostics; precise metadata-only expansion for coverage, no sample reselection or labels.")
    ...
    command=["env","CUDA_VISIBLE_DEVICES=","OMP_NUM_THREADS=1","OPENBLAS_NUM_THREADS=1","MKL_NUM_THREADS=1","PYTHONHASHSEED=20260906",PYTHON,
        "tools/v3/surface_coverage_v1.py","--spec",str(root/SPEC),"--run-dir",str(root/run)]
    ...
    spec=dict(schema_version="v3_run_spec_v1",gate=3,date="20260907",slug=SLUG,seed=20260906,operation="data_export",
        ...
        question="Do fixed3360 development observations spatially cover terminals and junctions across independent parent maps?",
        method="Original construction anchors and selected current float64 XYZ; open10m spatial candidates; parent-node deduplication across variants; no visibility or semantic labels.",
        baseline="Fixed selection and full source construction inventory; no detector comparison, nearby is not visible.",
        ...
        wall_time_cap_s=1800,estimated_cost=dict(compute="CPU only;3360 selected current poses,210 constructions; no scans",host_ram_gb=4,gpu_vram_gb=0,disk_gb=.5,wall_time_hours=.5),
        acceptance_criteria=["Exact fixed3360 selection,source hashes and current-pose rows; no C08-C10, scans, membership, labels or training.",
        ...
        expected_evidence=["Source snapshot,command/environment,per-observation nearby source anchors,per-split parent-deduplicated counts,raw log,summary,RUN_STATE,SHA256 seal;failure preserves active task."],
...
def execute(spec,run):
    from mtare_topo.data.gse_surface_coverage_v1 import CoverageReader, aggregate_coverage
    ...
    try:
        ...
        reader=CoverageReader(root,card["scope"])
        with (run/"logs/observations.jsonl").open("x") as log:
            for row in card["scope"]["entries"]:
                ...
                output=reader.read_task(active)
                write(run/"artifacts"/(active+".json"),output)
                entry=output
        ...
        summary=dict(status="FAILED" if error else "SPATIAL_COVERAGE_COMPLETE",error=error,active_task=active,
            completed_tasks=len(entries),coverage=aggregate_coverage(entries),elapsed_s=time.monotonic()-started,
