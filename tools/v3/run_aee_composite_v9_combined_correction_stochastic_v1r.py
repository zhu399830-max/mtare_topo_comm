...
"""Retry the exact V5 30-case matrix after a pre-case source-interface fix."""
...
RUN_ID = "gate6_20260823_aee_composite_v9_combined_correction_stochastic_v1r_seed20260820"
STATUS_PASS = "PASS_AEE_COMPOSITE_V9_COMBINED_CORRECTION_STOCHASTIC_V1R"
STATUS_FAIL = "FAIL_AEE_COMPOSITE_V9_COMBINED_CORRECTION_STOCHASTIC_V1R"
FAILED_STATUS = "FAIL_AEE_COMPOSITE_V9_COMBINED_CORRECTION_STOCHASTIC_V1"
...
def verify_failed_source(spec: dict, project_root: Path = base.base.PROJECT_ROOT) -> dict:
    ...
    if (
        ...
        or summary.get("failure_reason") != "'source_schedule_file_sha256'"
    ):
        raise RuntimeError("V1R source is not the exact zero-case interface failure")
    ...
    for line in seal_path.read_text(encoding="utf-8").splitlines():
        ...
        entries += 1
    return {
        "schema_version": "aee_combined_stochastic_failed_source_v1",
        "run": spec["failed_stochastic_run"],
        "status": FAILED_STATUS,
        "seal_sha256": spec["failed_stochastic_seal_sha256"],
...
def main() -> int:
    ...
    write_json(
        known.run_dir.resolve() / "config/failed_stochastic_source.json",
        verify_failed_source(spec),
    )
    base.RUN_ID = RUN_ID
