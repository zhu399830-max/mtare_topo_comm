...
from mtare_topo.governance_surface_features_v1 import SCHEMA, SLUG, POLICY, validate_card
from mtare_topo.governance_surface_selection import digest
from mtare_topo.governance_surface_material import read_pinned
from mtare_topo.data.gse_surface_feature_scope_v1 import compile_feature_scope
...
def freeze():
    ...
    approval = dict(status="APPROVED", approved_by="user-standing-scope-authorization", approved_at="2026-09-07",
        ...
        confirmation_reference="User approved GSE surface plan: 5090 public encoder extraction on fixed C01-C07 population; 设置一个目标一直执行; no routine reapproval.",
        scope="Exact3360 fiveframe inputs; same historical seed0epoch2 frozen encoder; no labels,selection,training,C08-C10 or graphs")
    ...
    run = "results/gate3_semantics/gate3_20260907_" + SLUG + "_seed0"
    command = ["env", "OMP_NUM_THREADS=1", "OPENBLAS_NUM_THREADS=1", "MKL_NUM_THREADS=1", "PYTHONHASHSEED=0",
               PYTHON, "tools/v3/surface_features_v1.py", "--spec", str(root / SPEC), "--run-dir", str(root / run)]
    spec = dict(schema_version="v3_run_spec_v1", gate=3, date="20260907", slug=SLUG, seed=0, operation="data_export",
        ...
        wall_time_cap_s=1800, estimated_cost=dict(compute="5090D inference only3360windows", host_ram_gb=32, gpu_vram_gb=28, disk_gb=8, wall_time_hours=.5),
        acceptance_criteria=["All3360 exact source windows exported once; no source IDs or labels in encoder input.",
        ...
        expected_evidence=["3360compactcaches,source/weight hashes,per-observation provenance,environment,logs,summary,RUN_STATE,seal"],
...
def execute(spec, run):
    ...
    try:
        ...
        with (run / "logs/observations.jsonl").open("x") as log:
            for task in scope["tasks"]:
                ...
                for row in task["observations"]:
                    observation = bind_feature_input(raw, expected_sha256=task["input_sha256"], task=task["task"],
                        row=row["input_row"], source_sequence_id=row["source_sequence_id"], frame_rows=row["frame_rows"])
            ...
            if len(entries) != 3360 or module_state_sha256(bound.backbone) != bound.state_sha256:
