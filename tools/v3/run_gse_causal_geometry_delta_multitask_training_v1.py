#!/usr/bin/env python3
"""Execute and seal one immutable three-seed geometry-delta multitask training."""
...
CARD_STATUS = "APPROVED_FOR_ONE_IMMUTABLE_GSE_CAUSAL_GEOMETRY_DELTA_MULTITASK_TRAINING_V1"
PYTHON = Path(
    "/home/zeng-workstation/.local/share/mtare_topo_comm/"
    "envs/phase3_torch290_cu129_zarr2187_v1/bin/python"
)
TRAINER = PROJECT_ROOT / "tools/v3/train_gse_causal_geometry_delta_multitask_v1.py"
...
def main() -> int:
    ...
    if run_dir.name != RUN_ID or load_json(run_dir / "RUN_STATE.json").get("state") != "CREATED_NOT_EXECUTED":
        raise RuntimeError("geometry-delta multitask one-shot state mismatch")
    ...
    try:
        if spec.get("gate") != 3 or spec.get("operation") != "training" or spec.get("seed") != 0:
            raise RuntimeError("geometry-delta multitask scope mismatch")
        ...
        if card.get("status") != CARD_STATUS or card.get("approval", {}).get("status") != "APPROVED":
            raise RuntimeError("geometry-delta multitask Data Card mismatch")
        for name, record in spec["frozen_tools"].items():
            if _sha256(PROJECT_ROOT / record["path"]) != record["sha256"]:
                raise RuntimeError(f"frozen geometry-delta multitask tool drift: {name}")
        for relative, expected in spec["frozen_inputs"].items():
            if _sha256(PROJECT_ROOT / relative) != expected:
                raise RuntimeError(f"frozen geometry-delta multitask input drift: {relative}")
        before = _sources()
        ...
        if environment.get("cuda_available") is not True:
            raise RuntimeError("geometry-delta multitask training requires frozen CUDA sidecar")
        ...
        if (
            ...
            or any(result.get(name) != 0 for name in (
                "backbone_optimizer_steps", "c09_worlds_read", "strict_test_worlds_read", "mtare_worlds_read"
            ))
        ):
            raise RuntimeError("geometry-delta multitask execution/evidence contract failed")
    ...
    write_json(
        ...
        {
            "schema_version": "gse_causal_geometry_delta_multitask_training_outer_v1",
