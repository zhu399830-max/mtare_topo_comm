...
def main() -> int:
    ...
    if run_dir.name != run_id or load_json(run_dir / "RUN_STATE.json").get("state") != "CREATED_NOT_EXECUTED":
        raise RuntimeError("geometry-anchored readiness V2 executes exactly once")
    ...
    try:
        ...
        write_json(run_dir / "config/environment.json", {"executable": str(PYTHON), "torch": __import__("torch").__version__, "gpu_used": False})
        ...
        command = [str(PYTHON), str(PROJECT_ROOT / "tools/v3/execute_gse_geometry_anchored_spatial_event_readiness_v2.py"), "--teacher-root", str(teacher), "--source-root", str(source), "--output-dir", str(run_dir / "artifacts/audit")]
        ...
        with (run_dir / "logs/audit.log").open("w", encoding="utf-8") as stream:
            code = subprocess.run(command, cwd=PROJECT_ROOT, env=environment, stdout=stream, stderr=subprocess.STDOUT, text=True, timeout=1800, check=False).returncode
        if code not in (0, 2):
            raise RuntimeError(f"readiness V2 executor system failure: {code}")
        ...
        if (
            ...
            or result.get("population", {}).get("tokens") != 131424
            ...
            raise RuntimeError("readiness V2 result contract drift")
        after = {relative: sha256(PROJECT_ROOT / relative) for relative in before}
        if before != after:
            raise RuntimeError("readiness V2 source changed")
        overall = str(result["status"])
    except Exception as exc:
        error = f"{type(exc).__name__}: {exc}"
        (run_dir / "logs/failure_traceback.log").write_text(traceback.format_exc(), encoding="utf-8")
    write_json(run_dir / "metrics/summary.json", {"schema_version": "gse_geometry_anchored_spatial_event_readiness_outer_v2", "overall_status": overall, "error": error, "result": result, "duration_seconds": time.monotonic() - started, "source_unchanged": bool(before and before == after)})
