#!/usr/bin/env python3
"""Execute and seal one immutable causal geometry-delta observability audit."""
...
CARD_STATUS = "APPROVED_FOR_ONE_IMMUTABLE_GSE_CAUSAL_GEOMETRY_DELTA_OBSERVABILITY_V1"
PYTHON = Path(
    "/home/zeng-workstation/.local/share/mtare_topo_comm/"
    "envs/phase3_torch290_cu129_zarr2187_v1/bin/python"
)
EXECUTOR = PROJECT_ROOT / "tools/v3/execute_gse_causal_geometry_delta_observability_v1.py"
...
VERIFIER = PROJECT_ROOT / (
    ...
    "gate3_20260826_gse_exit_token_association_corrective_v2_seed0"
)
PREDECESSOR = PROJECT_ROOT / (
    "results/gate3_semantics/"
    "gate3_20260827_gse_corrected_causal_event_training_v1_seed0"
)
...
def main() -> int:
    ...
    if run_dir.name != RUN_ID:
        raise RuntimeError("geometry-delta audit run identity mismatch")
    state = load_json(run_dir / "RUN_STATE.json")
    if state.get("state") != "CREATED_NOT_EXECUTED":
        raise RuntimeError("geometry-delta audit may execute only once")
    ...
    try:
        if spec.get("gate") != 3 or spec.get("operation") != "audit" or spec.get("seed") != 0:
            raise RuntimeError("geometry-delta audit scope mismatch")
        ...
        if card.get("status") != CARD_STATUS or card.get("approval", {}).get("status") != "APPROVED":
            raise RuntimeError("geometry-delta audit Data Card mismatch")
        for name, record in spec["frozen_tools"].items():
            if _sha256(PROJECT_ROOT / record["path"]) != record["sha256"]:
                raise RuntimeError(f"frozen geometry-delta tool drift: {name}")
        for relative, expected in spec["frozen_inputs"].items():
            if _sha256(PROJECT_ROOT / relative) != expected:
                raise RuntimeError(f"frozen geometry-delta input drift: {relative}")
        ...
        if environment != {"python": "3.13.5", "numpy": "2.1.3", "matplotlib": "3.10.0"}:
            raise RuntimeError(f"geometry-delta sidecar drift: {environment}")
        write_json(
            run_dir / "config/environment.json",
            {"executable": str(PYTHON), "versions": environment, "cpu_only": True},
        )
        write_json(
            run_dir / "config/source_integrity_before.json",
            before,
        )
        write_json(
            run_dir / "RUN_STATE.json",
            {
                ...
                "run_id": RUN_ID,
                "state": "RUNNING",
                "note": "C01-C08 read-only causal geometry-delta observability audit.",
            },
        )
        command = [
            "/usr/bin/time",
            ...
            "--pair-cache",
            str(VERIFIER / "artifacts/pair_cache/pairs.npz"),
        ]
        for seed in (0, 1, 2):
            command.extend(
                [
                    "--feature",
                    str(VERIFIER / f"artifacts/models/seed{seed}/frozen_observation_features.npy"),
                ]
            )
        ...
        log = run_dir / "logs/00_observability.log"
        ...
        result_path = run_dir / "metrics/observability_summary.json"
        result = load_json(result_path) if result_path.is_file() else {}
        after = _sources()
        output_bytes = sum(path.stat().st_size for path in run_dir.rglob("*") if path.is_file())
        required = [
            run_dir / "artifacts/auc_summary.csv",
            run_dir / "artifacts/delta_mae.csv",
            run_dir / "artifacts/fit_threshold_selection_transfer.csv",
            run_dir / "previews/gse_causal_geometry_delta_observability.png",
            run_dir / "previews/gse_causal_geometry_delta_observability.pdf",
            run_dir / "previews/gse_causal_geometry_delta_observability.svg",
            run_dir / "previews/gse_causal_geometry_delta_observability_source.json",
        ]
        if (
            before != after
            ...
            raise RuntimeError("geometry-delta audit violated its execution/evidence contract")
    ...
    write_json(
        ...
        {
            "schema_version": "gse_causal_geometry_delta_observability_outer_v1",
