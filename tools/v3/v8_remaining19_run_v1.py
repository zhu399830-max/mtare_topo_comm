"""Complete only the sealed141 population complement with unchanged V8."""
import argparse
import gzip
import json
from pathlib import Path
import v8_multiview_run_v1 as original
from v8_remaining_scope_v1 import compile_scope
from request_scoped_teacher_client_v1 import RequestScopedTeacherClient

base=original.base;ROOT=base.ROOT
SLUG='gse_v8_remaining19_v1';SCHEMA='v3_v8_remaining19_card_v1'
...
def freeze():
    ...
    a=dict(status='APPROVED',approved_by='user-standing-development-authorization',approved_at='2026-09-09',
        ...
        scope='Exact19 missing fit observations in S10_C04 across three variants; no completed122 recomputation.',
        confirmation_reference='User standing autonomous permission for evidence-backed execution improvements; current PLAN permits exact remaining19 completion after sealed122 reuse audit and successful lifetime pair; standing authorization covers scoped development execution after preflight. No retry of old141 run, no training/test or threshold changes.')
    ...
    spec.update(slug=SLUG,data_card=CARD,config_path=CARD,user_authorization=a,
        question='Can the exact19 missing observations complete the original141 mechanism population with unchanged V8 and bounded request lifetimes?',
        ...
        baseline='Sealed original121 plus lifetime-pair new1; prior duplicate192653 full outputs equal. No missing-item output exists for direct equality.',
        fallback='Fail and seal; no further lifetime retry, raising cap, dropped rays or training. Completion is not training qualification; all141 supervision must be evaluated separately.',
        acceptance_criteria=['Exactly the19 uncompleted original observations,31 variant frames, one fit parent; no calibration/test.',
            'Keep all original sources in order, same frozen archive, original geometry and unknown masks.',
            'All19 finish under the original cap, no dropped observations or retries; qualify full141 separately.'],
        expected_evidence=['19 full outputs, source hashes, logs, runtime, memory, RUN_STATE, seal'],
        environment=base.environment(),teacher_environment=base.old_environment(),
        estimated_cost=dict(compute='CPU only19 remaining diagnostic observations; no sensor rerendering/model/training',host_ram_gb=4,gpu_vram_gb=0,disk_gb=30,wall_time_hours=3),
        execution_correction=dict(index=1,only_override='request_object_lifetime',batching=False,explicit_gc=False,
            ...
            fresh_worker_confound=True,reused_completed_observations=122))
    spec['command']=['env','MALLOC_ARENA_MAX=2','CUDA_VISIBLE_DEVICES=','OMP_NUM_THREADS=1','OPENBLAS_NUM_THREADS=1',
        'PYTHONHASHSEED=20260906',base.PYTHON,'tools/v3/v8_remaining19_run_v1.py','--spec',str(ROOT/SPEC),'--run-dir',str(ROOT/RUN)]
