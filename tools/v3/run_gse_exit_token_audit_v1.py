#!/usr/bin/env python3
"""Execute once and seal the 90-world GSE incident-exit audit."""
...
RUN_ID = "gate2_20260824_gse_exit_token_audit_v1_seed0"
PASS_STATUS = "PASS_GSE_EXIT_TOKEN_AUDIT_V1"
SIDECAR = Path("/tmp/mtare_gate4_meshing_sidecar_v1/bin/python")
EXECUTOR = PROJECT_ROOT / "tools/v3/execute_gse_exit_token_audit_v1.py"
...
def _verify_manifest() -> dict:
    state = load_json(MANIFEST_RUN / "RUN_STATE.json")
    summary = load_json(MANIFEST_RUN / "metrics/summary.json")
    seal = MANIFEST_RUN / "artifacts/evidence_sha256.txt"
    if (
        state.get("state") != "COMPLETED"
        or state.get("overall_status") != "PASS_GSE_TEACHER_MANIFEST_V1"
        or summary.get("total_observation_count") != 212588
    ):
        raise RuntimeError("upstream teacher manifest is not the sealed PASS input")
    checked = 0
    for line in seal.read_text(encoding="utf-8").splitlines():
        expected, relative = line.split("  ", 1)
        if _sha256(PROJECT_ROOT / relative) != expected:
            raise RuntimeError(f"upstream teacher manifest seal mismatch: {relative}")
        checked += 1
    return {
        "run": str(MANIFEST_RUN.relative_to(PROJECT_ROOT)),
        "seal_sha256": _sha256(seal),
        "summary_sha256": _sha256(MANIFEST_RUN / "metrics/summary.json"),
...
def main() -> int:
    ...
    if spec.get("gate") != 2 or spec.get("operation") != "audit" or spec.get("seed") != 0:
        raise RuntimeError("exit-token audit scope mismatch")
    if spec.get("user_authorization", {}).get("status") != "APPROVED":
        raise RuntimeError("exit-token audit is not authorized")
    ...
    write_json(
        run_dir / "RUN_STATE.json",
        {
            ...
            "run_id": RUN_ID,
            "state": "RUNNING",
            "note": "Incident-only development exit-token audit; zero sensor export or training.",
        },
    )
    command = [
        str(SIDECAR),
    ...
    (run_dir / "logs/01_exit_token_audit.log").write_text(
    ...
    required = (
        run_dir / "artifacts/exit_token_audit.jsonl",
    ...
    passed = bool(
        ...
        and summary.get("totals", {}).get("candidate_count") == 448338
        and summary.get("totals", {}).get("zero_visible_node_event_count") == 0
    ...
    overall = PASS_STATUS if passed else "FAIL_GSE_EXIT_TOKEN_AUDIT_V1"
    write_json(
