...
from mtare_topo.governance_joint_partial_targets_v1 import SCHEMA,SLUG,POLICY,validate_card
...
def freeze():
    ...
    approval=dict(status="APPROVED",approved_by="user-standing-scope-authorization",approved_at="2026-09-07",
        ...
        scope="Fixed30C01 observations150frames; frozen original interface records; jointV2 partial anchor/opening/membership target diagnostic export; no qualified labels or training",
    ...
    command=["env","CUDA_VISIBLE_DEVICES=","OMP_NUM_THREADS=1","OPENBLAS_NUM_THREADS=1","MKL_NUM_THREADS=1","PYTHONHASHSEED=20260906",PYTHON,
        "tools/v3/joint_partial_targets_v1.py","--spec",str(root/SPEC),"--run-dir",str(root/run)]
    ...
    spec=dict(schema_version="v3_run_spec_v1",gate=3,date="20260907",slug=SLUG,seed=20260906,operation="data_export",
        ...
        question="Does the jointV2 producer yield usable partial anchor, opening and membership targets on original30 observations?",
        method="Reuse sealed original interfaces; jointV2 entering/interior junction evidence, terminal return evidence, independent window opening surface/crossing evidence; unknown preserved; source-bound record digests. Partial automatic supervision only.",
...
def execute(spec,run):
    ...
    from mtare_topo.teacher.gse_joint_reference_targets_v2 import produce_joint_reference_targets
