#!/usr/bin/env python3
"""Train one lightweight action-set decision-node detector."""
...
def _infer(model, dataset, *, device, batch_size: int) -> dict[str, np.ndarray]:
    ...
    with torch.inference_mode():
        for batch in loader:
            ...
            output["probability"].append(predicted["decision_probability"].cpu().numpy())
...
def _selection_mil(probability: np.ndarray, target: np.ndarray, episode: np.ndarray) -> dict[str, float]:
    ...
    return {
        ...
        "positive_episode_joint": positive_loss,
        "total": negative_loss + positive_loss,
    }
...
def main() -> int:
    ...
    if args.output_dir.exists():
        raise RuntimeError("action-set detector output exists; overwrite is forbidden")
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("formal action-set node training requires CUDA")
    ...
    if (
        manifest.get("schema_version") != "gse_action_set_node_cache_v1"
        or manifest.get("fit_observations") != 142184
        or manifest.get("selection_observations") != 45942
    ):
        raise RuntimeError("action-set cache manifest drift")
    ...
    fit = ActionSetNodeDataset(args.cache_dir, fit_rows)
    selection = ActionSetNodeDataset(args.cache_dir, selection_rows)
    ...
    for epoch in range(args.epochs):
        ...
        for batch in loader:
            predicted = _forward(model, batch, device)
            losses = action_set_episode_loss(
                predicted,
        ...
        if not np.array_equal(inferred["observation_row"], selection_rows):
            raise RuntimeError("action-set selection row order drift")
        ...
        if score["total"] < best_score:
            ...
            best_epoch = epoch
            torch.save({
                "schema_version": "gse_action_set_node_checkpoint_v1",
                "seed": args.seed,
                "epoch": epoch,
    ...
    summary = {
        "schema_version": "gse_action_set_node_training_seed_v1",
        "seed": args.seed,
        "epochs": args.epochs,
