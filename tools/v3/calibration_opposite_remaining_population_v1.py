"""Reuse original six-field encoding/binding; exact calibration289 population."""
import json
...
def export_population(root,run,scope,opened):
    ...
    if total!=146 or len(entries)!=13:raise ValueError('missing-only population incomplete')
    return dict(status='SIX_FIELD_INPUT_EXPORT_COMPLETE',observations=total,tasks=entries,reused_inputs=scope['reused_inputs'],
