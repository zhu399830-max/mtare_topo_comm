...
def main() -> int:
    ...
    synthetic = {"candidate_only": candidate_only, "objective": objective}
    ...
    summary = {
        "schema_version": "gse_sparse_relation_objective_recall_corrective_v1",
        "status": PASS if passed else FAIL,
        "scientific_pass": passed,
        "decision": "ALLOW_TIMEOUT_CORRECTED_THREE_SEED_TRAINING" if passed else "STOP_BEFORE_TRAINING",
        "totals": totals,
    ...
    fig.suptitle("Sparse relation metric corrective: every missed Teacher relation is an FN")
    for suffix in ("png", "pdf", "svg"):
        fig.savefig(output / f"gse_sparse_relation_objective_recall_corrective_v1.{suffix}", dpi=220)
    plt.close(fig)
