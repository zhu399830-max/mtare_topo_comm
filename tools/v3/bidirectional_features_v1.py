...
from mtare_topo.governance_bidirectional_features_v1 import SCHEMA, SLUG, POLICY, validate_card
from mtare_topo.governance_surface_selection import digest
from mtare_topo.governance_surface_material import read_pinned
from mtare_topo.data.bidirectional_feature_scope_v1 import compile_feature_scope
...
def freeze():
    ...
    approval = dict(status="APPROVED", approved_by="user-standing-scope-authorization", approved_at="2026-09-09",
        ...
        confirmation_reference="User standing autonomous authorization; PLAN allows same frozen encoder features for exact817 missing bidirectional fit/calibration observations, no training or new labels.",
        scope="Exact817 fiveframe inputs; same historical seed0epoch2 frozen encoder; no labels,selection,training,C08-C10 or graphs")
    ...
    command = ["env", "OMP_NUM_THREADS=1", "OPENBLAS_NUM_THREADS=1", "MKL_NUM_THREADS=1", "PYTHONHASHSEED=0",
               PYTHON, "tools/v3/bidirectional_features_v1.py", "--spec", str(root / SPEC), "--run-dir", str(root / run)]
    spec = dict(schema_version="v3_run_spec_v1", gate=3, date="20260909", slug=SLUG, seed=0, operation="data_export",
        ...
        question="Extract the original frozen encoder features for817 fixed rolling windows, shared by downstream C0/C1 diagnostics without teacher input.",
        ...
        baseline="Same encoder and compact feature contract as the completed307-observation comparison; no perception or graph performance claim.",
        fallback="Fail and seal on any source,environment,numeric or resource drift; no rerun or data substitution.",
        wall_time_cap_s=1800, estimated_cost=dict(compute="5090D inference only817windows", host_ram_gb=32, gpu_vram_gb=28, disk_gb=8, wall_time_hours=.5),
        acceptance_criteria=["All817 exact source windows exported once; no source IDs or labels in encoder input.",
        ...
        expected_evidence=["817compactcaches,source/weight hashes,per-observation provenance,environment,logs,summary,RUN_STATE,seal"],
...
def execute(spec, run):
    ...
    try:
        ...
        write(run / "config/feature_runtime.json", dict(**spec["environment"], torch=torch.__version__, gpu=torch.cuda.get_device_name(0)))
        ...
        with (run / "logs/observations.jsonl").open("x") as log:
            ...
            if len(entries) != 817 or module_state_sha256(bound.backbone) != bound.state_sha256:
