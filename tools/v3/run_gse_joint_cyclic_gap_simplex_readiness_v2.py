#!/usr/bin/env python3
"""Execute and seal the corrected Joint Cyclic Gap Simplex V2 readiness."""
...
CARD_STATUS = "APPROVED_FOR_ONE_IMMUTABLE_GSE_JOINT_CYCLIC_GAP_SIMPLEX_READINESS_V2"
...
def main() -> int:
    ...
    if run.name != run_id or load_json(run / "RUN_STATE.json").get("state") != "CREATED_NOT_EXECUTED": raise RuntimeError("gap-simplex V2 readiness executes exactly once")
    started = time.monotonic(); overall, error, result, returncode = FAIL, None, {}, None; before, after = {}, {}
    try:
        card = load_json(PROJECT_ROOT / spec["data_card"]); validation = validate_data_card(card)
        if not validation.passed or card.get("status") != CARD_STATUS: raise RuntimeError(f"gap-simplex V2 Data Card invalid: {validation.errors}")
        ...
        with (run / "logs/00_readiness.log").open("w", encoding="utf-8") as stream: returncode = subprocess.run(command, cwd=PROJECT_ROOT, env=process_environment, stdout=stream, stderr=subprocess.STDOUT, text=True, timeout=1200, check=False).returncode
        if returncode not in (0, 2): raise RuntimeError(f"gap-simplex V2 executor system failure: {returncode}")
        ...
        if (returncode == 0) != passed: raise RuntimeError("gap-simplex V2 status/return mismatch")
        ...
        if any(result.get(name) != 0 for name in forbidden): raise RuntimeError("gap-simplex V2 forbidden operation")
