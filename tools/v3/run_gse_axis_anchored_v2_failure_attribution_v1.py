#!/usr/bin/env python3
"""Execute and seal the frozen Axis-Anchored V2 failure attribution."""
...
FAIL = "FAIL_GSE_AXIS_ANCHORED_V2_FAILURE_ATTRIBUTION_V1"
CARD_STATUS = "APPROVED_FOR_ONE_IMMUTABLE_GSE_AXIS_ANCHORED_V2_FAILURE_ATTRIBUTION_V1"
...
TRAINING = PROJECT_ROOT / "results/gate3_semantics/gate3_20260829_gse_axis_anchored_event_relation_three_seed_training_v1_seed0"
...
def main() -> int:
    ...
    if run.name != run_id or load_json(run / "RUN_STATE.json").get("state") != "CREATED_NOT_EXECUTED":
        raise RuntimeError("V2 attribution executes exactly once")
    started = time.monotonic(); overall = FAIL; error = None; attribution = {}; before = {}; after = {}; peak_rss_kib = None; returncode = None
    try:
        ...
        environment = json.loads(subprocess.check_output([
            str(PYTHON), "-c", "import json,numpy,scipy,zarr,sys;print(json.dumps({'python':sys.version.split()[0],'numpy':numpy.__version__,'scipy':scipy.__version__,'zarr':zarr.__version__},sort_keys=True))"
        ], text=True))
        ...
        command = [
            str(PYTHON), str(PROJECT_ROOT / "tools/v3/execute_gse_axis_anchored_v2_failure_attribution_v1.py"),
            "--dataset-root", str(DATASET / "artifacts/dataset/train"),
            "--sequence-manifest", str(DATASET / "artifacts/sequence_manifest.jsonl"),
            "--association-pairs", str(DATASET / "artifacts/association_pairs_numeric.jsonl"),
            "--formal-summary", str(TRAINING / "metrics/summary.json"),
            "--slot-summary", str(SLOT / "metrics/summary.json"),
            "--current-source", str(PROJECT_ROOT / "src/mtare_topo/representation/gse_axis_anchored_event_relation.py"),
            "--predecessor-source", str(PROJECT_ROOT / "src/mtare_topo/representation/gse_circular_peak_geometry_model.py"),
            "--output-dir", str(output),
        ]
        for seed in range(3):
            command.extend(("--prediction-root", str(TRAINING / f"artifacts/models/seed{seed}/development_predictions")))
        process_environment = os.environ.copy(); process_environment["PYTHONPATH"] = str(PROJECT_ROOT / "src") + os.pathsep + str(PROJECT_ROOT / "tools/v3")
        ...
        if returncode != 0:
            raise RuntimeError("V2 attribution subprocess failed")
        attribution = load_json(output / "summary.json")
        if attribution.get("status") != PASS or attribution.get("scientific_pass") is not True:
            raise RuntimeError("V2 attribution result contract failed")
        required = [output / "summary.json", output / "figure_source.json", output / "relation_attribution.csv", *[output / f"gse_axis_anchored_v2_failure_attribution_v1.{suffix}" for suffix in ("png", "pdf", "svg")]]
        if any(not path.is_file() or path.stat().st_size == 0 for path in required):
            raise RuntimeError("V2 attribution evidence incomplete")
        after = {relative: sha256(PROJECT_ROOT / relative) for relative in before}
        if before != after:
            raise RuntimeError("frozen V2 attribution source changed")
        overall = PASS
    ...
    write_json(run / "metrics/summary.json", {
        "schema_version": "gse_axis_anchored_v2_failure_attribution_outer_v1", "overall_status": overall,
    ...
    entries = seal(run); print(json.dumps({"overall_status": overall, "error": error, "decision": attribution.get("decision"), "seal_entries": entries}, indent=2))
