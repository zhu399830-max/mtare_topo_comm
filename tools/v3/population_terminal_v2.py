...
from mtare_topo.governance_population_terminal_v2 import SCHEMA,SLUG,POLICY,validate_card
...
def freeze():
    ...
    command=["env","CUDA_VISIBLE_DEVICES=","OMP_NUM_THREADS=1","OPENBLAS_NUM_THREADS=1","MKL_NUM_THREADS=1","PYTHONHASHSEED=20260906",PYTHON,
        "tools/v3/population_terminal_v2.py","--spec",str(root/SPEC),"--run-dir",str(root/run)]
    ...
    spec=dict(schema_version="v3_run_spec_v1",gate=3,date="20260907",slug=SLUG,seed=20260906,operation="data_export",
        ...
        method="Original degree-one .05m/64 cap faces; original normalize64/cast32 ray inputs and Open3D exact t_hit equality; unique saved source; open10m ROI; no tolerance enlargement or labels.",
...
def execute(spec,run):
    ...
    from mtare_topo.teacher.gse_terminal_diagnostic_v2 import diagnose_observation
