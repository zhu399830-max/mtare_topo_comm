#!/usr/bin/env python3
"""Train one exit-token-aware open-set verifier seed on frozen GSE outputs."""
...
def _extract_features(
    ...
    token_batches: dict[str, list[np.ndarray]] = {key: [] for key in token_keys}
    identity_batches = []
    for batch in loader:
        ...
        for key in token_keys:
            token_batches[key].append(numpy_output[key])
    ...
    token_outputs = {
        key: np.concatenate(token_batches[key], axis=0)[order].astype(np.float32)
        for key in token_keys
    }
    ...
    if (
        ...
        or any(value.shape[0] != len(dataset) or not np.all(np.isfinite(value)) for value in token_outputs.values())
    ):
        raise RuntimeError("feature extraction did not cover C01--C08 exactly once")
    return features, token_outputs, identities, _sha256(checkpoint_path)
...
def train(
    ...
    features, token_outputs, feature_global_indices, checkpoint_sha = _extract_features(
        dataset_run, checkpoint_path, device
    )
    np.save(output_dir / "frozen_observation_features.npy", features, allow_pickle=False)
    np.savez_compressed(
        output_dir / "frozen_exit_token_outputs.npz",
        global_sequence_index=feature_global_indices,
        **token_outputs,
    )
    ...
    for partition in ("fit", "selection"):
        ...
        base_pair = symmetric_pair_features(
            normalized[left], normalized[right], arrays[f"{partition}_distance_m"]
        )
        left_tokens = {key: value[left] for key, value in token_outputs.items()}
        right_tokens = {key: value[right] for key, value in token_outputs.items()}
        token_pair = exit_token_pair_features(left_tokens, right_tokens)
        pair_by_partition[partition] = np.concatenate((base_pair, token_pair), axis=1).astype(np.float32)
    ...
    model = GSEExitTokenAssociationVerifier().to(device)
    ...
    for epoch in range(1, 31):
        ...
        for start in range(0, len(order), 2048):
            ...
            final_loss = hard_negative_tail_loss(output["association_score"], target)
        ...
        if score < best_loss - 1e-8:
            ...
            torch.save(
                {
                    "schema_version": "gse_exit_token_association_checkpoint_v2",
    ...
    summary = {
        "schema_version": "gse_exit_token_association_training_seed_v2",
        "overall_status": (
            "PASS_GSE_EXIT_TOKEN_ASSOCIATION_TRAINING_SEED_V2"
            if passed else "FAIL_GSE_EXIT_TOKEN_ASSOCIATION_TRAINING_SEED_V2"
        ),
        "seed": seed,
        ...
        "tail_risk_loss": "equal_weight_lowest_positive_and_highest_negative_same_count",
        "source_gse_checkpoint_sha256": checkpoint_sha,
