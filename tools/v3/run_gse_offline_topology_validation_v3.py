#!/usr/bin/env python3
"""Execute and seal one immutable C09 GSE-Graph V3 topology validation."""
...
ENSEMBLE = PROJECT_ROOT / "results/gate3_semantics/gate3_20260826_gse_distance_aware_ensemble_calibration_v1_seed0"
NODE_CALIBRATION: Path | None = None
EVALUATOR = PROJECT_ROOT / "tools/v3/evaluate_gse_offline_topology_v3.py"
CARD_STATUS = "APPROVED_FOR_ONE_IMMUTABLE_GSE_OFFLINE_TOPOLOGY_VALIDATION_V3"
RUN_SCHEMA = "gse_offline_topology_run_v3"
...
def _sources() -> dict:
    complete = {
        ...
        ENSEMBLE: "PASS_GSE_DISTANCE_AWARE_ENSEMBLE_CALIBRATION_V1",
    }
    result = {
        run.name: verify_complete_run_seal(PROJECT_ROOT, run, status)
        for run, status in complete.items()
    }
    ...
    result[VERIFIER.name] = verify_failed_component_run_seal(
        PROJECT_ROOT, VERIFIER, "FAIL_GSE_EXIT_TOKEN_ASSOCIATION_CORRECTIVE_V2"
    )
    if NODE_CALIBRATION is not None:
        result[NODE_CALIBRATION.name] = verify_complete_run_seal(
            PROJECT_ROOT,
            NODE_CALIBRATION,
            "PASS_GSE_NODE_MATCHABILITY_ENSEMBLE_CALIBRATION_V1",
        )
    ensemble = load_json(ENSEMBLE / "artifacts/calibration/summary.json")
    if (
        ensemble.get("overall_status") != "PASS_GSE_DISTANCE_AWARE_ENSEMBLE_CALIBRATION_V1"
        or ensemble.get("selection", {}).get("threshold") != 0.9431912302970886
        or ensemble.get("contract", {}).get("maximum_candidate_distance_m") != 16.0
    ):
        raise RuntimeError("frozen ensemble source drift")
    return result
...
def main() -> int:
    ...
    if run_dir.name != RUN_ID or load_json(run_dir / "RUN_STATE.json").get("state") != "CREATED_NOT_EXECUTED":
        raise RuntimeError("one-time C09 V3 run identity mismatch")
    if (
        spec.get("gate") != 4
        or spec.get("operation") != "topology_replay"
        or spec.get("user_authorization", {}).get("status") != "APPROVED"
    ):
        raise RuntimeError("C09 V3 topology scope is not authorized")
    card = load_json(PROJECT_ROOT / spec["data_card"])
    if card.get("status") != CARD_STATUS:
        raise RuntimeError("operation-bound C09 V3 Data Card mismatch")
    for name, record in spec["frozen_tools"].items():
        if _sha256(PROJECT_ROOT / record["path"]) != record["sha256"]:
            raise RuntimeError(f"frozen C09 V3 tool drift: {name}")
    for relative, expected in spec["frozen_inputs"].items():
        if _sha256(PROJECT_ROOT / relative) != expected:
            raise RuntimeError(f"frozen C09 V3 input drift: {relative}")
    ...
    try:
        if shutil.disk_usage(PROJECT_ROOT).free < 8 * 1024**3:
            raise RuntimeError("less than 8 GiB free before C09 V3 replay")
        ...
        environment = json.loads(subprocess.check_output([
            str(PYTHON), "-c",
            "import json,numpy,scipy,torch,zarr,sys;print(json.dumps({'python':sys.version.split()[0],'numpy':numpy.__version__,'scipy':scipy.__version__,'torch':torch.__version__,'cuda':torch.version.cuda,'zarr':zarr.__version__},sort_keys=True))",
        ], text=True))
        ...
        command = [
            ...
            "--ensemble-run", str(ENSEMBLE),
            "--output-dir", str(output_dir),
            "--device", "cuda:0",
        ]
        if NODE_CALIBRATION is not None:
            command.extend(["--node-calibration-run", str(NODE_CALIBRATION)])
        ...
        with (run_dir / "logs/01_offline_topology.log").open("w", encoding="utf-8") as stream:
            completed = subprocess.run(
                ...
                stdout=stream, stderr=subprocess.STDOUT, timeout=8 * 60 * 60, check=False,
            )
        ...
        if result_bytes > DISK_LIMIT_BYTES:
            raise RuntimeError("C09 V3 evidence exceeded the frozen 5 GiB limit")
        if maximum_child_rss_kib > RSS_LIMIT_KIB:
            raise RuntimeError("C09 V3 exceeded the frozen 16 GiB child RSS limit")
        if completed.returncode != 0 or evaluator_summary.get("overall_status") != PASS_STATUS:
            raise RuntimeError("C09 V3 failed the pre-registered scientific gate")
        source_after = _sources()
        if source_before != source_after:
            raise RuntimeError("source evidence changed during C09 V3 replay")
        overall = PASS_STATUS
