#!/usr/bin/env python3
"""Freeze JCGS boundary readiness V5R with deterministic cuBLAS."""
...
RUN_ID = "gate3_20260829_gse_joint_cyclic_gap_simplex_readiness_v5r_seed0"
SPEC = PROJECT_ROOT / "configs/v3/gate3/gse_joint_cyclic_gap_simplex_readiness_v5r.json"
DATA_CARD = "configs/v3/gate3/data_cards/gse_joint_cyclic_gap_simplex_readiness_v5r.json"
...
def main() -> int:
    if SPEC.exists():
        raise RuntimeError("gap-simplex V5R spec already exists")
    card = load_json(PROJECT_ROOT / DATA_CARD)
    validation = validate_data_card(card)
    if not validation.passed:
        raise RuntimeError(f"gap-simplex V5R Data Card invalid: {validation.errors}")
    ...
    failed_v5 = "results/gate3_semantics/gate3_20260829_gse_joint_cyclic_gap_simplex_readiness_v5_seed0"
    inputs = [
        DATA_CARD,
        ...
        f"{failed_v5}/RUN_STATE.json", f"{failed_v5}/metrics/summary.json", f"{failed_v5}/artifacts/evidence_sha256.txt",
    ...
    tools = {
        ...
        "prior_runner": "tools/v3/run_gse_joint_cyclic_gap_simplex_readiness_v5.py",
        "runner": "tools/v3/run_gse_joint_cyclic_gap_simplex_readiness_v5r.py",
        "freezer": "tools/v3/freeze_gse_joint_cyclic_gap_simplex_readiness_spec_v5r.py",
    ...
    spec = {
        ...
        "date": "20260829", "slug": "gse_joint_cyclic_gap_simplex_readiness_v5r", "seed": 0,
        "operation": "audit", "data_card": DATA_CARD,
        "question": "Does the unchanged canonical circular-index correction pass exact CUDA replay when the frozen deterministic cuBLAS environment is propagated correctly?",
        "method": "Repeat V5 byte-for-byte at the data/model/criteria level and add only CUBLAS_WORKSPACE_CONFIG=:4096:8 before the CUDA subprocess.",
        "baseline": "V5 failed before loss evaluation because deterministic cuBLAS workspace was omitted; no scientific check ran and zero optimizer steps occurred.",
        "fallback": "Stop JCGS before retraining if the exact batch or prior readiness fails under the corrected environment.",
        ...
        "acceptance_criteria": [
            ...
            "Exact 128-row deterministic CUDA loss forward/backward completes with finite gradients.",
            "All 12 unit regressions and prior readiness checks pass.",
            "Only environment propagation differs from V5; zero optimizer/checkpoint/C09/C10/M-TARE/graph/planner."
        ],
        ...
        "expected_evidence": ["Exact boundary coordinate and deterministic CUDA replay summary.", "Unit and prior readiness evidence.", "Environment, commands, RUN_STATE and SHA-256 seal."],
        "estimated_cost": {"compute": "One deterministic CUDA batch forward/backward plus CPU readiness", "wall_time_hours": 0.15, "host_ram_gb": 6, "gpu_memory_gb": 4, "disk_gb": 0.1},
        ...
        "command": ["/usr/bin/timeout", "1800s", PYTHON, "tools/v3/run_gse_joint_cyclic_gap_simplex_readiness_v5r.py", "--spec", str(SPEC), "--run-dir", str(PROJECT_ROOT / f"results/gate3_semantics/{RUN_ID}")]
    }
