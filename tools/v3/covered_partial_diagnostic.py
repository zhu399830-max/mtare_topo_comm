"""Single immutable partial teacher diagnostic; never train."""
import argparse
import json
...
from mtare_topo.evaluation.covered_partial_diagnostic import execute as diagnose
...
def execute(spec,run):
    ...
    def resource_check():
        if resource.getrusage(resource.RUSAGE_SELF).ru_maxrss*1024>4*1024**3: raise MemoryError('4GiB RAM cap')
        if sum(p.stat().st_size for p in run.rglob('*') if p.is_file())>2*1024**3: raise RuntimeError('2GiB output cap')
    def expired(*_): raise TimeoutError('partial diagnostic time cap')
    ...
    try:
        ...
        with (run/'logs/progress.jsonl').open('x') as log:
            ...
            metrics=diagnose(ROOT,run,card['scope'],progress,resource_check)
        verify()
    except Exception:
        error=traceback.format_exc();(run/'logs/error.log').write_text(error)
    finally: signal.alarm(0)
    summary=dict(metrics,state='PARTIAL_DIAGNOSTIC_COMPLETE_NOT_TRAINING_QUALIFICATION' if error is None else 'PARTIAL_DIAGNOSTIC_EXECUTION_FAIL',
