...
from mtare_topo.governance_development_paired_train_v1 import SCHEMA,SLUG,POLICY,validate_card
...
from mtare_topo.representation.development_paired_training import build_model,state_sha256,batch_schedule,forward_observation,train_update
...
def freeze():
    ...
    approval=dict(status='APPROVED',approved_by='user-standing-scope-authorization',approved_at='2026-09-09',
        ...
        confirmation_reference='User standing autonomous authorization for C01-C07 paired representation training; PLAN unified reader/loss/scoring and existing5090 checks complete. Partial reference diagnostic only, not full structure or graph success.')
    ...
    files=sorted({str(p.relative_to(ROOT)) for folder in ('src/mtare_topo','tools/v3') for p in (ROOT/folder).rglob('*.py')}|{CARD,MATCHING})
    command=['env','OMP_NUM_THREADS=1','OPENBLAS_NUM_THREADS=1','MKL_NUM_THREADS=1','PYTHONHASHSEED=0','CUBLAS_WORKSPACE_CONFIG=:4096:8',PYTHON,
             'tools/v3/development_paired_train_v1.py','--spec',str(ROOT/SPEC),'--run-dir',str(ROOT/RUN)]
    spec=dict(schema_version='v3_run_spec_v1',gate=3,date='20260909',slug=SLUG,seed=0,operation='training',
        ...
        question='Under identical inputs, initialization and budget, do voxel or SPG groups improve known junction positions and witnessed branch recovery over scanning-layout groups?',
        method='Existing point encoder and block query/branch head488584parameters; frozen context; shared seed0 schedule; AdamW1e-3wd1e-4 micro1accum4; each2000updates; no best checkpoint selection.',
