"""Narrow authority for source-section diagnostics; no training labels."""
from mtare_topo.governance_surface_selection import digest

SCHEMA = "v3_surface_section_diagnostic_card_v1"
SLUG = "gse_surface_section_diagnostic_v1"
POLICY = {"radius_m":10.,"mesh_axial_m":.05,"mesh_angular":64,"extra_range_error_m":0.,
...
def validate_card(card):
    ...
    if type(card) is not dict or set(card)!={"schema_version","card_id","operation","scope","scope_sha256","policy","approval"}:
        return ValidationReport(False,("closed section diagnostic card required",))
    if (card["schema_version"],card["card_id"],card["operation"])!=(SCHEMA,SLUG,"data_export"):
        errors.append("section-only export scope required")
