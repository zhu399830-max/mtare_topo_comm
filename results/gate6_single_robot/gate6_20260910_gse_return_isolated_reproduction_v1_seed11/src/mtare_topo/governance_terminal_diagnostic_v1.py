"""Narrow authority for source-terminal diagnostics; no training labels."""
from mtare_topo.governance_surface_selection import digest

SCHEMA = "v3_surface_terminal_diagnostic_card_v1"
SLUG = "gse_surface_terminal_diagnostic_v1"
...
def validate_card(card):
    ...
    if type(card) is not dict or set(card)!={"schema_version","card_id","operation","scope","scope_sha256","policy","approval"}:
        return ValidationReport(False,("closed terminal diagnostic card required",))
    if (card["schema_version"],card["card_id"],card["operation"])!=(SCHEMA,SLUG,"data_export"):
        errors.append("terminal-only export scope required")
