...
def freeze():
    ...
    write(ROOT/SPEC,dict(schema_version='v3_run_spec_v1',gate=GATE,date='20260910',slug=SLUG,
        ...
        method='Existing organized sensor operator and finite geometry frontend; '+('tentative XY requests through original local planner' if VERSION>=3 else 'no waypoint publisher'),
        ...
        image=IMAGE,source_sha256={str(p.relative_to(ROOT)):sha(p) for p in sources}))
    print(SPEC)
...
def execute():
    ...
    write(evidence/'policy.json',dict(composition_policy=dict(max_residual_m=.01,
        ...
        anchor_spacing_m=4.,lookahead_m=4.,rigid_motion=VERSION>=4))
...
if __name__=='__main__':
    p=argparse.ArgumentParser();p.add_argument('--freeze',action='store_true');p.add_argument('--execute',action='store_true');p.add_argument('--version',type=int,choices=[1,2,3,4,5,6,7,8,9],default=1);a=p.parse_args()
