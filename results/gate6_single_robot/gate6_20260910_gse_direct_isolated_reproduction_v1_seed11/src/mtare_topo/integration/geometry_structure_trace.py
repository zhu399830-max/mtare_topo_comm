...
def geometry_structure_record(observation, *, source_frame_keys,
    ...
    for index, primitive in enumerate(observation.primitives):
        ...
        primitives.append(dict(index=index, fit_status=fit.reason,
            contributing_frames=(list(primitive.contributing_frames)
                                 if hasattr(primitive,'contributing_frames') else None),
            current_sector_columns=(list(primitive.current_sector_columns)
                                    if getattr(primitive,'current_sector_columns',None) is not None else None),
