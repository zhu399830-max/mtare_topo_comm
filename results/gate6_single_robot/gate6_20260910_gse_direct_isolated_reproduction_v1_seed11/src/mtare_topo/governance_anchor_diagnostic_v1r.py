"""Narrow authority for source-anchor diagnostics; no training labels."""
from mtare_topo.governance_surface_selection import digest

SCHEMA = "v3_surface_anchor_diagnostic_card_v1r"
SLUG = "gse_surface_anchor_diagnostic_v1r"
...
def validate_card(card):
    ...
    if type(card) is not dict or set(card)!={"schema_version","card_id","operation","scope","scope_sha256","policy","approval"}:
        return ValidationReport(False,("closed anchor diagnostic card required",))
    if (card["schema_version"],card["card_id"],card["operation"])!=(SCHEMA,SLUG,"data_export"):
        errors.append("anchor-only export scope required")
