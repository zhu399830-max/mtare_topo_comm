...
def validate_annotation_plan(card: Mapping[str, Any]) -> ValidationReport:
    ...
    for key in ("planned_ai_sample_count", "planned_human_gold_count"):
        ...
        if (key == 'planned_human_gold_count' and value == 0
                and card.get('schema_version') in {'gse_ai_three_case_pilot_v1','gse_ai_fixed_fit_review_v1'}):
            continue
        if not isinstance(value, int) or isinstance(value, bool) or value <= 0:
            ...
            if not data_card_path.is_file():
                ...
                try:
                    ...
                    if operation == 'ai_annotation' and gate != 1:
                        if gate != 3 or card.get('schema_version') not in {'gse_ai_three_case_pilot_v1','gse_ai_fixed_fit_review_v1'}:
